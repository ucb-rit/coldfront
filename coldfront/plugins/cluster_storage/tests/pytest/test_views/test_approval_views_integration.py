...
class TestStorageRequestDetailView:
    ...
    def test_requester_can_view_own_request(
        ...
        """Test requester can view their own storage request."""
...
class TestStorageRequestReviewEligibilityView:
    ...
    def test_get_displays_eligibility_review_form(
        ...
        assert 'storage_request' in response.context
    ...
    def test_post_redirects_to_detail_view(
        ...
        assert response.status_code == 302
        assert f'/storage-requests/{request.pk}/' in response.url
...
class TestStorageRequestReviewIntakeConsistencyView:
    ...
    def test_get_displays_intake_consistency_review_form(
        ...
        client.force_login(staff_user)
        url = reverse('storage-request-review-intake-consistency',
                     kwargs={'pk': request.pk})
        response = client.get(url)
...
class TestStorageRequestReviewSetupView:
    ...
    def test_get_displays_setup_form(
        ...
        client.force_login(staff_user)
        url = reverse('storage-request-review-setup',
                     kwargs={'pk': request.pk})
        response = client.get(url)
...
class TestStorageRequestEditView:
    ...
    def test_get_displays_edit_form(
        ...
        client.force_login(staff_user)
        url = reverse('storage-request-edit', kwargs={'pk': request.pk})
        response = client.get(url)
...
class TestStorageRequestReviewDenyView:
    ...
    def test_get_displays_deny_form(
        ...
        client.force_login(staff_user)
        url = reverse('storage-request-deny', kwargs={'pk': request.pk})
        response = client.get(url)
...
class TestStorageRequestUndenyView:
    ...
    def test_undeny_denied_request_returns_to_under_review(
        ...
        client.force_login(staff_user)
        url = reverse('storage-request-undeny', kwargs={'pk': request.pk})
...
class TestStorageRequestListView:
    """Test storage request list view."""
    ...
    def test_superuser_can_access_list(
        ...
        url = reverse('storage-request-list')
        response = client.get(url)

        assert response.status_code == 200
        assert 'storage_requests' in response.context
    ...
    def test_staff_with_view_permission_can_access(
        ...
        permission = Permission.objects.get(
            codename='can_view_all_storage_requests'
        )
        staff_user.user_permissions.add(permission)
    ...
    def test_list_displays_multiple_requests(
        ...
        storage_requests = response.context['storage_requests']
        assert request1 in storage_requests
        assert request2 in storage_requests
    ...
    def test_list_filters_by_status(
        ...
        url = reverse('storage-request-list') + '?status=Under Review'
        response = client.get(url)

        storage_requests = list(response.context['storage_requests'])
        assert request1 in storage_requests
        assert request2 not in storage_requests
