...
from coldfront.plugins.departments.models import (
    Department,
    HistoricalUserDepartment,
    UserDepartment,
)
...
class TestUpdateDepartmentsView(TransactionTestBase):
    ...
    def _assert_historical_user_departments(
        self, user, department, expected_history_types
    ):
    ...
    def _assert_user_departments(
        ...
        for user_department in user_departments:
            ...
            expected_is_authoritative = (
                expected_departments_name_and_is_authoritative.pop(department_name)
            )
            self.assertEqual(
    ...
    def test_departments_required(self):
        ...
        UserDepartment.objects.create(user=self.user, department=self.department1)
        ...
        try:
            UserDepartment.objects.get(user=self.user, department=self.department1)
    ...
    def test_sets_selected_departments_non_authoritatively(self):
        ...
        self._assert_historical_user_departments(self.user, self.department1, ["+"])
        ...
        self._assert_historical_user_departments(self.user, self.department2, ["+"])
        self._assert_historical_user_departments(self.user, self.department3, ["+"])
    ...
    def test_creates_data_source_departments_and_sets_authoritatively(self):
        ...
        self.assertFalse(Department.objects.filter(name="Department F").exists())
        self.assertFalse(Department.objects.filter(name="Department L").exists())
    ...
    def test_authoritative_not_overridden_as_non_authoritative(self):
        ...
        department_f = Department.objects.create(name="Department F", code="DEPTF")
    ...
    def test_outdated_authoritative_departments_unset(self):
        ...
        department_m = Department.objects.create(name="Department M", code="DEPTM")
    ...
    def test_data_source_error_authoritative_unaffected(self):
        ...
        method_to_patch = (
            "coldfront.plugins.departments.utils.queries.fetch_departments_for_user"
        )
