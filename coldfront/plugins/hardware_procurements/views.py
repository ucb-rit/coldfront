...
class HardwareProcurementDetailView(
...
class HardwareProcurementListView(
    LoginRequiredMixin, UserPassesTestMixin, TemplateView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        for status in ("completed", "inactive", "pending", "retired"):
            context[f"{status}_url"] = f"{list_url}?{urlencode({'status': status})}"
    ...
    def _get_procurements(self):
        ...
        if not user_can_see_all_procurements:
            fetch_hardware_procurements_kwargs["user_data"] = UserInfoDict.from_user(
                user
            )
