...
def get_data_source(data_source=None, **kwds):
    klass = import_string(data_source or settings.DATA_SOURCE_CONFIG["DATA_SOURCE"])
...
def fetch_hardware_procurements(data_source=None, user_data=None, status=None):
    ...
    return data_source.fetch_hardware_procurements(user_data=user_data, status=status)
