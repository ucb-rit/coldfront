...
def mock_hardware_procurements():
    ...
    return [
        MockHardwareProcurement(
            _id="4fcd2992", pi_emails=["pi1@email.com"], poc_emails=["poc1@email.com"]
        ),
        MockHardwareProcurement(
            _id="586fcf26", pi_emails=["pi1@email.com"], poc_emails=["poc2@email.com"]
        ),
        MockHardwareProcurement(
            _id="f3920d70", pi_emails=["pi2@email.com"], poc_emails=["poc1@email.com"]
        ),
        MockHardwareProcurement(
            _id="947ff714", pi_emails=["pi2@email.com"], poc_emails=["poc2@email.com"]
        ),
        MockHardwareProcurement(
            _id="ba0dcb06", pi_emails=["pi3@email.com"], poc_emails=["poc3@email.com"]
        ),
    ]
...
class TestHardwareProcurementsCacheManager:
    ...
    def _populate_cache(
        ...
        with patch(self._cache_module, mock_cache):
            with patch(
                self._look_up_func_module, side_effect=mock_look_up_user_by_email
            ):
    ...
    def test_get_cached_procurements(
        self,
        mock_hardware_procurements,
    ...
    def test_get_cached_procurements_by_user(
        self,
        mock_hardware_procurements,
        mock_hardware_procurements_by_id,
        mock_look_up_user_by_email,
        ...
        with patch(self._cache_module, mock_cache):
            for hardware_procurement in cache_manager.get_cached_procurements_for_user(
                user_data
            ):
                procurement_id = hardware_procurement.get_id()
    ...
    def test_get_cached_procurements_by_user_omits_if_not_associated(
        self, mock_hardware_procurements, mock_look_up_user_by_email, user_data
    ):
        mock_cache = MockCache()
        cache_manager = HardwareProcurementsCacheManager(self._cache_key)
        self._populate_cache(
        ...
        with patch.object(
            MockHardwareProcurement, "is_user_associated", return_value=False
        ):
            ...
            # yielding.
            with patch(self._cache_module, mock_cache):
                generator = cache_manager.get_cached_procurements_for_user(user_data)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                MockHardwareProcurement(
                    _id="edf11235", pi_emails=["pi1@email.com", "pi2@email.com"]
                ),
                [1, 2],
            ),
            (
            ...
            (
                MockHardwareProcurement(
                    _id="565210ac", poc_emails=["poc1@email.com", "poc2@email.com"]
                ),
                [3, 4],
            ),
    ...
    def test_lookup_associated_user_ids(
        self, hardware_procurement, sorted_expected_user_ids, mock_look_up_user_by_email
    ):
        cache_manager = HardwareProcurementsCacheManager(self._cache_key)
        with patch(self._look_up_func_module, side_effect=mock_look_up_user_by_email):
    ...
    def test_populate_cache(
        ...
        for user_id, user_procurements_by_id in mock_cache[self._cache_key].items():
            ...
            for procurement_id, procurement in user_procurements_by_id.items():
                assert procurement_id in expected_procurement_ids
                assert procurement == mock_hardware_procurements_by_id[procurement_id]
                expected_procurement_ids.remove(procurement_id)
