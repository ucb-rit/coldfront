...
class DirectoryService:
    ...
    @staticmethod
    def get_directory_name_for_project(project, fsa_request=None):
        """
        Get the directory name for a project's faculty storage.

        This method handles both existing allocations and pending requests:
        1. If fsa_request provided, checks its state for directory_name,
           then falls back to project name (for pending requests)
        2. If no fsa_request, looks up existing allocation
        3. Returns None if no allocation exists and no fsa_request provided

        Args:
            project: The Project object
            fsa_request: Optional FacultyStorageAllocationRequest to check
                           for proposed directory name

        Returns:
            The directory name (string), or None if no allocation exists
            and no fsa_request provided
    ...
    @staticmethod
    def get_directory_path_for_project(project, fsa_request=None):
        """
        ...
        Args:
            project: The Project object
            fsa_request: Optional FacultyStorageAllocationRequest
