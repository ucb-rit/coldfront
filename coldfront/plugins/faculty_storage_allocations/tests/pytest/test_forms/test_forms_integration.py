...
class TestReviewStatusFormIntegration:
    ...
    def test_form_with_database_request_data(self, test_project, test_user, test_pi):
        ...
        FacultyStorageAllocationRequestService.update_eligibility_state(
            request, form.cleaned_data["status"], form.cleaned_data["justification"]
        )
    ...
    def test_form_history_tracking(self, test_project, test_user, test_pi):
...
class TestFSARequestSearchFormIntegration:
    ...
    def test_form_filters_requests_by_status(self, test_project, test_user, test_pi):
...
class TestFSARequestFormIntegration:
    ...
    def test_form_disables_ineligible_pis(
        self, test_project, test_pi, project_user_role_pi, project_user_status_active
    ):
        ...
        # Create an existing FSA request for this PI (makes them ineligible)
        create_fsa_request(
            status="Under Review",
            project=test_project,
            requester=test_pi,
        ...
        assert pi_project_user.pk in form.fields["pi"].widget.disabled_choices
    ...
    def test_form_only_shows_active_pi_project_users(
        ...
        inactive_user = User.objects.create_user(
            username="inactive_pi", email="inactive@test.com"
        )
