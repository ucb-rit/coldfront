"""Shared test utilities for faculty_storage_allocations plugin tests.
...
def assert_request_state_valid(request):
    """Assert that a FSA request has valid state structure.
...
def assert_request_has_allocations(request, expected_count=2):
    """Assert that a FSA request has created the expected allocations.
