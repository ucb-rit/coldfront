...
class TestEligibilityServiceUnit:
    ...
    def test_is_eligible_when_no_existing_requests_and_whitelist_disabled(
        ...
        mock_request_model.objects.filter.return_value.exclude.return_value.exists.return_value = False

        # Execute
        is_eligible, reason = FSARequestEligibilityService.is_eligible_for_request(
            mock_user
        )
        ...
        mock_request_model.objects.filter.assert_called_once_with(pi=mock_user)
        mock_request_model.objects.filter.return_value.exclude.assert_called_once_with(
            status__name="Denied"
        )
    ...
    def test_is_eligible_when_on_whitelist_and_no_existing_requests(
        ...
        mock_email_address.objects.filter.assert_called_once_with(user=mock_user)
    ...
    def test_is_not_eligible_when_has_non_denied_request(
        ...
        mock_request_model.objects.filter.return_value.exclude.return_value.exists.return_value = True
...
class TestEligibilityServiceComponent:
    ...
    def test_is_eligible_when_no_existing_requests(self, mock_settings, test_pi):
    ...
    def test_is_eligible_when_only_has_denied_requests(
        self, mock_settings, test_pi, test_project, test_user, fsa_request_status_denied
    ):
    ...
    def test_whitelist_enabled_with_matching_email(self, mock_settings, test_pi):
        ...
        EmailAddress.objects.create(
            user=test_pi, email="pi@berkeley.edu", verified=True, primary=True
        )
    ...
    def test_whitelist_enabled_with_non_matching_email(self, mock_settings, test_pi):
    ...
    def test_whitelist_enabled_with_multiple_emails_one_matches(
        ...
        EmailAddress.objects.create(
            user=test_pi, email="pi@gmail.com", verified=True, primary=True
        )
        EmailAddress.objects.create(
            user=test_pi,
        ...
        EmailAddress.objects.create(
            user=test_pi, email="pi@lbl.gov", verified=True, primary=False
        )
