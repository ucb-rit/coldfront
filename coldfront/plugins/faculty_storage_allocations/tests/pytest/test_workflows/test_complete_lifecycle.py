...
class TestRequestLifecycleEndToEnd:
    ...
    def test_user_submits_request_admin_approves_agent_completes(
        self,
        mock_send_completion_email_to_users,
        test_project,
        test_user,
        test_pi,
        staff_user,
        ...
        complete_url = reverse(
            "faculty-storage-allocation-request-complete", kwargs={"pk": request.id}
        )
        complete_response = api_client.patch(
            complete_url, {"directory_name": "fc_test_dir"}, format="json"
        )
    ...
    def test_user_submits_request_admin_denies(self, test_project, test_user, test_pi):
    ...
    def test_stale_request_recovery_workflow(self, test_project, test_user, test_pi):
...
class TestAPIWorkflow:
    ...
    def test_api_agent_full_workflow(
        ...
        complete_url = reverse(
            "faculty-storage-allocation-request-complete", kwargs={"pk": claimed_id}
        )
        complete_response = api_client.patch(
            complete_url, {"directory_name": "fc_api_test"}, format="json"
        )
    ...
    def test_api_multiple_request_workflow(
        ...
        for i in range(3):
            ...
            complete_response = api_client.patch(
                complete_url, {"directory_name": f"fc_test_{i}"}, format="json"
            )
            assert complete_response.status_code == status.HTTP_200_OK
...
class TestWebUIWorkflow:
    ...
    def test_denial_state_transitions(self, test_project, test_user, test_pi):
        ...
        FacultyStorageAllocationRequestService.update_intake_consistency_state(
            request, "Denied", "Data inconsistency detected"
        )
        FacultyStorageAllocationRequestService.deny_request(request)
        ...
        assert (
            "inconsistency"
            in request.state["intake_consistency"]["justification"].lower()
        )
    ...
    def test_allocation_creation_during_completion(
        self, test_project, test_user, test_pi, resource_faculty_storage_directory
    ):
        ...
        assert resource_faculty_storage_directory in allocation.resources.all()
