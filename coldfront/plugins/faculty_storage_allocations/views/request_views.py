...
class FSARequestAccessMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        """Check if the user has permission to access FSA requests."""
...
class FSARequestLandingView(LoginRequiredMixin, FSARequestAccessMixin, TemplateView):
    """A view for the landing page for FSA requests."""

    template_name = "faculty_storage_allocations/request/fsa_request_landing.html"
...
class FSARequestView(LoginRequiredMixin, FSARequestAccessMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        is_eligible, reason = FSARequestEligibilityService.is_eligible_for_request(pi)
        if not is_eligible:
        ...
        try:
            ...
            logger.exception(f"Error creating FSA request: {e}")
            return self.form_invalid(form)
