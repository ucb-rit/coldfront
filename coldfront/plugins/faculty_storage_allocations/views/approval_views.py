...
class FSARequestReadOnlyAccessMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        if user.has_perm(
            "faculty_storage_allocations.can_view_all_fsa_requests"
        ) or user.has_perm("faculty_storage_allocations.can_manage_fsa_requests"):
            return True

        # Allow the requester and PI to view their own request
        if hasattr(self, "fsa_request"):
            if user == self.fsa_request.requester or user == self.fsa_request.pi:
                return True
    ...
    def add_amount_context(self, context):
        ...
        if hasattr(self, "fsa_request"):
            ...
            context["approved_amount_tb"] = (
                ...
                if self.fsa_request.approved_amount_gb
                else None
            )
        return context
...
class FSARequestViewMixin(FSARequestAmountMixin):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        self.fsa_request = get_object_or_404(FacultyStorageAllocationRequest, pk=pk)
    ...
    def get_context_data(self, **kwargs):
        ...
        user = self.request.user
        context["is_allowed_to_manage_request"] = user.is_superuser or user.has_perm(
    ...
    def update_request_status_from_reviews(self):
        ...
        try:
            ...
            if eligibility_status == "Denied" or intake_status == "Denied":
                FacultyStorageAllocationRequestService.deny_request(self.fsa_request)
                messages.success(self.request, "The request has been denied.")
                return True

            # If both reviews are approved, move to Approved - Processing
            elif eligibility_status == "Approved" and intake_status == "Approved":
                FacultyStorageAllocationRequestService.approve_request(self.fsa_request)
            ...
            elif eligibility_status == "Pending" or intake_status == "Pending":
            ...
            messages.error(
                self.request, "Unexpected failure. Please contact an administrator."
            )
            logger.exception(
                f"Error updating request status for {self.fsa_request.pk}: {e}"
            )
            return False
...
class FSARequestDetailView(
    LoginRequiredMixin, FSARequestViewMixin, FSARequestReadOnlyAccessMixin, TemplateView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["allow_editing"] = (
            user.is_superuser
            or user.has_perm("faculty_storage_allocations.can_manage_fsa_requests")
        ) and self.fsa_request.status.name == "Under Review"
        ...
        context["proposed_directory_path"] = (
            DirectoryService.get_directory_path_for_project(
                self.fsa_request.project, fsa_request=self.fsa_request
            )
        )
        ...
        try:
            latest_update_timestamp = self.fsa_request.latest_update_timestamp()
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not (
            ...
            messages.error(
                request, "You do not have permission to complete FSA requests."
            )
        ...
        if not eligibility_approved:
            messages.error(
                request, "Cannot complete request: Eligibility has not been approved."
            )
            return self.get(request, *args, **kwargs)
        ...
        if not setup_complete:
            messages.error(
                request, "Cannot complete request: Setup has not been completed."
            )
            return self.get(request, *args, **kwargs)
        ...
        try:
            ...
            messages.error(
                request, "Unexpected failure. Please contact an administrator."
            )
            logger.exception(f"Error completing FSA request {fsa_request.pk}: {e}")
            return self.get(request, *args, **kwargs)
    ...
    def _get_checklist(self):
        ...
        checklist.append(
            [
                ...
                reverse(
                    "faculty-storage-allocation-request-review-setup", kwargs={"pk": pk}
                ),
            ]
        )
    ...
    def _get_setup_status(self):
        ...
        if (
            ...
            or state["intake_consistency"]["status"] == "Denied"
        ):
            return "N/A"
    ...
    def _is_checklist_complete(self):
        ...
        return (
            ...
            and state["intake_consistency"]["status"] == "Approved"
            and state["setup"]["status"] == "Complete"
        )


class FSARequestListView(
    LoginRequiredMixin, FSARequestViewAllAccessMixin, TemplateView
):
    template_name = "faculty_storage_allocations/approval/fsa_request_list.html"

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        fsa_requests = FacultyStorageAllocationRequest.objects.all().order_by(
            "-request_time"
        )
        ...
        if search_form.is_valid():
            ...
            if data.get("project"):
                fsa_requests = fsa_requests.filter(project=data["project"])
            if data.get("pi"):
                fsa_requests = fsa_requests.filter(pi=data["pi"])
            if data.get("status"):
                fsa_requests = fsa_requests.filter(status__name=data["status"])
...
class FSARequestEditView(
    LoginRequiredMixin, FSARequestAdminAccessMixin, FSARequestViewMixin, FormView
):
    ...
    def get_initial(self):
        ...
        old_amount = (
            self.fsa_request.approved_amount_gb or self.fsa_request.requested_amount_gb
        )
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            (f"Storage amount updated from {old_amount} GB to {storage_amount_gb} GB."),
        )

        return super().form_valid(form)


class FSARequestReviewEligibilityView(
    LoginRequiredMixin, FSARequestAdminAccessMixin, FSARequestViewMixin, FormView
):
...
class FSARequestReviewIntakeConsistencyView(
    LoginRequiredMixin, FSARequestAdminAccessMixin, FSARequestViewMixin, FormView
):
    form_class = FSARequestReviewStatusForm
    template_name = "faculty_storage_allocations/approval/fsa_request_review.html"
...
class FSARequestReviewSetupView(
    LoginRequiredMixin, FSARequestAdminAccessMixin, FSARequestViewMixin, FormView
):
    ...
    def get_initial(self):
        ...
        initial["directory_name"] = (
            setup.get("directory_name", "") or self.fsa_request.project.name
        )
        return initial
    ...
    def form_valid(self, form):
        ...
        if status == "Complete":
            FacultyStorageAllocationRequestService.update_setup_state(
                self.fsa_request, directory_name=directory_name, status="Complete"
            )
            messages.success(
                self.request,
            ...
            FacultyStorageAllocationRequestService.update_setup_state(
                self.fsa_request, status=status
            )
            messages.success(self.request, f"Setup status updated to {status}.")

        return super().form_valid(form)


class FSARequestReviewDenyView(
    LoginRequiredMixin, FSARequestAdminAccessMixin, FSARequestViewMixin, FormView
):
...
class FSARequestUndenyView(
    LoginRequiredMixin, FSARequestAdminAccessMixin, FSARequestViewMixin, View
):
    def dispatch(self, request, *args, **kwargs):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            FacultyStorageAllocationRequestService.undeny_request(self.fsa_request)
