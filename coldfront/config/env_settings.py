...
EMAIL_BACKEND = env(
    "DJANGO__EMAIL_BACKEND", default="django.core.mail.backends.smtp.EmailBackend"
)
...
EMAIL_DEVELOPMENT_EMAIL_LIST = env.list("COLDFRONT__EMAIL_DEVELOPMENT_EMAIL_LIST")
...
if (
    env.bool("DJANGO_FLAGS__LRC_ONLY_VALUE")
    and env("HPCS__BILLING_VALIDATOR_BACKEND", default=None) is not None
):
...
if env.bool("HPCS__ENABLE_DJANGO_DEBUG_TOOLBAR", default=False):
    EXTRA_EXTRA_APPS += ["debug_toolbar"]

    EXTRA_EXTRA_MIDDLEWARE += ["debug_toolbar.middleware.DebugToolbarMiddleware"]

    # IP addresses that can view the django debug toolbar.
    INTERNAL_IPS = env.list("HPCS__DJANGO_DEBUG_TOOLBAR_INTERNAL_IPS", default=[])
...
FLAGS = {
    ...
    "BASIC_AUTH_ENABLED": [
        {
            "condition": "boolean",
            "value": env.bool("DJANGO_FLAGS__BASIC_AUTH_ENABLED_VALUE", default=False),
        },
    ],
    "DEV_AUTH_ENABLED": [
        {
            "condition": "boolean",
            "value": env.bool("DJANGO_FLAGS__DEV_AUTH_ENABLED_VALUE", default=False),
        },
    ],
    "BRC_ONLY": [
        {
            "condition": "boolean",
            "value": env.bool("DJANGO_FLAGS__BRC_ONLY_VALUE", default=False),
        },
    ],
    ...
    "LINK_LOGIN_ENABLED": [
        {
            "condition": "boolean",
            "value": env.bool("DJANGO_FLAGS__LINK_LOGIN_ENABLED_VALUE", default=False),
        },
    ],
    "LRC_ONLY": [
        {
    ...
    "MULTIPLE_EMAIL_ADDRESSES_ALLOWED": [
        {
            "condition": "boolean",
            "value": env.bool(
                "DJANGO_FLAGS__MULTIPLE_EMAIL_ADDRESSES_ALLOWED_VALUE", default=False
            ),
        },
    ],
...
if not (FLAGS["BRC_ONLY"][0]["value"] ^ FLAGS["LRC_ONLY"][0]["value"]):
    raise ImproperlyConfigured("Exactly one of BRC_ONLY, LRC_ONLY should be enabled.")
if (
    sum(
        [
...
if not FLAGS["SSO_ENABLED"][0]["value"] and FLAGS["LINK_LOGIN_ENABLED"][0]["value"]:
    raise ImproperlyConfigured(
        "LINK_LOGIN_ENABLED should only be enabled when SSO_ENABLED is enabled."
    )
...
if env.bool("HPCS__PLUGIN_ANALYTICS_ENABLED", default=False):
    EXTRA_EXTRA_APPS += ["coldfront.plugins.analytics"]
...
if env.bool("HPCS__PLUGIN_DEPARTMENTS_ENABLED", default=False):
    EXTRA_EXTRA_APPS += ["coldfront.plugins.departments"]
...
if env.bool("HPCS__PLUGIN_FACULTY_STORAGE_ALLOCATIONS_ENABLED", default=False):
    EXTRA_EXTRA_APPS += ["coldfront.plugins.faculty_storage_allocations"]
...
if env.bool("HPCS__PLUGIN_HARDWARE_PROCUREMENTS_ENABLED", default=False):
    EXTRA_EXTRA_APPS += ["coldfront.plugins.hardware_procurements"]
...
if FILE_STORAGE["backend"] == "google_drive":
    ...
    # Path to a Google service account private key JSON file.
