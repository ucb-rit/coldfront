...
def get_compute_resource_names():
    ...
    names = [
        x.replace(" Compute", "")
        for x in Resource.objects.filter(name__endswith=" Compute")
...
def get_computing_allowance_project_prefixes():
    ...
    prefixes = ResourceAttribute.objects.filter(
        resource_attribute_type=resource_attribute_type, resource__in=resources
...
def get_primary_compute_resource():
    ...
    return Resource.objects.get(name__iexact=f"{settings.PRIMARY_CLUSTER_NAME} Compute")
