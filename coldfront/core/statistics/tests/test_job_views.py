...
class TestJobBase(TestBase):
    ...
    def setUp(self):
        ...
        self.pi = User.objects.create(
            email="pi@email.com", first_name="PI", last_name="PI", username="pi"
        )
...
class TestSlurmJobListView(TestJobBase):
    ...
    def test_user_list_view_content(self):
        ...
        self.assertContains(
            response, '<span class="badge badge-success">COMPLETED</span>'
        )
        self.assertNotContains(
            response, '<span class="badge badge-success">COMPLETING</span>'
        )
    ...
    def test_show_all_jobs_ineffective_for_non_admins(self):
        ...
        response = self.get_response(self.user2, url + "?show_all_jobs=on")
        self.assertNotContains(response, self.job1.jobslurmid)
        self.assertNotContains(response, self.job2.jobslurmid)
        ...
        # project2)
        response = self.get_response(self.pi, url + "?show_all_jobs=on")
        self.assertContains(response, self.job1.jobslurmid)
        self.assertNotContains(response, self.job2.jobslurmid)
    ...
    def test_pi_username_dropdown_scoped_to_managed_projects(self):
        ...
        user3 = User.objects.create(username="user3", email="user3@email.com")
        ProjectUser.objects.create(
            user=user3,
            project=self.project3,
        ...
        username_choices = [
            choice[0] for choice in form.fields["username"].widget.choices
        ]
    ...
    def test_saves_filters_in_session(self):
        ...
        url = (
            reverse("slurm-job-list")
            + "?show_all_jobs=on&username=user1&partition=test_partition"
        )
        self.get_response(self.admin, url)

        self.assertEqual(
            self.client.session.get("job_search_filters")["username"],
            self.user1.username,
        )
        self.assertEqual(
            self.client.session.get("job_search_filters")["partition"], "test_partition"
        )
    ...
    def test_status_colors(self):
        ...
        def helper_test_status_colors(status_list, status_type):
            url = (
                reverse("slurm-job-list")
                + "?show_all_jobs=on&jobslurmid="
                + self.job1.jobslurmid
            )
            for status in status_list:
                ...
                if status == "CANCEL":
                    self.assertContains(
                        response, '<span class="badge badge-danger">CANCELLED</span>'
                    )
...
class TestExportJobListView(TestJobBase):
    ...
    def _create_job_csv_line(self, job):
        job_line = (
            f"{job.jobslurmid},{job.userid.username},"
