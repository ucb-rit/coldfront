...
def job_query_filtering(job_list, data):
    ...
    if status:
        if status == 'COMPLETING':
            job_list = job_list.filter(
                jobstatus__in=['COMPLETED', 'COMPLETING'])
        else:
    ...
    if data.get('project_name'):
        job_list = job_list.filter(
            accountid__name=data.get('project_name'))

    if data.get('username'):
        job_list = job_list.filter(
            userid__username=data.get('username'))
    ...
    if data.get('submitdate_after'):
        after = display_time_zone_date_to_utc_datetime(
            data.get('submitdate_after'))
...
class JobSearchFilterSessionStorage(object):
    ...
    def set(self, filters):
        self._request.session[self._session_key] = (
        ...
        for date_key in self._date_keys:
            ...
            if serialized_date is not None:
                filters_copy[date_key] = datetime.strptime(
                    serialized_date, self._date_format).date()
        return filters_copy
