...
from coldfront.core.statistics.utils_.accounting_utils import (
    set_job_amount,
    validate_job_dates,
)
...
class Command(BaseCommand):
    ...
    def _add_reset_subparser(parsers):
        ...
        parser = parsers.add_parser(
            "reset",
            help=(
                "Set amounts for relevant jobs to zero, and update associated usages."
            ),
        )
        parser.add_argument(
            "qos_names",
    ...
    def _zero_out_free_qos_jobs(self, qos_names, project=None, dry_run=False):
        ...
        for job in Job.objects.filter(**kwargs).iterator():
            ...
            if not dry_run:
                try:
                    set_job_amount(jobslurmid, zero, update_usages=update_usages)
                except Exception as e:
                    self.logger.exception(e)
                    message = (
                        f"Failed to update amount for Job {jobslurmid}. Details:\n{e}"
                    )
                    self.stderr.write(self.style.ERROR(message))

        for project_name in total_by_project_name:
            usage_str = str(total_by_project_name[project_name]["usage"])
            total_by_project_name[project_name]["usage"] = usage_str
        result_json = json.dumps(total_by_project_name, indent=4, sort_keys=True)
        ...
        if not dry_run:
            compact_result_json = json.dumps(total_by_project_name, sort_keys=True)
            self.logger.info(
                f"Corrected amounts for {num_jobs} jobs under free QoSes "
