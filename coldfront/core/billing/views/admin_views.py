...
from coldfront.core.billing.forms import (
    BillingIDCreationForm,
    BillingIDSetProjectDefaultForm,
    BillingIDSetRechargeForm,
    BillingIDSetUserAccountForm,
    BillingIDUsagesSearchForm,
    BillingIDValidateManyForm,
)
...
class BillingIDCreateView(LoginRequiredMixin, UserPassesTestMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.exception(
                f"Failed to create BillingActivity for {billing_id}. Details:\n{e}"
            )
            messages.error(
...
class BillingIDSetView(LoginRequiredMixin, UserPassesTestMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self._billing_id_type not in self._allowed_billing_id_types_and_labels:
    ...
    def form_valid(self, form):
        ...
        if self._billing_activity_manager.billing_activity:
            prev_billing_id = self._billing_activity_manager.billing_activity.full_id()
        try:
    ...
    def get_context_data(self, **kwargs):
        ...
        context["billing_id_type_label"] = self._allowed_billing_id_types_and_labels[
            self._billing_id_type
        ]
        return context
...
class BillingIDUsagesSearchView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        next_url = f"{reverse('billing-id-usages')}?{urlencode(self.request.GET)}"
