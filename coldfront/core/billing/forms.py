...
class BillingIDValidationForm(BillingIDValidityMixin, forms.Form):
    ...
    def clean_billing_id(self):
        ...
        self.validate_billing_id(billing_id, ignore_invalid=not self.enforce_validity)
        return billing_id
...
class BillingIDCreationForm(BillingIDValidityMixin, forms.Form):
    ...
    def clean_billing_id(self):
        ...
        if get_billing_activity_from_full_id(billing_id):
            raise forms.ValidationError(f"Project ID {billing_id} already exists.")
        return billing_id
...
class BillingIDBaseSetForm(BillingIDValidityMixin, forms.Form):
    billing_activity = BillingActivityChoiceField(
        label="Project ID", queryset=BillingActivity.objects.all(), required=True
    )
    ignore_invalid = forms.BooleanField(
        initial=False, label="Set the Project ID even if it is invalid.", required=False
    )
...
class BillingIDSetProjectDefaultForm(BillingIDBaseSetForm):
    project = forms.CharField(widget=forms.TextInput(attrs={"readonly": "readonly"}))
...
class BillingIDUsagesSearchForm(forms.Form):
    ...
    user = forms.ModelChoiceField(
        help_text=(
            "Filter results to include usages of IDs associated with the selected user."
        ),
        label="User",
