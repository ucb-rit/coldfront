...
def find_and_replace_billing_activity(
    find_obj, replace_obj, project_obj, dry_run=False
):
...
def find_billing_activity(billing_activity_obj, project_obj):
    ...
    """
    assert (
        isinstance(billing_activity_obj, BillingActivity)
        or billing_activity_obj is None
    )
    assert isinstance(project_obj, Project)
    ...
    for project_user_obj in project_user_objs:
        try:
            project_user_manager = ProjectUserBillingActivityManager(project_user_obj)
...
def get_billing_activity_from_full_id(full_id):
    ...
    try:
        billing_project = BillingProject.objects.get(identifier=project_identifier)
...
def get_billing_id_usages(full_id=None, project_obj=None, user_obj=None):
    ...
    if user_profile_kwargs:
        user_profiles = UserProfile.objects.filter(**user_profile_kwargs).order_by("id")
    else:
