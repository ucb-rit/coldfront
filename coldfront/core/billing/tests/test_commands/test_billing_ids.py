...
class TestBillingIds(TestBillingBase):
    ...
    def setUp(self):
        ...
        self.original_billing_activity = get_billing_activity_from_full_id("000000-000")
    ...
    def test_set_billing_id_invalid(self):
        ...
        for call in calls:
            ...
            self.assertEqual(manager.billing_activity, self.original_billing_activity)
            with self.assertRaises(CommandError) as cm:
                command(*args)
            self.assertIn("is invalid", str(cm.exception))
            self.assertEqual(manager.billing_activity, self.original_billing_activity)
    ...
    def test_set_project_default_success(self):
        ...
        output, error = self.command.set_project_default(self.project_name, billing_id)
        self.assertIn("Updated", output)
        self.assertFalse(error)
        ...
        other_billing_activity = get_billing_activity_from_full_id(other_billing_id)
    ...
    def test_set_user_account_success(self):
        ...
        output, error = self.command.set_user_account(self.user.username, billing_id)
        self.assertIn("Updated", output)
        self.assertFalse(error)
