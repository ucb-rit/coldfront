...
from coldfront.core.allocation.utils_.secure_dir_utils.new_directory import (
    is_project_eligible_for_secure_dirs,
)
from coldfront.core.billing.utils.queries import (
    is_project_billing_id_required_and_missing,
)
...
from coldfront.core.project.utils import (
    annotate_queryset_with_cluster_name,
    is_primary_cluster_project,
)
from coldfront.core.project.utils_.addition_utils import (
    can_project_purchase_service_units,
)
...
class ProjectDetailView(LoginRequiredMixin, UserPassesTestMixin, DetailView):
    ...
    def test_func(self):
        ...
        if project_obj.projectuser_set.filter(
            user=self.request.user, status__name__in=["Active", "Pending - Remove"]
        ).exists():
            return True
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.object.projectuser_set.filter(user=self.request.user).exists():
            ...
            if (
                project_user.status.name == "Active"
                and project_user.role.name == "User"
            ):
                context["can_leave_project"] = True

            if (
                project_user.role.name == "Manager"
                and len(self.object.projectuser_set.filter(role__name="Manager")) > 1
            ):
                context["can_leave_project"] = True
        ...
        try:
            ...
            statuses = allocation_obj.allocationuserattribute_set.select_related(
                "allocation_user__user"
            ).filter(allocation_attribute_type__name="Cluster Account Status")
        ...
        project_users = (
            self.object.projectuser_set.select_related("user")
        ...
        if _is_primary_cluster_project:
            ...
            renew_allowance_visible = computing_allowance.is_renewal_supported()
        ...
        context["purchase_sus_visible"] = (
            flag_enabled("SERVICE_UNITS_PURCHASABLE")
            and _is_primary_cluster_project
            and can_project_purchase_service_units(self.object)
            and context.get("is_allowed_to_update_project", False)
        )
        ...
        if flag_enabled("FACULTY_STORAGE_ALLOCATIONS_ENABLED"):
            ...
            context["request_faculty_storage_allocations_visible"] = (
                is_primary_cluster_project(self.object)
                and is_project_eligible_for_faculty_storage_allocations(self.object)
                and has_eligible_pi_for_fsa_request(self.object)
                and (
                    self.request.user.is_superuser
                    or context.get("is_allowed_to_update_project", False)
                )
            )
        else:
        ...
        context["request_secure_directory_visible"] = (
            flag_enabled("SECURE_DIRS_REQUESTABLE")
            and is_project_eligible_for_secure_dirs(self.object)
            and (
                self.request.user.is_superuser
                or context.get("is_allowed_to_update_project", False)
            )
        ...
        allocation_requests = SavioProjectAllocationRequest.objects.filter(
            project=self.object, status__name="Approved - Complete"
        ).order_by("request_time")

        if allocation_requests.exists():
            survey_answers_list = list(
                map(
                    lambda x: SavioProjectSurveyForm(
                        initial=x.survey_answers, disable_fields=True
                    ),
                    allocation_requests,
                )
            )

            context["survey_answers"] = list(
                zip(allocation_requests, survey_answers_list)
            )

        context["user_agreement_signed"] = access_agreement_signed(self.request.user)
...
class ProjectListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if project_search_form.is_valid():
            ...
            if data.get("last_name"):
                ...
                project_ids = pi_project_users.values_list("project_id", flat=True)
            ...
            if data.get("cluster_name"):
                projects = projects.filter(
                    cluster_name__icontains=data.get("cluster_name")
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["renew_allowance_visible"] = ProjectUser.objects.filter(
            user=self.request.user, role__name__in=role_names, status=status
        )
...
class ProjectArchivedListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    model = Project
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if project_obj.status.name != "Active":
            messages.error(
                request,
                (
                    "You cannot add users to a project with "
...
class ProjectAddUsersSearchResultsView(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if matches:
            ...
            # cache User objects
            match_users = User.objects.filter(
                username__in=[form._username for form in formset]
            )
            for form in formset:
...
class ProjectAddUsersView(LoginRequiredMixin, UserPassesTestMixin, View):
        ...
        try:
            with transaction.atomic():
                ...
                project_user_obj = self._update_or_create_project_user(
                    project_obj, user_obj, role_choice, project_user_status_choice
                )

                self._update_or_create_allocation_users(
                    allocation_form_data, user_obj, allocation_user_status_choice
                )
    ...
    def _update_or_create_allocation_users(
        self, allocation_form_data, user_obj, status_choice
    ):
    ...
    @staticmethod
    def _update_or_create_project_user(
        project_obj, user_obj, role_choice, status_choice
    ):
...
class ProjectUserDetail(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if project_obj.projectuser_set.filter(id=project_user_pk).exists():
            project_user_obj = project_obj.projectuser_set.get(pk=project_user_pk)
            managers = project_obj.projectuser_set.filter(
                role__name="Manager", status__name="Active"
            )
            project_pis = project_obj.projectuser_set.filter(
            ...
            if project_user_obj.role.name == "Principal Investigator":
                enable_notifications = (
                    project_user_update_form.data.get("enable_notifications", "off")
                    == "on"
                )
                ...
                if not managers.exists() and not enable_notifications:
                    messages.error(
                        request,
                        "PIs can disable notifications when at least one manager exists.",
                    )
                else:
                ...
                if old_role.name == "Manager" and new_role.name == "User":
                    if not managers.filter(~Q(pk=project_user_pk)).exists():
                        ...
                        if not project_pis.exists():
                            ...
                            messages.error(
                                request,
                                "The project must have at least one PI or manager with notifications enabled.",
                            )
