...
class TestSavioProjectRequestDetailView(TestBase):
    ...
    def setUp(self):
        ...
        self.project, self.new_project_request = create_project_and_request(
            f"{prefix}_project",
            "New",
            computing_allowance,
            allocation_period,
        ...
        self.new_project_request.billing_activity = BillingActivity.objects.create(
    ...
    def test_post_approves_and_processes_request_for_started_period(self):
        ...
        self.assertEqual(new_project_request.status.name, "Approved - Complete")
        self.assertTrue(
            pre_time
        ...
        self.assertGreater(
            Decimal(self.service_units_attribute.value), self.existing_service_units
        )
    ...
    def test_post_approves_not_processes_request_for_non_started_period(self):
        ...
        # started.
        next_allowance_year_allocation_period = get_next_allowance_year_period()
        self.assertIsNotNone(next_allowance_year_allocation_period)
        new_project_request.allocation_period = next_allowance_year_allocation_period
        new_project_request.save()
        ...
        # set.
        new_project_request.refresh_from_db()
        self.assertEqual(new_project_request.status.name, "Approved - Scheduled")
        self.assertTrue(pre_time <= new_project_request.approval_time <= post_time)
        ...
        email = mail.outbox[0]
        self.assertIn(
            (f"New Project Request ({new_project_request.project.name}) Approved"),
            email.subject,
        )
    ...
    def test_post_approves_and_processes_request_for_null_period_condo(self):
        ...
        computing_allowance = Resource.objects.get(name=computing_allowance_name)
        project_name_prefix = self.interface.code_from_name(computing_allowance.name)

        self.project.name = f"{project_name_prefix}{self.project.name[3:]}"
        self.project.save()
        self.new_project_request.allocation_type = self.interface.name_short_from_name(
