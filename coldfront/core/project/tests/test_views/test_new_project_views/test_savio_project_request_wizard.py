...
from coldfront.core.utils.tests.test_base import TransactionTestBase, enable_deployment
...
FLAGS_COPY["USER_DEPARTMENTS_ENABLED"] = [{"condition": "boolean", "value": True}]
...
class TestSavioProjectRequestWizard(TransactionTestBase):
    ...
    def _send_post_data(
        ...
        if pi_departments is not None:
            pi_department_form_data = {
                "4-departments": [department.pk for department in pi_departments],
                current_step_key: "4",
            }
    ...
    def test_post_creates_pi_user_if_nonexistent(self):
        ...
        self.assertEqual(new_pi_user.first_name, new_pi_details_data["first_name"])
        self.assertEqual(
            new_pi_user.userprofile.middle_name, new_pi_details_data["middle_name"]
        )
        self.assertEqual(new_pi_user.last_name, new_pi_details_data["last_name"])
    ...
    def test_post_sets_user_departments(self):
        ...
        self.assertTrue(
            self._user_department_model.objects.filter(
                user=self.user, department=self._department_1, is_authoritative=False
            ).exists()
        )
        self.assertTrue(
            self._user_department_model.objects.filter(
                user=self.user,
                department=self._department_model.objects.get(name="Department F"),
        ...
        self.assertTrue(
            self._user_department_model.objects.filter(
                user=self.user,
                department=self._department_model.objects.get(name="Department L"),
                is_authoritative=True,
            ).exists()
        )
