...
class TestProjectJoinRequestListView(TestBase):
    ...
    def setUp(self):
        ...
        self.pending_add = ProjectUserStatusChoice.objects.get(name="Pending - Add")
    ...
    def assert_request_shown(self, response, request):
        ...
        request_date = request.created.astimezone(
            pytz.timezone(settings.DISPLAY_TIME_ZONE)
        ).strftime("%b. %d, %Y")
    ...
    def test_only_old_requests(self):
        ...
        request1 = ProjectUserJoinRequest.objects.create(
            project_user=self.project1_user1, reason="test reason 1"
        )

        request2 = ProjectUserJoinRequest.objects.create(
            project_user=self.project1_user2, reason="test reason 2"
        )
    ...
    def test_single_request_given_old_requests(self):
        ...
        """
        request1 = ProjectUserJoinRequest.objects.create(
            project_user=self.project1_user1, reason="old request 1"
        )
        request2 = ProjectUserJoinRequest.objects.create(
            project_user=self.project1_user1, reason="old request 2"
        )
        request3 = ProjectUserJoinRequest.objects.create(
            project_user=self.project1_user1, reason="new request 1"
        )
        self.project1_user1.status = self.pending_add
        self.project1_user1.save()
    ...
    def test_multiple_requests(self):
        ...
        request3 = ProjectUserJoinRequest.objects.create(
            project_user=self.project2_user2, reason="new request 2"
        )
        self.project1_user1.status = self.pending_add
        self.project1_user1.save()
