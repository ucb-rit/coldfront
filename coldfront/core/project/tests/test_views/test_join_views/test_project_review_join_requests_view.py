...
class TestViewMixin(object):
    ...
    def setUp(self):
        ...
        self.project0 = self.create_active_project_with_pi("fc_project0", self.pi)
        self.allocation0 = create_project_allocation(
            self.project0, settings.ALLOCATION_MIN
        ).allocation
    ...
    def review_requests_url(project):
        ...
        the given Project."""
        return reverse("project-review-join-requests", kwargs={"pk": project.pk})
...
class TestBRCProjectReviewJoinRequestsView(TestViewMixin, TestBase):
    ...
    def test_not_sets_user_profile_fields(self):
        ...
        project_user = ProjectUser.objects.get(user=self.user, project=self.project0)
...
class TestLRCProjectReviewJoinRequestsView(TestViewMixin, LRCTestBase):
    ...
    def test_host_user_submitted(self):
        ...
        response = self.create_join_request(self.user, self.project0, host_user=self.pi)
        self.assertEqual(response.status_code, HTTPStatus.FOUND)
        join_request = ProjectUserJoinRequest.objects.get(
            project_user__user=self.user, project_user__project=self.project0
        )
