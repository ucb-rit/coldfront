...
FSA_PLUGIN_INSTALLED = (
    "coldfront.plugins.faculty_storage_allocations" in settings.INSTALLED_APPS
)
...
class TestProjectDetailView(TestBase):
    ...
    def test_renew_allowance_button_conditionally_enabled(self):
        ...
        pi_2 = User.objects.create(
            email="pi_2@email.com", first_name="PI", last_name="2", username="pi_2"
        )
        ...
        with override_settings(FLAGS=flags_copy):
            ...
            self.assertFalse(
                AllocationRenewalRequest.objects.filter(
                    allocation_period=allocation_period, pi__in=[self.user, pi_2]
                ).exists()
            )
            ...
            denied_request_status = AllocationRenewalRequestStatusChoice.objects.get(
                name="Denied"
            )
            ...
            # period.
            approved_request_status = AllocationRenewalRequestStatusChoice.objects.get(
                name="Approved"
            )
    ...
    def test_renew_allowance_button_conditionally_visible(self):
        ...
        project_name_prefixes_by_allowance_name = {
            allowance.name: computing_allowance_interface.code_from_name(allowance.name)
            for allowance in all_allowances
        }

        expected_num_successes = len(eligible_role_names) * len(
            eligible_allowance_names
        )
        ...
        for role in all_roles:
            ...
            for allowance in all_allowances:
                ...
                response = self.client.get(url)
                if (
                    role.name in eligible_role_names
                    and allowance.name in eligible_allowance_names
                ):
                    self.assertContains(response, button_text)
    ...
    @unittest.skipIf(
        not FSA_PLUGIN_INSTALLED, "Faculty Storage Allocations plugin not installed"
    )
    @override_settings(
        FLAGS={
    ...
    def test_request_storage_link_invisible_when_pi_not_on_whitelist(self):
        ...
        EmailAddress.objects.create(
            user=self.user, email="pi@example.com", verified=True, primary=True
        )
        ...
        with (
            ...
            patch(
                "coldfront.plugins.faculty_storage_allocations.conf.settings.ELIGIBLE_PI_EMAIL_WHITELIST",
                ["other@berkeley.edu"],
            ),
        ):
    ...
    def test_request_storage_link_visible_when_different_pi_is_eligible(self):
        ...
        pi_2 = User.objects.create(
            email="pi_2@example.com", first_name="PI", last_name="Two", username="pi_2"
        )
        ProjectUser.objects.create(
            project=project,
            role=ProjectUserRoleChoice.objects.get(name="Principal Investigator"),
