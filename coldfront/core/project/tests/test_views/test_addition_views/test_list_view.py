...
class TestViewMixin:
    ...
    def setUp(self):
        ...
        # Create two Projects.
        self.project_a = self.create_active_project_with_pi("ac_project_a", self.user_a)
        self.project_b = self.create_active_project_with_pi("ac_project_b", self.user_b)
    ...
    def test_permissions_get(self):
        ...
        ProjectUser.objects.filter(user__in=users).update(status=pending_remove_status)
        ...
        ProjectUser.objects.filter(user__in=users).update(
            role=user_role, status=active_status
        )
        self.assert_has_access(url, self.user_a, has_access=False)
        self.assert_has_access(url, self.user_b, has_access=False)
        ...
        self.assertTrue(self.user_a.has_perm(f"allocation.{permission.codename}"))
        self.assert_has_access(url, self.user_a)
        ...
        self.assertFalse(self.user_a.has_perm(f"allocation.{permission.codename}"))
        expected_messages = [
            "You must be an active PI or manager of a Project.",
        ]
        self.assert_has_access(
            url, self.user_a, has_access=False, expected_messages=expected_messages
        )
...
class TestAllocationAdditionRequestCompletedListView(TestViewMixin, TestBase):
    ...
    def setUp(self):
        ...
        self.request_a.status = AllocationAdditionRequestStatusChoice.objects.get(
            name="Complete"
        )
        self.request_a.save()
        self.request_b.status = AllocationAdditionRequestStatusChoice.objects.get(
            name="Denied"
        )
        self.request_b.save()
    ...
    def test_type(self):
        ...
        response = self.client.get(self.url)
        self.assertContains(response, "Completed Service Units Purchase Requests")
...
class TestAllocationAdditionRequestPendingListView(TestViewMixin, TestBase):
    ...
    def test_type(self):
        """Test that the correct type is displayed on the page."""
        response = self.client.get(self.url)
        self.assertContains(response, "Pending Service Units Purchase Requests")
