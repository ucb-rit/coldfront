...
class TestAllocationAdditionRequestDetailView(TestBase):
    ...
    def request_detail_url(pk):
        ...
        return reverse("service-units-purchase-request-detail", kwargs={"pk": pk})
    ...
    def test_permissions_get(self):
        ...
        project_user = ProjectUser.objects.get(project=self.project, user=self.user)
        ...
        self.user = User.objects.get(pk=self.user.pk)
        self.assertFalse(self.user.has_perm(f"allocation.{permission.codename}"))
        expected_messages = [
    ...
    def test_permissions_post(self):
        ...
        redirect_url = self.request_detail_url(self.allocation_addition_request.pk)
    ...
    def test_post_blocked_until_checklist_complete(self, mock_method):
        ...
        memorandum_signed = self.allocation_addition_request.state["memorandum_signed"]
