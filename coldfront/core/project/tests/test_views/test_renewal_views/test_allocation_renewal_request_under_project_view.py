...
class TestAllocationRenewalRequestUnderProjectViewMixin:
    ...
    def _send_post_requests(
        self, allocation_period, project, pi_project_user, billing_id=None
    ):
        ...
        for i, data in enumerate(form_data):
            ...
            if i == len(form_data) - 1:
                self.assertRedirects(response, self._project_detail_url(project.pk))
            else:
...
class TestBRCAllocationRenewalRequestUnderProjectView(
    TestAllocationRenewalRequestUnderProjectViewMixin, TestBase
):
...
class TestLRCAllocationRenewalRequestUnderProjectView(
    TestAllocationRenewalRequestUnderProjectViewMixin, LRCTestBase
):
    """A class for testing AllocationRenewalRequestUnderProjectView on
    the LRC deployment."""
    ...
    def test_post_updates_matching_billing_activities(self):
        ...
        allocation_period = get_current_allowance_year_period()
        self._send_post_requests(
            allocation_period, project, pi_project_user, billing_id=new_billing_id
        )
        ...
        assert other_project_user_manager.billing_activity == other_billing_activity
