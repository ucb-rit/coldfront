...
class TestPendingJoinRequestReminderCommand(TestBase):
    ...
    def setUp(self):
        ...
        self.request1 = ProjectUserJoinRequest.objects.create(project_user=project_user)

        self.request2 = ProjectUserJoinRequest.objects.create(
            project_user=project_user2
        )
            ...
            for addr in email.to:
                if addr in manager_emails:
                    ...
                    for request in [self.request1, self.request2]:
                        request_list = (
                            f"{request.project_user.user.first_name} "
                    ...
                    self.fail(
                        "Email not sent to either the user or managers/PIs of project"
                    )
    ...
    def test_command_two_proj_multiple_requests(self):
        ...
        # Create Projects.
        project2 = Project.objects.create(name="project2", status=active_project_status)
        ...
        request3 = ProjectUserJoinRequest.objects.create(project_user=project_user2)
            ...
            for addr in email.to:
                if addr in manager_emails1:
                    try:
                        ...
                        request_list = (
                            f"{request3.project_user.user.first_name} "
    ...
    def test_command_previously_denied_requests(self):
        ...
        request4 = ProjectUserJoinRequest.objects.create(project_user=proj_user)
        ...
        for email in mail.outbox:
            for addr in email.to:
                if addr in manager_emails:
                    ...
                    request_list = (
                        f"{request4.project_user.user.first_name} "
