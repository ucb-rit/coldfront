...
class TestProjectsRenew(TestProjectsBase):
    ...
    def _assert_project_inactive(self, project_name):
        ...
        self.assertEqual(
            still_inactive_proj.status, ProjectStatusChoice.objects.get(name="Inactive")
        )
    ...
    def _set_up_allocation_periods(self):
        ...
        AllocationPeriod.objects.filter(
            self._ica_computing_allowance.get_period_filters()
        ).delete()
        ...
        for period in ica_periods:
            self._set_service_units_to_be_allocated_for_period(
                self._ica_computing_allowance, period, self._ica_num_service_units
            )

    def _set_service_units_to_be_allocated_for_period(
        self, computing_allowance, allocation_period, num_service_units
    ):
    ...
    def test_dry_run(self):
        ...
        self.assertEqual(
            Decimal(self.service_units_attribute.value), settings.ALLOCATION_MIN
        )
    ...
    def test_success(self):
        ...
        self.assertEqual(
            now_active_proj.status, ProjectStatusChoice.objects.get(name="Active")
        )
        self.service_units_attribute.refresh_from_db()
        self.assertEqual(
            Decimal(self.service_units_attribute.value), self._ica_num_service_units
        )

        request = AllocationRenewalRequest.objects.get(
            requester=self.user, pi=self.user, pre_project=project
        )
        self.assertEqual(
    ...
    def test_validate_requester(self):
        ...
        invalid_usernames = [invalid_requester.username, removed_requester.username]
    ...
    def test_validate_pi(self):
        ...
        for invalid_pi in invalid_pis:
            ...
            self.assertIn(f"{invalid_pi} is not an active PI", str(cm.exception))
            self._assert_project_inactive(project.name)
            self.service_units_attribute.refresh_from_db()
            self.assertEqual(
                Decimal(self.service_units_attribute.value), settings.ALLOCATION_MIN
    ...
    def test_validate_allocation_period(self):
        ...
        self.assertIn("AllocationPeriod already ended", str(cm.exception))
        self._assert_project_inactive(project.name)
        ...
        self.assertIn("AllocationPeriod does not start until", str(cm.exception))
        self._assert_project_inactive(project.name)
...
class ProjectsCommand(object):
    ...
    def renew(self, name, allocation_period, pi_username, requester_username, **flags):
        """Call the 'renew' subcommand with the given positional arguments."""
        args = ["renew", name, allocation_period, pi_username, requester_username]
