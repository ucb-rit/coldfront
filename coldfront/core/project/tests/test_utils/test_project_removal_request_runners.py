...
class TestRemovalRequestRunnerBase(TestBase):
    ...
    def setUp(self):
        ...
        for user in [self.user1, self.user2, self.user3]:
            ...
            objects = create_user_project_allocation(user, self.project1, sus)
...
class TestProjectRemovalRequestRunner(TestRemovalRequestRunnerBase):
    ...
    def test_normal_user_self_removal_request(self):
        ...
        self.assertEqual(
            self.project1.projectuser_set.get(user=self.user1).status.name, "Active"
        )
        ...
        self.assertEqual(
            self.project1.projectuser_set.get(user=self.user1).status.name,
            "Pending - Remove",
        )
        ...
        manager_pi_queryset = self.project1.projectuser_set.filter(
            role__name__in=["Manager", "Principal Investigator"], status__name="Active"
        ).exclude(user=removal_request.requester)

        email_to_set = {proj_user.user.email for proj_user in manager_pi_queryset}

        email_body = (
            f"{self.user1.first_name} {self.user1.last_name} of "
        ...
        email_body_admin = (
            f"There is a new request to remove "
            f"{self.user1.first_name} {self.user1.last_name} "
            f"from {self.project1.name}."
        )
        ...
        # check messages
        test_message = (
            f"Error requesting removal of user {self.user1.username}. "
    ...
    def test_normal_user_pi_removal_request(self):
        ...
        email_body = (
            f"{self.pi1.first_name} {self.pi1.last_name} of "
            f"Project {self.project1.name} has requested to remove "
            f"{self.user1.first_name} {self.user1.last_name} "
            f"from {self.project1.name}"
        )
    ...
    def test_pi_self_removal_request(self):
        ...
        # Test project user status before removal
        self.assertEqual(
            self.project1.projectuser_set.get(user=self.pi1).status.name, "Active"
        )

        request_runner = ProjectRemovalRequestRunner(self.pi1, self.pi1, self.project1)
        removal_request = request_runner.run()
        success_messages, error_messages = request_runner.get_messages()
        ...
        # check messages
        test_message = (
            f"Error requesting removal of user {self.pi1.username}. "
    ...
    def test_single_manager_self_removal_request(self):
        ...
        # Test project user status before removal
        self.assertEqual(
            self.project1.projectuser_set.get(user=self.manager).status.name, "Active"
        )
        ...
        # check messages
        test_message = (
            f"Error requesting removal of user {self.manager.username}. "
    ...
    def test_manager_removal_request_given_multiple_managers(self):
        ...
        request_runner = ProjectRemovalRequestRunner(self.pi1, manager2, self.project1)
        removal_request = request_runner.run()
        success_messages, error_messages = request_runner.get_messages()
        ...
        # Test project user status
        self.assertEqual(
            self.project1.projectuser_set.get(user=manager2).status.name,
            "Pending - Remove",
        )
        ...
        # check messages
        test_message = (
            f"Error requesting removal of user {manager2.username}. "
            f"An active project removal request for user "
...
class TestProjectRemovalRequestProcessingRunner(TestRemovalRequestRunnerBase):
    ...
    def setUp(self):
        ...
        self.complete_status = ProjectUserRemovalRequestStatusChoice.objects.get(
            name="Complete"
        )
        ...
        ProjectUser.objects.filter(user=self.pi2).update(enable_notifications=False)
    ...
    def _assert_emails_sent(self):
        ...
        expected_to = {user.email for user in [self.user1, self.pi1, self.manager]}
    ...
    def test_asserts_input_request(self):
        ...
        invalid_statuses = ProjectUserRemovalRequestStatusChoice.objects.exclude(
    ...
    def test_email_failure_no_rollback(self):
        ...
        with patch.object(
            ProjectRemovalRequestProcessingRunner, "_send_emails", raise_exception
        ):
            runner = ProjectRemovalRequestProcessingRunner(self.request_obj)
    ...
    def test_exception_outside_transaction_no_rollback(self):
        ...
        with patch.object(
            ProjectRemovalRequestProcessingRunner, "_send_emails_safe", raise_exception
        ):
            runner = ProjectRemovalRequestProcessingRunner(self.request_obj)
            with self.assertLogs(self._module, "INFO") as log_cm:
