...
from coldfront.core.project.tests.test_utils.test_addition_utils.test_runner_mixin import (
    TestRunnerMixin,
)
...
class TestAllocationAdditionDenialRunner(TestRunnerMixin, TestBase):
    ...
    def setUp(self):
        ...
        set_project_user_usage_value(self.user, self.project, user_values[1])
        ...
        self.request_obj = AllocationAdditionRequest.objects.create(
            requester=self.user,
    ...
    def test_runner_not_creates_transactions(self):
        ...
        old_project_user_transaction_count = ProjectUserTransaction.objects.filter(
            project_user__project=self.project
        ).count()
        ...
        new_project_user_transaction_count = ProjectUserTransaction.objects.filter(
            project_user__project=self.project
        ).count()

        self.assertEqual(old_project_transaction_count, new_project_transaction_count)
        self.assertEqual(
            old_project_user_transaction_count, new_project_user_transaction_count
        )
