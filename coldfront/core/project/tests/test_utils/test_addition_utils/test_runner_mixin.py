...
class TestRunnerMixin(object):
    ...
    def assert_allocation_service_units_values(
        self, allocation, expected_value, expected_usage
    ):
        ...
        self.assertEqual(expected_usage, attribute.allocationattributeusage.value)

    def assert_allocation_user_service_units_values(
        self, allocation_user, expected_value, expected_usage
    ):
        ...
        attributes = allocation_user.allocationuserattribute_set.filter(**kwargs)
        self.assertEqual(attributes.count(), 1)
        attribute = attributes.first()
        self.assertEqual(str(expected_value), attribute.value)
        self.assertEqual(expected_usage, attribute.allocationuserattributeusage.value)
