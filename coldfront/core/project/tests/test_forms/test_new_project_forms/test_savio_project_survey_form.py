...
class TestSavioProjectSurveyForm(TestBase):
    ...
    def test_text_updated_for_deployment(self):
        ...
        with override_settings(PRIMARY_CLUSTER_NAME="Savio", PROGRAM_NAME_SHORT="BRC"):
            form = SavioProjectSurveyForm(computing_allowance=computing_allowance)
            for field_name, field_attribute in altered_fields:
                attribute_value = getattr(form.fields[field_name], field_attribute)
                self.assertTrue("Savio" in attribute_value or "BRC" in attribute_value)
        ...
        with override_settings(
            ...
            for field_name, field_attribute in altered_fields:
                attribute_value = getattr(form.fields[field_name], field_attribute)
                self.assertTrue(
                    "Lawrencium" in attribute_value or "LRC" in attribute_value
                )
        ...
        for allowance in ComputingAllowanceInterface().allowances():
            ...
            for field_name, field_attribute in altered_fields:
                ...
                if not wrapper.is_instructional():
                    self.assertTrue(
                        "research" in attribute_value
                        or "project" in attribute_value
                        or "user" in attribute_value
                    )
                else:
                    self.assertTrue(
                        "course" in attribute_value or "student" in attribute_value
                    )
