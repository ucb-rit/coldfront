...
class Command(BaseCommand):
    ...
    def _add_create_subparser(parsers):
        ...
        parser.add_argument("name", help="The name of the project to create.", type=str)
        parser.add_argument(
            "cluster_name",
            help=(
    ...
    def _add_renew_subparser(parsers):
        ...
        parser = parsers.add_parser(
            "renew", help="Renew a PI's allowance under a project."
        )
        parser.add_argument(
    ...
    @staticmethod
    def _create_project_with_compute_allocation_and_pis(
    ...
    def _validate_create_options(options):
        ...
        if Project.objects.filter(name=project_name).exists():
            raise CommandError(f'A Project with name "{project_name}" already exists.')
        ...
        lowercase_primary_cluster_name = (
            get_primary_compute_resource_name().replace(" Compute", "").lower()
        )
        is_cluster_primary = lowercase_cluster_name == lowercase_primary_cluster_name
        if is_primary_cluster_project(Project(name=project_name)) or is_cluster_primary:
            raise CommandError(
        ...
        try:
            ...
            raise CommandError(f"Cluster {uppercase_cluster_name} does not exist.")
    ...
    def _validate_renew_options(options):
        ...
        try:
            ...
            raise CommandError(f'A Project with name "{project_name}" does not exist.')
        ...
        try:
            ...
            raise CommandError(f'User with username "{pi_username}" does not exist.')
        ...
        num_service_units = calculate_service_units_to_allocate(
            computing_allowance, request_time, allocation_period=allocation_period
        )
