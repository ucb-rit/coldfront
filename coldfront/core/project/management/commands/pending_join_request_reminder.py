...
from coldfront.core.project.utils import (
    project_join_list_url,
    review_project_join_requests_url,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for pk in projects_with_pending_join_requests:
            ...
            if settings.EMAIL_ENABLED:
                ...
                try:
                    ...
                    plain_template = (
                        "email/project_join_request/pending_project_join_requests.txt"
                    )
                    html_template = (
                        "email/project_join_request/pending_project_join_requests.html"
                    )
                    send_email_template(
                        subject,
                        plain_template,
        ...
        for pk in users_with_pending_join_requests:
            ...
            proj_join_requests_queryset = (
                ProjectUserJoinRequest.objects.filter(
                    project_user__user=user, project_user__status__name="Pending - Add"
                )
                .order_by("project_user", "-created")
                .distinct("project_user")
            )
