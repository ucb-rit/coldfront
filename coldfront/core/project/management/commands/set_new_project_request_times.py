...
from coldfront.core.project.models import (
    HistoricalSavioProjectAllocationRequest,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        approved_processing_status = ProjectAllocationRequestStatusChoice.objects.get(
            name="Approved - Processing"
        )
        approved_complete_status = ProjectAllocationRequestStatusChoice.objects.get(
        ...
        for request in SavioProjectAllocationRequest.objects.all():
            ...
            historical_objects_processed = (
                HistoricalSavioProjectAllocationRequest.objects.filter(
                    id=request.id, status=approved_processing_status
            ...
            historical_objects_complete = (
                HistoricalSavioProjectAllocationRequest.objects.filter(
                    id=request.id, status=approved_complete_status
                ).order_by("history_date")
            )
