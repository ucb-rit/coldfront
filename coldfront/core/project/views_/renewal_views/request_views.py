...
from coldfront.core.billing.utils.queries import (
    find_and_replace_billing_activity,
...
class AllocationRenewalLandingView(
        ...
        for allowance in sorted(interface.allowances(), key=lambda a: a.pk):
            ...
            if wrapper.is_renewable():
                if wrapper.is_renewal_supported():
                    ...
                    renewal_not_supported_allowance_names.append(allowance_name)

        context["allowances"] = allowances
        context["yearly_allowance_names"] = ", ".join(yearly_allowance_names)
        context["renewal_supported_allowance_names"] = renewal_supported_allowance_names
...
class AllocationRenewalMixin:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if flag_enabled("BRC_ONLY"):
            ...
            self.computing_allowance = Resource.objects.get(name=LRCAllowances.PCA)
        else:
    ...
    def get_form_initial(self, step):
        ...
        if step == self.step_numbers_by_form_name["billing_id"]:
            if self._billing_id_required():
                ...
                billing_id = self._get_requested_project_current_billing_id_or_none(
    ...
    def send_emails(request_obj):
        ...
        if (
            request_obj.pi not in request_obj.post_project.pis()
            and not request_obj.new_project_request
        ):
            try:
                send_new_allocation_renewal_request_pooling_notification_email(
                    request_obj
    ...
    def _add_context_data_for_billing_ids(self, context):
        ...
        if billing_id_change_requested:
            prev_billing_id = self._get_requested_project_current_billing_id_or_none(
    ...
    @staticmethod
    def _create_allocation_renewal_request(
        requester,
        pi,
        computing_allowance,
        allocation_period,
        ...
        request_kwargs["status"] = AllocationRenewalRequestStatusChoice.objects.get(
            name="Under Review"
        )
    ...
    def _get_requested_project_current_billing_id_or_none(project):
        ...
        if project.pk is not None:
            ...
            billing_id = (
                billing_activity.full_id() if billing_activity is not None else None
            )
        else:
    ...
    def _is_pi_of_project(self, user, project):
        ...
        return self._get_or_set_cached_value(
            cache_key, lambda: user in project.pis(active_only=False)
        )
    ...
    def _update_project_billing_activities(self, project, billing_id):
        ...
        try:
            ...
            logger.exception(e)
            error_message = (
                "Failed to update LBL Project ID. Please contact an administrator."
            )
            ...
            try:
                prev_billing_activity_str = (
                    (f"{prev_billing_activity.full_id()} ({prev_billing_activity.pk})")
                    if prev_billing_activity
                    else "None"
                )
                new_billing_activity_str = (
                    f"{new_billing_activity.full_id()} ({new_billing_activity.pk})"
                )
...
class AllocationRenewalRequestView(
    LoginRequiredMixin, UserPassesTestMixin, AllocationRenewalMixin, SessionWizardView
):
    FORMS = [
    ...
    def test_func(self):
        ...
        if has_access:
            return True
        message = (
            "You must be an active Manager or Principal Investigator of a Project."
        )
        messages.error(self.request, message)
    ...
    def get_form_kwargs(self, step=None):
        ...
        if step == self.step_numbers_by_form_name["allocation_period"]:
            ...
            tmp = {}
            self._set_data_from_previous_steps(step, tmp)
            kwargs["currently_pooled"] = (
                "current_project" in tmp and tmp["current_project"].is_pooled()
            )
    ...
    def get_template_names(self):
        ...
        resolved_template_name = os.path.join(self._TEMPLATES_DIR, template_file_name)
    ...
    def done(self, form_list, **kwargs):
        ...
        if billing_id is not None:
            self._update_project_billing_activities(requested_project, billing_id)
    ...
    def show_new_project_forms_condition(self):
        ...
        form_class = ProjectRenewalPoolingPreferenceForm
        return cleaned_data.get("preference", None) == form_class.POOLED_TO_UNPOOLED_NEW
    ...
    def __handle_create_new_project(self, form_data):
        ...
        try:
            ...
            logger.error(f"Project {data['name']} unexpectedly already exists.")
            raise e
    ...
    def __handle_create_new_project_request(self, pi, project, survey_data):
        ...
        request_kwargs["status"] = ProjectAllocationRequestStatusChoice.objects.get(
            name="Under Review"
        )
    ...
    def _set_data_from_previous_steps(self, step, dictionary):
        ...
        computing_allowance_wrapper = ComputingAllowance(self.computing_allowance)
        ...
        if step > allocation_period_form_step:
            data = self.get_cleaned_data_for_step(str(allocation_period_form_step))
            if data:
                dictionary.update(data)
                dictionary["allocation_amount"] = self._get_service_units_to_allocate(
        ...
        if step > pooling_preference_form_step:
            data = self.get_cleaned_data_for_step(str(pooling_preference_form_step))
            if data:
                dictionary.update(data)
                ...
                if (
                    preference == form_class.UNPOOLED_TO_UNPOOLED
                    or preference == form_class.POOLED_TO_POOLED_SAME
                ):
                    dictionary["requested_project"] = dictionary["current_project"]
        ...
        if step > project_selection_form_step:
            try:
                data = self.get_cleaned_data_for_step(str(project_selection_form_step))
            except KeyError:
                pass
            else:
                if data:
...
class AllocationRenewalRequestUnderProjectView(
    LoginRequiredMixin, UserPassesTestMixin, AllocationRenewalMixin, SessionWizardView
):
    FORMS = [
        ("allocation_period", SavioProjectAllocationPeriodForm),
    ...
    def done(self, form_list, **kwargs):
        """Perform processing and store information in a request
        object."""
        redirect_url = reverse("project-detail", kwargs={"pk": self.project_obj.pk})
        tmp = {}
        ...
        billing_id = tmp.get("billing_id", None)
        if billing_id is not None:
            self._update_project_billing_activities(self.project_obj, billing_id)
    ...
    def test_func(self):
        ...
        if is_user_manager_or_pi_of_project(user, self.project_obj):
            return True
        message = (
            "You must be an active Manager or Principal Investigator of the Project."
        )
        messages.error(self.request, message)
    ...
    def _set_data_from_previous_steps(self, step, dictionary):
        ...
        if step > pi_selection_form_step:
            ...
            if data:
                ...
                form_class = ProjectRenewalPoolingPreferenceForm
                dictionary["breadcrumb_pooling_preference"] = (
                    form_class.SHORT_DESCRIPTIONS.get(pooling_preference, "Unknown")
                )
