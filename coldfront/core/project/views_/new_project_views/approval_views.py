...
from coldfront.core.project.forms import (
    MemorandumSignedForm,
    ReviewDenyForm,
    ReviewStatusForm,
)
from coldfront.core.project.forms_.new_project_forms.approval_forms import (
    SavioProjectAllocationRequestSearchForm,
    SavioProjectReviewSetupForm,
    VectorProjectReviewSetupForm,
)
from coldfront.core.project.forms_.new_project_forms.request_forms import (
    NewProjectExtraFieldsFormFactory,
...
from coldfront.core.project.utils_.new_project_utils import (
    ...
    SavioProjectProcessingRunner,
    VectorProjectProcessingRunner,
    savio_request_state_status,
    send_project_request_pooling_email,
    send_project_request_ready_for_processing_email,
    vector_request_state_status,
)
...
class SavioProjectRequestListView(LoginRequiredMixin, ListFilterMixin, TemplateView):
...
class SavioProjectRequestMixin(object):
    ...
    def get_service_units_to_allocate(self):
        ...
        if self.computing_allowance_obj.are_service_units_user_specified():
            num_service_units_int = self.request_obj.extra_fields["num_service_units"]
            ...
            num_service_units = calculate_service_units_to_allocate(
                self.computing_allowance_obj, self.request_obj.request_time, **kwargs
            )
    ...
    def redirect_if_disallowed_status(
        self,
        http_request,
    ...
    def set_common_context_data(self, context):
        ...
        context["allowance_requires_funds_transfer"] = (
            ...
            and context["allowance_has_extra_fields"]
        )
        try:
...
class SavioProjectRequestEditExtraFieldsView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, TemplateView
):
    template_name = (
        "project/project_request/savio/project_request_edit_extra_fields.html"
    )
    ...
    def post(self, request, *args, **kwargs):
        form = NewProjectExtraFieldsFormFactory().get_form(
            self.computing_allowance_obj, request.POST
        )
        if form.is_valid():
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            reverse("new-project-request-detail", kwargs={"pk": self.request_obj.pk})
        )
...
class SavioProjectRequestNotifyPIView(
    MOURequestNotifyPIViewMixIn, SavioProjectRequestEditExtraFieldsView
):
    def email_pi(self):
        super()._email_pi(
            "Savio Project Request Ready To Be Signed",
...
class SavioProjectRequestDetailView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, DetailView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            latest_update_timestamp = self.request_obj.latest_update_timestamp()
        ...
        context["mou_required"] = ComputingAllowance(
            self.request_obj.computing_allowance
        ).requires_memorandum_of_understanding()
        if context["mou_required"]:
            context["can_download_mou"] = (
                self.request_obj.state["notified"]["status"] == "Complete"
            )
            context["can_upload_mou"] = self.request_obj.status.name == "Under Review"
            context["mou_uploaded"] = bool(self.request_obj.mou_file)

            context["unsigned_download_url"] = reverse(
                "new-project-request-download-unsigned-mou",
                kwargs={"pk": self.request_obj.pk, "request_type": "new-project"},
            )
            context["signed_download_url"] = reverse(
                "new-project-request-download-mou",
                kwargs={"pk": self.request_obj.pk, "request_type": "new-project"},
            )
            context["signed_upload_url"] = reverse(
                "new-project-request-upload-mou",
                kwargs={"pk": self.request_obj.pk, "request_type": "new-project"},
            )
            context["mou_type"] = "Memorandum of Understanding"

        context["is_recharge"] = ComputingAllowance(
            self.request_obj.computing_allowance
        ).is_recharge()
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            should_process_request = self.has_request_allocation_period_started()
    ...
    def get_checklist(self):
        ...
        checklist.append(
            [
                ...
                True,
                reverse("new-project-request-review-eligibility", kwargs={"pk": pk}),
            ]
        )
        ...
        checklist.append(
            [
                ("Confirm that the project satisfies the readiness status criteria."),
                readiness["status"],
                readiness["timestamp"],
                True,
                reverse("new-project-request-review-readiness", kwargs={"pk": pk}),
            ]
        )
        ...
        if mou_required:
            ...
            checklist.append(
                [
                    ...
                    reverse("new-project-request-notify-pi", kwargs={"pk": pk}),
                ]
            )
            is_notified = notified["status"] == "Complete"

            memorandum_signed = state["memorandum_signed"]
            task_text = "Confirm that the Memorandum of Understanding has been signed."
            if (
                self.computing_allowance_obj.is_recharge()
                and self.computing_allowance_obj.requires_extra_information()
            ):
                task_text += " Additionally, confirm that funds have been transferred."
            checklist.append(
                [
                    task_text,
        ...
        is_memorandum_signed = (
            not mou_required or state["memorandum_signed"]["status"] == "Complete"
        )
    ...
    def get_setup_status(self):
        ...
        if (
            ...
            or state["readiness"]["status"] == "Denied"
        ):
            return "N/A"
        else:
            pending = "Pending"
            if self.computing_allowance_obj.requires_memorandum_of_understanding():
    ...
    def is_checklist_complete(self):
        ...
        return (
            status_choice.name == "Approved - Processing"
            and self.request_obj.state["setup"]["status"] == "Complete"
        )


class SavioProjectReviewEligibilityView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, FormView
):
    form_class = ReviewStatusForm
    template_name = "project/project_request/savio/project_review_eligibility.html"
    ...
    def get_success_url(self):
        return reverse(
            "new-project-request-detail", kwargs={"pk": self.kwargs.get("pk")}
        )


class SavioProjectReviewReadinessView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, FormView
):
    form_class = ReviewStatusForm
    template_name = "project/project_request/savio/project_review_readiness.html"
...
class SavioProjectReviewMemorandumSignedView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, FormView
):
    ...
    def dispatch(self, request, *args, **kwargs):
        pk = self.kwargs.get("pk")
        self.set_attributes(pk)
        if not self.computing_allowance_obj.requires_memorandum_of_understanding():
            message = (
...
class SavioProjectReviewSetupView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, FormView
):
    ...
    def form_valid(self, form):
        form_data = form.cleaned_data
        status = form_data["status"]
        requested_name = self.request_obj.state["setup"]["name_change"][
            "requested_name"
        ]
        ...
        message = (
            f"Setup status for request {self.request_obj.pk} has been set to {status}."
        )
        messages.success(self.request, message)
    ...
    def get_form_kwargs(self):
        ...
        kwargs["requested_name"] = self.request_obj.state["setup"]["name_change"][
            "requested_name"
        ]
...
class SavioProjectReviewDenyView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, FormView
):
    form_class = ReviewDenyForm
    template_name = "project/project_request/savio/project_review_deny.html"
...
class SavioProjectUndenyRequestView(
    LoginRequiredMixin, UserPassesTestMixin, SavioProjectRequestMixin, View
):
    def test_func(self):
        """UserPassesTestMixin tests."""
        if self.request.user.is_superuser:
        ...
        message = "You do not have permission to undeny a project request."
        messages.error(self.request, message)
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            reverse("new-project-request-detail", kwargs={"pk": kwargs.get("pk")})
        )
...
class VectorProjectRequestMixin(object):
    ...
    def redirect_if_disallowed_status(
        self, http_request, disallowed_status_names=("Approved - Complete", "Denied")
    ):
        """Return a redirect response to the detail view for this
        project request if its status has one of the given disallowed
        names, after sending a message to the user. Otherwise, return
        ...
        if not isinstance(self.request_obj, VectorProjectAllocationRequest):
            raise TypeError(
                f"Request object has unexpected type {type(self.request_obj)}."
            )
        status_name = self.request_obj.status.name
        if status_name in disallowed_status_names:
            message = (
...
class VectorProjectRequestDetailView(
    LoginRequiredMixin, UserPassesTestMixin, VectorProjectRequestMixin, DetailView
):
...
class VectorProjectReviewEligibilityView(
    LoginRequiredMixin, UserPassesTestMixin, VectorProjectRequestMixin, FormView
):
    form_class = ReviewStatusForm
    template_name = "project/project_request/vector/project_review_eligibility.html"
    ...
    def get_success_url(self):
        return reverse(
            "vector-project-request-detail", kwargs={"pk": self.kwargs.get("pk")}
        )


class VectorProjectReviewSetupView(
    LoginRequiredMixin, UserPassesTestMixin, VectorProjectRequestMixin, FormView
):
...
class VectorProjectUndenyRequestView(
    LoginRequiredMixin, UserPassesTestMixin, VectorProjectRequestMixin, View
):
    def test_func(self):
        """UserPassesTestMixin tests."""
        if self.request.user.is_superuser:
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            reverse("vector-project-request-detail", kwargs={"pk": kwargs.get("pk")})
        )
