...
class ProjectRemoveUsersView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_users_to_remove(self, project_obj):
        ...
        if num_managers > 1:
            query_set = (
                project_obj.projectuser_set.filter(status__name="Active")
                .exclude(role__name="Principal Investigator")
                .order_by("user__username")
            )
        else:
            query_set = (
                project_obj.projectuser_set.filter(status__name="Active")
                .exclude(role__name__in=["Principal Investigator", "Manager"])
                .order_by("user__username")
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        user_pks = [
            int(user_pk) for user_pk in self.request.POST.get("user_pks").split(",")
        ]
        ...
        for user_pk in user_pks:
            ...
            try:
                ...
                logger.exception(e)
                error_message = "Unexpected error. Please contact an administrator."
...
class ProjectRemoveSelf(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    def test_func(self):
        ...
        if project_obj.projectuser_set.filter(
            user=self.request.user, role__name="User", status__name="Active"
        ).exists():
            return True
        ...
        if (
            ...
            and len(project_obj.projectuser_set.filter(role__name="Manager")) > 1
        ):
            return True
...
class ProjectRemovalRequestListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        ...
        project_removal_status_complete, _ = (
            ProjectUserRemovalRequestStatusChoice.objects.get_or_create(name="Complete")
        )
        ...
        project_removal_status_not_complete = [
            project_removal_status_pending,
        ...
        if removal_request_search_form.is_valid():
            ...
            if data.get("username"):
                project_removal_request_list = project_removal_request_list.filter(
            ...
            if data.get("requester"):
                project_removal_request_list = project_removal_request_list.filter(
                    requester__username__icontains=data.get("requester")
                )
    ...
    def test_func(self):
        ...
        message = "You do not have permission to review project removal requests."
...
# TODO: forms.
class ProjectRemovalRequestUpdateStatusView(
    LoginRequiredMixin, UserPassesTestMixin, FormView
):
    form_class = ProjectRemovalRequestUpdateStatusForm
    template_name = "project/project_removal/project_removal_request_update_status.html"
    ...
    def test_func(self):
        ...
        message = "You do not have permission to update project removal requests."
        messages.error(self.request, message)
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if status != "Pending":
            ...
            messages.error(request, message)
            return HttpResponseRedirect(reverse("project-removal-request-list"))
        return super().dispatch(request, *args, **kwargs)
...
class ProjectRemovalRequestCompleteStatusView(
    LoginRequiredMixin, UserPassesTestMixin, FormView
):
