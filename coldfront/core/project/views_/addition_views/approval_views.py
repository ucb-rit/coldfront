...
from coldfront.core.project.forms import MemorandumSignedForm, ReviewDenyForm
...
from coldfront.core.project.utils_.addition_utils import (
    AllocationAdditionDenialRunner,
...
class AllocationAdditionRequestDetailView(
    ...
    def get_context_data(self, **kwargs):
        ...
        context["review_controls_visible"] = (
            ...
            and self.request_obj.status.name not in ("Denied", "Complete")
        )
        ...
        context["unsigned_download_url"] = reverse(
            "service-units-purchase-request-download-unsigned-mou",
            kwargs={
        ...
        context["signed_download_url"] = reverse(
            "service-units-purchase-request-download-mou",
            kwargs={
                "pk": self.request_obj.pk,
                "request_type": "service-units-purchase",
            },
        ...
        context["signed_upload_url"] = reverse(
            "service-units-purchase-request-upload-mou",
            kwargs={
                "pk": self.request_obj.pk,
                "request_type": "service-units-purchase",
            },
...
class AllocationAdditionRequestListView(
    ...
    def get_context_data(self, **kwargs):
        ...
        request_list = AllocationAdditionRequest.objects.filter(
            status__name__in=status__name__in
        ).order_by(order_by)
...
class AllocationAdditionReviewBase(LoginRequiredMixin, UserPassesTestMixin, FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if status_name in ["Complete", "Denied"]:
            ...
            return HttpResponseRedirect(
                reverse("service-units-purchase-request-detail", kwargs={"pk": pk})
            )
        return super().dispatch(request, *args, **kwargs)
...
class AllocationAdditionReviewMemorandumSignedView(AllocationAdditionReviewBase):
    ...
    def get_template_names(self):
        return [os.path.join(self.template_dir, "review_memorandum_signed.html")]


class AllocationAdditionEditExtraFieldsView(
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data["num_service_units"] != str(
            self.request_obj.num_service_units
        ):
            self.request_obj.num_service_units = Decimal(
                form.cleaned_data["num_service_units"]
            )
        self.request_obj.save()
...
class AllocationAdditionNotifyPIView(
    MOURequestNotifyPIViewMixIn, AllocationAdditionEditExtraFieldsView
):
    def email_pi(self):
        super()._email_pi(
            "Service Units Purchase Request Ready To Be Signed",
