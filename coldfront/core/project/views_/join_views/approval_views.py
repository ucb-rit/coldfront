...
class ProjectReviewJoinRequestsView(
    ...
    def test_func(self):
        return self._is_superuser_or_project_owner() or self.request.user.has_perm(
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.project_obj.status.name not in [
            "Active",
            "Inactive",
            "New",
        ]:
    ...
    def get(self, request, *args, **kwargs):
        ...
        if users_to_review:
            formset = formset_factory(self.form_class, max_num=len(users_to_review))
    ...
    def post(self, request, *args, **kwargs):
        ...
        users_to_review = self.users_to_review
        formset = formset_factory(self.form_class, max_num=len(users_to_review))
        formset = formset(request.POST, initial=users_to_review, prefix="userform")
        ...
        for form in formset:
            ...
            project_user_obj = self.project_obj.projectuser_set.get(user=user_obj)
            try:
    ...
    @staticmethod
    def _process_project_user_request(project_user_obj, project_user_status_choice):
...
class ProjectJoinRequestListView(LoginRequiredMixin, UserPassesTestMixin, ListView):
    ...
    def get_queryset(self):
        ...
        project_join_requests = ProjectUserJoinRequest.objects.filter(
        ...
        if join_request_search_form.is_valid():
            ...
            if data.get("username"):
                project_join_requests = project_join_requests.filter(
    ...
    def test_func(self):
        ...
        message = "You do not have permission to view project join requests."
