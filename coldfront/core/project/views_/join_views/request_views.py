...
from coldfront.core.project.forms import ProjectSearchForm, ProjectSelectHostUserForm
...
from coldfront.core.user.utils_.host_user_utils import is_lbl_employee, needs_host
...
class ProjectJoinView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def test_func(self):
        ...
        if self.request.user.userprofile.access_agreement_signed_date is None:
            messages.error(
                self.request,
                "You must sign the User Access Agreement before you can join a project.",
            )
            return False
        ...
        if project_users.exists():
            ...
            if project_user.status.name == "Active":
                message = f"You are already a member of Project {project_obj.name}."
                messages.error(self.request, message)
                return False
            if project_user.status.name == "Pending - Add":
                message = (
                    f"You have already requested to join Project {project_obj.name}."
                )
        ...
        if len(reason) < 20:
            message = (
                "Please provide a valid reason to join the project (min 20 characters)"
            )
            messages.error(self.request, message)
            return False
    ...
    def post(self, request, *args, **kwargs):
        ...
        select_host_user_form = ProjectSelectHostUserForm(
            project=project_obj.name, data=self.request.POST
        )
        host_user = None
        if select_host_user_form.is_valid():
            host_user = User.objects.get(
        ...
        if project_users.exists():
            ...
            if project_user.status.name == "Active":
                message = (
                    f"You are already an Active member of Project {project_obj.name}."
                )
                messages.warning(self.request, message)
            ...
            project_user = ProjectUser.objects.create(
                user=user_obj, project=project_obj, role=role, status=status
            )

        # Create a join request
        ProjectUserJoinRequest.objects.create(
            project_user=project_user, reason=reason, host_user=host_user
        )
...
class ProjectJoinListView(ProjectListView, UserPassesTestMixin):
    ...
    def get_queryset(self):
        ...
        projects = (
            Project.objects.prefetch_related("field_of_science", "status")
            .filter(
                status__name__in=[
                    "Active",
                ]
    ...
    def get_context_data(self, **kwargs):
        ...
        is_requester_or_pi = Q(requester=user_obj) | Q(pi=user_obj)
        pending_project_request_statuses = ["Under Review", "Approved - Processing"]
        is_part_of_pending_savio_project_request = set(
            SavioProjectAllocationRequest.objects.prefetch_related("project")
            .filter(
                is_requester_or_pi, status__name__in=pending_project_request_statuses
            )
        ...
        is_part_of_pending_vector_project_request = set(
            VectorProjectAllocationRequest.objects.prefetch_related("project")
            .filter(
                is_requester_or_pi, status__name__in=pending_project_request_statuses
            )
            .values_list("project__name", flat=True)
        )
        pending_removal_requests = set(
            [
        ...
        join_requests = Project.objects.filter(
        ...
        is_being_added_to_project = ProjectUser.objects.filter(
            user=user_obj, status__name="Pending - Add"
        ).exists()
        context["require_host_user"] = (
            flag_enabled("LRC_ONLY")
            and not is_lbl_employee(user_obj)
            and needs_host(user_obj)
            and not is_being_added_to_project
        )
        if context["require_host_user"]:
            selecthostform_dict = {}
            for project in context.get("project_list"):
                selecthostform_dict[project.name] = ProjectSelectHostUserForm(
