...
from coldfront.api.statistics.utils import (
    ...
    set_project_allocation_value,
    set_project_usage_value,
    set_project_user_allocation_value,
...
from coldfront.core.utils.common import project_detail_url, utc_now_offset_aware
...
class AllocationAdditionRunnerBase:
    ...
    def assert_request_status(self, expected_status):
        ...
        if not isinstance(expected_status, AllocationAdditionRequestStatusChoice):
            raise TypeError("Status is not an AllocationAdditionRequestStatusChoice.")
...
class AllocationAdditionDenialRunner(AllocationAdditionRunnerBase):
    ...
    def deny_request(self):
        ...
        self.request_obj.status = AllocationAdditionRequestStatusChoice.objects.get(
            name="Denied"
        )
    ...
    def send_email(self):
        ...
        try:
            ...
            template_name = "email/project_allocation_addition/request_denied.txt"
            context = {
...
class AllocationAdditionProcessingRunner(AllocationAdditionRunnerBase):
    ...
    def run(self):
        ...
        total_service_units = self.compute_updated_service_units(added_service_units)
    ...
    def send_email(self, added_service_units, total_service_units):
        ...
        try:
            request = self.request_obj
            project = request.project
            subject = f"Service Units Purchase Request ({project.name}) Processed"
            template_name = "email/project_allocation_addition/request_processed.txt"
            context = {
    ...
    def update_allocation(self, num_service_units, date_time):
        ...
        ProjectTransaction.objects.create(
            project=project, date_time=date_time, allocation=num_service_units
        )
...
def has_pending_allocation_addition_request(project):
    ...
    return AllocationAdditionRequest.objects.filter(
        project=project, status=under_review_status
    ).exists()
