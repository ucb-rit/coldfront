...
def create_project_users(project, requester, pi, request_class, email_strategy=None):
    ...
    if not is_requester_pi:
        ...
        if project.projectuser_set.filter(user=requester).exists():
            requester_project_user = project.projectuser_set.get(user=requester)
            ...
            requester_project_user = ProjectUser.objects.create(
                project=project, user=requester, role=manager_role, status=active_status
            )
            run_runner = True
        if run_runner:
