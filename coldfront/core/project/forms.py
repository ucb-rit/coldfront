...
from coldfront.core.project.models import Project, ProjectReview, ProjectUserRoleChoice
...
class ProjectSearchForm(forms.Form):
    ...
    cluster_name = forms.ChoiceField(
        label=CLUSTER_NAME, required=False, widget=forms.Select()
    )
...
class ProjectAddUserForm(forms.Form):
    ...
    role = forms.ModelChoiceField(
        queryset=ProjectUserRoleChoice.objects.all().filter(
            ~Q(name="Principal Investigator")
        ),
        empty_label=None,
    )
    selected = forms.BooleanField(initial=False, required=False)
    user_access_agreement = forms.CharField(
        max_length=16, required=False, disabled=True
    )
...
class ProjectReviewEmailForm(forms.Form):
    ...
    def __init__(self, pk, *args, **kwargs):
        ...
        self.fields["email_body"].initial = f"Dear {project_review_obj.project.name} managers \n{EMAIL_DIRECTOR_PENDING_PROJECT_REVIEW_EMAIL}"
...
class ProjectSelectHostUserForm(forms.Form):
    host_user = forms.ChoiceField(
        label="Host User", choices=[], widget=forms.Select(), required=True
    )
