...
class SavioProjectAllocationPeriodForm(forms.Form):
    ...
    def allocation_period_choices(self, computing_allowance, utc_dt, display_timezone):
    ...
    def _allocation_period_choices_brc(self, computing_allowance, date, f, order_by):
    ...
    def _allocation_period_choices_lrc(self, computing_allowance, date, f, order_by):
        allowance_name = computing_allowance.get_name()
    ...
    def _allocation_period_choices_allowance_year(
        self, computing_allowance, date, f, order_by
    ):
        if flag_enabled("ALLOCATION_RENEWAL_FOR_NEXT_PERIOD_REQUESTABLE"):
            ...
            started_before_date = date + timedelta(
                days=self.NUM_DAYS_IN_ALLOCATION_YEAR
            )
...
class ComputingAllowanceForm(forms.Form):
    computing_allowance = ComputingAllowanceField(
        ...
        queryset=Resource.objects.filter(
            resource_type__name="Computing Allowance"
        ).order_by("pk"),
    )
    ...
    def disable_pi_choices(self):
        ...
        if self.computing_allowance.is_one_per_pi() and self.allocation_period:
            ...
            new_project_request_status_names = list(
                non_denied_new_project_request_statuses().values_list("name", flat=True)
            )
            disable_user_pks.update(
                pis_with_new_project_requests_pks(
            ...
            renewal_request_status_names = list(
                non_denied_renewal_request_statuses().values_list("name", flat=True)
            )
            disable_user_pks.update(
                pis_with_renewal_requests_pks(
                    self.allocation_period,
        ...
        if flag_enabled("LRC_ONLY"):
            ...
            disable_user_pks.update(
                User.objects.exclude(pk__in=lbl_employees()).values_list(
                    "pk", flat=True
                )
            )
...
class NewProjectExtraFieldsFormFactory:
    ...
    def _get_form_class(computing_allowance):
        ...
        raise ValueError(
            f"Computing Allowance {allowance_name} does not require extra fields."
        )
...
class SavioProjectICAExtraFieldsForm(SavioProjectExtraFieldsForm):
    ...
    num_students = forms.IntegerField(
        help_text=(
            "Specify the number of students you anticipate having in this course."
        ),
        ...
        widget=TextInput(
            attrs={"type": "number", "min": "1", "max": "3000", "step": "1"}
        ),
    )
    num_gsis = forms.IntegerField(
        ...
        widget=TextInput(
            attrs={"type": "number", "min": "1", "max": "50", "step": "1"}
        ),
    )
    ...
    max_simultaneous_jobs = forms.IntegerField(
        ...
        widget=TextInput(
            attrs={"type": "number", "min": "1", "max": "100000", "step": "1"}
        ),
    )
    max_simultaneous_nodes = forms.IntegerField(
        ...
        widget=TextInput(
            attrs={"type": "number", "min": "1", "max": "10000", "step": "1"}
        ),
    )
...
class SavioProjectRechargeExtraFieldsForm(SavioProjectExtraFieldsForm):
    ...
    chartstring_account_type = forms.CharField(
        help_text=("Provide the type of account represented by the chartstring."),
...
class PooledProjectChoiceField(forms.ModelChoiceField):
    def label_from_instance(self, obj):
        ...
        names = sorted(
            f"{pu.user.first_name} {pu.user.last_name}" for pu in pi_project_users
        )
...
class SavioProjectPooledProjectSelectionForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.computing_allowance is not None:
            self.computing_allowance = ComputingAllowance(self.computing_allowance)
            prefix = self.interface.code_from_name(self.computing_allowance.get_name())
...
class SavioProjectDetailsForm(forms.Form):
    name = forms.CharField(
        ...
        validators=[
            ...
            RegexValidator(
                r"^[0-9a-z]+$",
                message=("Name must contain only lowercase letters and numbers."),
            ),
        ],
    )
    ...
    def clean_name(self):
        ...
        if Project.objects.filter(name=name):
            raise forms.ValidationError(f"A project with name {name} already exists.")
        return name
...
class SavioProjectSurveyForm(forms.Form):
    ...
    system_needs = forms.MultipleChoiceField(
        choices=(
            ...
            (
                "cannot_purchase",
                ("Provides a resource since my group/area cannot purchase its own"),
            ),
            (
        ...
        label=("Which of the following best describes your need for this system:"),
        required=False,
    ...
    memory_per_core = forms.CharField(
        label="How much memory per core does your typical job require?", required=False
    )
    ...
    interconnect = forms.ChoiceField(
        ...
        help_text=(
            "Does your application require low latency communication between nodes?"
        ),
    ...
    def _update_field_attributes(self):
        ...
        if flag_enabled("BRC_ONLY"):
            ...
            if (
                isinstance(self.computing_allowance, ComputingAllowance)
                and self.computing_allowance.is_instructional()
            ):
                self.fields[
                    "scope_and_intent"
                ].label = "Scope and intent of coursework needing computation"
                ...
                ].label = "Computational aspects of the coursework"
                self.fields["existing_resources"].label = (
                    f"Existing computing resources (outside of "
