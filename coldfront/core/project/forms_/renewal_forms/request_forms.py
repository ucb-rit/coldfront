...
class ProjectRenewalPISelectionForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not (
            self.computing_allowance and self.allocation_period_pk and self.project_pks
        ):
            return
        ...
        pi_project_users = (
            ProjectUser.objects.prefetch_related("user")
            .filter(project__pk__in=self.project_pks, role=role, status=status)
...
class ProjectRenewalPoolingPreferenceForm(forms.Form):
    ...
    POOLED_TO_POOLED_DIFFERENT = AllocationRenewalRequest.POOLED_TO_POOLED_DIFFERENT
    ...
    non_pooled_choices = [
        (UNPOOLED_TO_UNPOOLED, "Renew the PI's allowance under the same project."),
        (UNPOOLED_TO_POOLED, "Pool the PI's allowance under a different project."),
    ]
    ...
    pooled_choices = [
        ...
        (
            ...
            ("Stop pooling the PI's allowance. Create a new project to renew under."),
        ),
    ]
...
class ProjectRenewalProjectSelectionForm(forms.Form):
    project = PooledProjectChoiceField(
        queryset=Project.objects.none(), required=True, widget=forms.Select()
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        project_pks = list(
            ...
            .filter(user__pk=self.pi_pk, role=role, status=status)
        ...
        self.fields["project"].queryset = (
            Project.objects.filter(**_filter).exclude(**exclude).order_by("name")
        )
...
class ProjectRenewalSurveyForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["was_survey_completed"] = forms.BooleanField(
            label=(
                "I have completed the survey and did NOT edit the pre-filled"
    ...
    def validate_survey_completed(self, value):
        ...
        try:
            is_survey_completed = is_renewal_survey_completed(
                self.allocation_period_name, self.project_name, self.pi_username
            )
...
class DeprecatedProjectRenewalSurveyForm(forms.Form):
    ...
    def _update_brc_survey_fields(self):
        ...
        self.fields["do_you_use_mybrc"] = forms.ChoiceField(
            ...
            label=("9. Do you use the Savio Account Management portal MyBRC?"),
            required=True,
            widget=forms.RadioSelect(),
        )
        ...
        self.fields[
            ...
            label=(
                mark_safe(
                    "<h4>BRC is planning an in-person training session within"
        ...
        self.fields["training_session_usefulness_of_basic_savio_cluster"] = (
            forms.ChoiceField(
                choices=(
                    ("1", ("1 - Not useful")),
                    ("2", ("2")),
        ...
        self.fields["training_session_usefulness_of_advanced_savio_cluster"] = (
            forms.ChoiceField(
                choices=(
                    ("1", ("1 - Not useful")),
                    ("2", ("2")),
        ...
        self.fields["training_session_usefulness_of_singularity_on_savio"] = (
            forms.ChoiceField(
                choices=(
                    ("1", ("1 - Not useful")),
                    ("2", ("2")),
        ...
        self.fields["training_session_usefulness_of_analytic_envs_on_demand"] = (
            forms.ChoiceField(
                choices=(
                    ("1", ("1 - Not useful")),
                    ("2", ("2")),
    ...
    def _update_lrc_survey_fields(self):
        ...
        self.fields["question_1"] = forms.CharField(
            label="Question 1", required=True, widget=forms.Textarea(attrs={"rows": 3})
        )

        self.fields["question_2"] = forms.CharField(
            label="Question 2", required=True, widget=forms.Textarea(attrs={"rows": 3})
        )
