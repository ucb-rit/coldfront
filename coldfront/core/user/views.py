...
from coldfront.core.user.forms import (
    UserAccessAgreementForm,
...
from coldfront.core.user.utils import (
    ...
    send_account_already_active_email,
)
...
class UserProfile(TemplateView):
    ...
    def _get_flag_states(self):
        ...
        self._flag_user_departments_enabled = flag_enabled("USER_DEPARTMENTS_ENABLED")
    ...
    def _update_context_with_billing_data(context, viewed_user):
        ...
        try:
            ...
            message = f"User {viewed_user.username} unexpectedly has no UserProfile."
    ...
    def _update_context_with_email_and_account_data(self, context, viewed_user):
        ...
        context["email_addresses_visible"] = self._flag_multiple_email_addresses_allowed
        ...
        context["third_party_accounts_visible"] = (
            self._flag_sso_enabled and self._flag_multiple_email_addresses_allowed
        )
    ...
    def _update_context_with_identity_linking_request_data(self, context):
        ...
        pending_requests = user_requests.filter(status__name="Pending").order_by(
            "request_time"
        )
        complete_requests = user_requests.filter(status__name="Complete").order_by(
            "completion_time"
        )
...
class UserSearchAll(LoginRequiredMixin, ListView):
    ...
    def _filter_users_by_email(users, email):
        user_ids = EmailAddress.objects.filter(
            primary=False, email__icontains=email
...
class UserReactivateView(FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            self.logger.error(
                f"Unexpectedly found multiple Users with email address {email}."
            )
...
def activate_user_account(request, uidb64=None, token=None):
    ...
    if user and token:
        ...
        message = "Failed to activate account. Please contact an administrator."
        messages.error(request, message)
...
class EmailAddressExistsView(View):
    def get(self, request, *args, **kwargs):
        email_address_exists = EmailAddress.objects.filter(
            email=self.kwargs.get("email").lower()
        ).exists()
