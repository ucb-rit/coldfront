...
class TestBase(AllTestsBase):
    ...
    def assert_has_access(self, user, has_access, url):
        ...
        status_code = (
            [HTTPStatus.OK, HTTPStatus.FOUND] if has_access else [HTTPStatus.FORBIDDEN]
        )
...
class TestStaffViewPermissions(TestBase):
    ...
    def test_pi_allocation_renewal_request_detail(self):
        ...
        under_review_request_status = AllocationRenewalRequestStatusChoice.objects.get(
            name="Under Review"
        )
        ...
        self.assert_has_access(
            self.staff1, True, reverse("pi-allocation-renewal-pending-request-list")
        )

        # pi-allocation-renewal-completed-request-list
        self.assert_has_access(
            self.staff1, True, reverse("pi-allocation-renewal-completed-request-list")
        )
    ...
    def test_vector_project_request(self):
        ...
        status = ProjectAllocationRequestStatusChoice.objects.get(name="Under Review")
        request = VectorProjectAllocationRequest.objects.create(
            requester=self.user1, pi=self.user1, project=new_project, status=status
        )
        ...
        self.assert_has_access(
            self.staff1,
            True,
            reverse("vector-project-request-detail", kwargs={"pk": request.pk}),
        )

        # vector-project-pending-request-list
        self.assert_has_access(
            self.staff1, True, reverse("vector-project-pending-request-list")
        )

        # vector-project-completed-request-list
        self.assert_has_access(
            self.staff1, True, reverse("vector-project-completed-request-list")
        )
    ...
    def test_savio_project_request(self):
        ...
        under_review_request_status = ProjectAllocationRequestStatusChoice.objects.get(
            name="Under Review"
        )
        ...
        self.assert_has_access(
            self.staff1,
            True,
            reverse("new-project-request-detail", kwargs={"pk": request.pk}),
        )

        # new-project-pending-request-list
        self.assert_has_access(
            self.staff1, True, reverse("new-project-pending-request-list")
        )

        # new-project-completed-request-list
        self.assert_has_access(
            self.staff1, True, reverse("new-project-completed-request-list")
        )
    ...
    def test_project_review(self):
        project_review = ProjectReview.objects.create(
            project=self.project,
            status=ProjectReviewStatusChoice.objects.get(name="Completed"),
        ...
        self.assert_has_access(
            self.staff1,
            True,
            reverse("project-review-email", kwargs={"pk": project_review.pk}),
        )
    ...
    def test_allocation(self):
        ...
        self.assert_has_access(
            self.staff1,
            True,
            reverse("allocation-detail", kwargs={"pk": allocation.pk}),
        )
    ...
    def test_project_detail(self):
        # project-detail
        self.assert_has_access(
            self.staff1, True, reverse("project-detail", kwargs={"pk": self.project.pk})
        )
