...
class TestLinkPersonalAccount(TestBase):
        ...
        def assert_button_html(is_enabled):
            ...
            if not is_enabled:
                ...
                with self.assertRaises(Exception) as _:
                    soup.find("form", {"id": "request-linking-email-form"}).find(
                        "button"
                    )
            else:
                ...
                button = soup.find("form", {"id": "request-linking-email-form"}).find(
                    "button"
                )
            self.assertIn("Request Linking Email", str(button))

        # The user has cluster access.
        allocation_user_attribute = grant_user_cluster_access_under_test_project(
            self.user
        )
    ...
    def test_section_hidden_if_viewing_other_user_profile(self):
        ...
        url = reverse("user-profile", kwargs={"viewed_username": requester.username})
        response = self.client.get(url)
        html = response.content.decode("utf-8")
    ...
    def test_time_requested_and_time_sent_correct(self):
        ...
        def assert_times(_time_requested, _time_sent):
            ...
            requested_td = str(soup.find("td", {"id": "time-requested-timestamp"}))
        ...
        def format_date(d):
            ...
            return localize(d.astimezone(pytz.timezone(settings.DISPLAY_TIME_ZONE)))
