...
class RequestHubView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_cluster_account_request(self):
        ...
        cluster_account_list_complete = ClusterAccessRequest.objects.filter(
            status__name__in=["Denied", "Complete"], **kwargs
        ).order_by("-modified")

        cluster_account_list_pending = ClusterAccessRequest.objects.filter(
            status__name__in=["Pending - Add", "Processing"], **kwargs
        ).order_by("-modified")

        cluster_request_object.num = self.paginators
        cluster_request_object.pending_queryset = self.create_paginator(
            cluster_account_list_pending
        )

        cluster_request_object.complete_queryset = self.create_paginator(
        ...
        cluster_request_object.button_text = "Go To Cluster Access Requests Main Page"
        cluster_request_object.id = "cluster_access_request_section"
        cluster_request_object.help_text = "Showing your cluster access requests."
    ...
    def get_faculty_storage_allocation_request(self):
        ...
        fsa_request_list_pending = FacultyStorageAllocationRequest.objects.filter(
            status__name__in=pending_status_names, *args
        ).order_by("-request_time")

        complete_status_names = ["Approved - Complete", "Denied"]
        fsa_request_list_complete = FacultyStorageAllocationRequest.objects.filter(
    ...
    def get_hardware_procurement_request(self):
        ...
        hardware_procurement_request_obj.pending_queryset = self.create_paginator(
            pending_hardware_procurements
        )
        hardware_procurement_request_obj.complete_queryset = self.create_paginator(
            complete_hardware_procurements
        )
        ...
        hardware_procurement_request_obj.table = (
            "hardware_procurements/hardware_procurement_request_list_table.html"
        )
        hardware_procurement_request_obj.button_path = "hardware-procurement-list"
        hardware_procurement_request_obj.button_text = (
            "Go To Hardware Procurement Requests Main Page"
        )
        hardware_procurement_request_obj.id = "hardware_procurement_request_section"
    ...
    def get_project_removal_request(self):
        ...
        removal_request_pending = ProjectUserRemovalRequest.objects.filter(
        ...
        removal_request_object.pending_queryset = self.create_paginator(
            removal_request_pending
        )

        removal_request_object.complete_queryset = self.create_paginator(
        ...
        removal_request_object.button_path = "project-removal-request-list"
        removal_request_object.button_text = "Go To Project Removal Requests Main Page"
    ...
    def get_savio_project_request(self):
        ...
        savio_proj_request_object.pending_queryset = self.create_paginator(
            project_request_pending
        )

        savio_proj_request_object.complete_queryset = self.create_paginator(
            project_request_complete
        )
        ...
        savio_proj_request_object.button_path = "new-project-request-list"
        savio_proj_request_object.button_text = "Go To New Project Requests Main Page"
    ...
    def get_vector_project_request(self):
        ...
        project_request_pending = VectorProjectAllocationRequest.objects.filter(
        ...
        vector_proj_request_object.pending_queryset = self.create_paginator(
            project_request_pending
        )

        vector_proj_request_object.complete_queryset = self.create_paginator(
            project_request_complete
        )
        ...
        vector_proj_request_object.button_path = "vector-project-pending-request-list"
    ...
    def get_project_join_request(self):
        ...
        project_join_request_pending = ProjectUserJoinRequest.objects.filter(
        ...
        proj_join_request_object.pending_queryset = self.create_paginator(
            project_join_request_pending
        )

        proj_join_request_object.complete_queryset = self.create_paginator(
            project_join_request_complete
        )

        proj_join_request_object.num_pending = project_join_request_pending.count()

        proj_join_request_object.title = "Project Join Requests"
        proj_join_request_object.table = "project/project_join_request_list_table.html"
        proj_join_request_object.button_path = "project-join-request-list"
        proj_join_request_object.button_text = "Go To Project Join Requests Main Page"
    ...
    def get_project_renewal_request(self):
        ...
        proj_renewal_request_object.pending_queryset = self.create_paginator(
            project_renewal_request_pending
        )

        proj_renewal_request_object.complete_queryset = self.create_paginator(
            project_renewal_request_complete
        )
        ...
        proj_renewal_request_object.button_path = "pi-allocation-renewal-request-list"
    ...
    def get_su_purchase_request(self):
        ...
        if not self.show_all_requests:
            request_ids = [
                r.id
                for r in su_purchase_request_pending
                if is_user_manager_or_pi_of_project(user, r.project)
            ]
            su_purchase_request_pending = su_purchase_request_pending.filter(
                id__in=request_ids
            )

            request_ids = [
                r.id
                for r in su_purchase_request_complete
                if is_user_manager_or_pi_of_project(user, r.project)
            ]
            su_purchase_request_complete = su_purchase_request_complete.filter(
                id__in=request_ids
            )

        su_purchase_request_object.num = self.paginators
        su_purchase_request_object.pending_queryset = self.create_paginator(
            su_purchase_request_pending
        )

        su_purchase_request_object.complete_queryset = self.create_paginator(
            su_purchase_request_complete
        )

        su_purchase_request_object.num_pending = su_purchase_request_pending.count()
    ...
    def get_secure_dir_join_request(self):
        ...
        if not self.show_all_requests:
            ...
            request_pks = [
                request.pk
                for request in secure_dir_join_pending
                if request.allocation.project.projectuser_set.filter(
                    user=user,
            ...
            request_pks = [
                request.pk
                for request in secure_dir_join_complete
                if request.allocation.project.projectuser_set.filter(
                    user=user,
                    role__name="Principal Investigator",
                    status__name="Active",
        ...
        secure_dir_join_request_object.pending_queryset = self.create_paginator(
            secure_dir_join_pending
        )

        secure_dir_join_request_object.complete_queryset = self.create_paginator(
            secure_dir_join_complete
        )

        secure_dir_join_request_object.num_pending = secure_dir_join_pending.count()
        ...
        secure_dir_join_request_object.button_arg1 = "add"
        secure_dir_join_request_object.button_arg2 = "pending"
    ...
    def get_secure_dir_remove_request(self):
        ...
        if not self.show_all_requests:
            ...
            request_pks = [
                request.pk
                for request in secure_dir_remove_pending
                if request.allocation.project.projectuser_set.filter(
                    user=user,
                    role__name="Principal Investigator",
                    status__name="Active",
            ...
            request_pks = [
                request.pk
                for request in secure_dir_remove_complete
                if request.allocation.project.projectuser_set.filter(
                    user=user,
                    role__name="Principal Investigator",
                    status__name="Active",
        ...
        secure_dir_remove_request_object.pending_queryset = self.create_paginator(
            secure_dir_remove_pending
        )

        secure_dir_remove_request_object.complete_queryset = self.create_paginator(
            secure_dir_remove_complete
        )

        secure_dir_remove_request_object.num_pending = secure_dir_remove_pending.count()

        secure_dir_remove_request_object.title = "Secure Directory Removal Requests"
        ...
        secure_dir_remove_request_object.button_arg1 = "remove"
        secure_dir_remove_request_object.button_arg2 = "pending"
        secure_dir_remove_request_object.button_text = (
            "Go To Secure Directory Removal Requests Main Page"
        )
        secure_dir_remove_request_object.id = "secure_dir_remove_request_section"
    ...
    def get_secure_dir_request(self):
        ...
        if not self.show_all_requests:
            ...
            request_pks = [
                request.pk
                for request in secure_dir_pending
                if request.project.projectuser_set.filter(
                    user=user,
                    role__name="Principal Investigator",
                    status__name="Active",
                ).exists()
            ...
            request_pks = [
                request.pk
                for request in secure_dir_complete
                if request.project.projectuser_set.filter(
                    user=user,
                    role__name="Principal Investigator",
                    status__name="Active",
        ...
        secure_dir_request_object.pending_queryset = self.create_paginator(
            secure_dir_pending
        )

        secure_dir_request_object.complete_queryset = self.create_paginator(
            secure_dir_complete
        )

        secure_dir_request_object.num_pending = secure_dir_pending.count()

        secure_dir_request_object.title = "Secure Directory Requests"
        secure_dir_request_object.table = (
            "secure_dir/secure_dir_request/secure_dir_request_list_table.html"
        )
        secure_dir_request_object.button_path = "secure-dir-pending-request-list"
        secure_dir_request_object.button_text = (
            "Go To Secure Directory Requests Main Page"
        )
        secure_dir_request_object.id = "secure_dir_request_section"
    ...
    def get_context_data(self, **kwargs):
        ...
        ) and self.show_all_requests

        for req in requests:
            request_obj = eval(f"self.get_{req}()")
            if context["show_all"]:
                request_obj.help_text = (
                    f"Showing all {request_obj.title} in {settings.PORTAL_NAME}."
                )
