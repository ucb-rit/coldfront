...
# Settings for the 'New-Additions' tab of the 'LRC Projects(Master)'
# spreadsheet.
...
PROJECT_SPREADSHEET_ID = '14YeWGBHYK-9fQ1QtxKzRhyqoPqJlvOZWD8VHocpBX50'
PROJECT_SPREADSHEET_ROW_START = 2
PROJECT_SPREADSHEET_TAB = 'New-Additions'

USER_PROJECT_FILE_PATH = '/tmp/user_projects'

# Settings for the 'All-Users-Current' tab of the 'LRC Accounts(Master)'
# spreadsheet.
...
USER_SPREADSHEET_ID = '1TEE_WPtmxoxqVrsTSVy64XdfGQbS7DsrPvB58gKJ87o'
USER_SPREADSHEET_ROW_START = 1
USER_SPREADSHEET_TAB = 'All-Users-Current'


class Command(BaseCommand):

    help = 'Creates users and accounts from the LRC spreadsheets.'
    ...
    def handle(self, *args, **options):
        ...
        # already exist).
        user_project_data = self.get_user_project_data(USER_PROJECT_FILE_PATH)
    ...
    def get_project_data():
        ...
        col_start = 1
        col_end = PROJECT_SPREADSHEET_COLS['PI_EMAILS']
    ...
    def get_user_data():
        ...
        col_start = 1
        col_end = USER_SPREADSHEET_COLS['EMAIL']
        return get_gspread_worksheet_data(
            worksheet, row_start, row_end, col_start, col_end)

    def get_user_project_data(self, user_project_file_path):
        """Return a list of lists where each entry corresponds to a
        single user and its projects from the file of user-project
        associations.
        ...
        Raises:
            - Exception, if any errors occur
        """
        if not self.file_exists(user_project_file_path):
            raise FileNotFoundError(
                f'File {user_project_file_path} does not exist.')
        user_project_associations = []
        with open(user_project_file_path, 'r') as user_project_file:
            for line in user_project_file:
                fields = [field.strip() for field in line.rstrip().split(':')]
                if len(fields) != 8:
                    self.logger.error(
                        f'The association {fields} does not have 7 fields.')
                    continue
                user_username = fields[0].strip()
                if not user_username:
                    self.logger.error(
                        f'The association {fields} is missing a username.')
                    continue
                project_names = ','.join(
                    [p.split('|')[0] for p in fields[7].strip().split(',')])
                user_project_associations.append(
                    [user_username, project_names])
        return user_project_associations
