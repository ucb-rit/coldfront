...
class Command(BaseCommand):
    ...
    def assert_condo_and_recharge_project_allocation_values(self):
        lawrencium_compute_resource = Resource.objects.get(name="LAWRENCIUM Compute")
        computing_allowance_interface = get_computing_allowance_interface()
        for resource_name in ("Condo Allocation", "Recharge Allocation"):
            prefix = computing_allowance_interface.code_from_name(resource_name)
            for project in Project.objects.filter(name__startswith=prefix).iterator():
                ...
                allocation = Allocation.objects.filter(
                    project=project, resources=lawrencium_compute_resource
                ).first()
    ...
    def assert_departmental_project_allocation_values(self):
        ...
        for resource in departmental_compute_resources:
            ...
            if num_allocations == 0:
                message = f'There is no Allocation to Resource "{resource.name}".'
    ...
    def assert_pca_project_allocation_values(self):
        ...
        computing_allowance = Resource.objects.get(name="PI Computing Allowance")

        computing_allowance_interface = get_computing_allowance_interface()
        prefix = computing_allowance_interface.code_from_name("PI Computing Allowance")
        ...
        for project in Project.objects.filter(name__startswith=prefix).iterator():
            ...
            if allocation.start_date is None:
                message = f"Allocation {allocation.pk} unexpectedly has no start_date."
                self._write_error(message)
            else:
            ...
            self._assert_allocation_end_date(
                allocation, allocation_period.end_date if was_renewed else None
            )
            # The Allocation should have Service Units proportional to the
            # number of renewals under it.
            expected_num_service_units = num_complete_requests * num_service_units
            self._assert_allocation_num_service_units(
    ...
    def assert_user_profile_is_pi_consistent_with_project_pis(self):
        user_pks_with_pi_status = set(
            User.objects.filter(userprofile__is_pi=True).values_list("pk", flat=True)
        )
        seen_users = set()

        project_users = ProjectUser.objects.filter(role__name="Principal Investigator")
    ...
    def _assert_allocation_num_service_units(
        self, allocation, expected_num_service_units
    ):
        ...
        if allocation_attribute is None:
            message = f'Allocation {allocation.pk} has no "Service Units" attribute.'
            self._write_error(message)
    ...
    def _assert_allocation_user_num_service_units(
        self, allocation_user, expected_num_service_units
    ):
        allocation_attribute_type = AllocationAttributeType.objects.get(
            name="Service Units"
        )
        ...
        if allocation_user_attribute is None:
            message = (
                f'AllocationUser {allocation_user.pk} has no "Service Units" attribute.'
            )
            self._write_error(message)
            return
    ...
    def _assert_allocation_user_status(self, allocation_user, expected_status_name):
