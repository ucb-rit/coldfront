...
class Command(BaseCommand):
    help = (
        "Update the state and extra fields for Project Allocation Requests"
        ", Secure Directory Requests, and Allocation Addition Requests."
    )

    def add_arguments(self, parser):
        parser.add_argument(
            "--all", action="store_true", help="Update both state and extra fields"
        )
        parser.add_argument("--state", action="store_true", help="Update state")
        parser.add_argument(
            "--extra_fields", action="store_true", help="Update extra fields"
        )
    ...
    def add_notified_to_state(request, dry_run=False):
        if "notified" not in request.state:
            status = "Pending" if request.status.name == "Under Review" else "Complete"
    ...
    def handle(self, *args, **options):
        ...
        if options["state"]:
            for request in SavioProjectAllocationRequest.objects.all():
                if ComputingAllowance(
                    request.computing_allowance
        ...
        if options["extra_fields"]:
            for request in SavioProjectAllocationRequest.objects.all():
                if ComputingAllowance(request.computing_allowance).is_instructional():
