...
def get_role_and_pi_mapping():
    ...
    with open(file_path, "r") as fp:
        for line in fp:
            ...
            PROJECT_TITLE, GRANT_TITLE, RF_Award_Number, ROLE, PI = line.strip().split(
                delimiter
            )
        ...
        with open(file_path, "r") as fp:
            for line in fp:
                ...
                if funding_agency in agency_mapping:
                    funding_agency_obj = GrantFundingAgency.objects.get(
                        name=agency_mapping[funding_agency]
                    )
                ...
                try:
                    project_obj = Project.objects.get(
                        title=project__title.strip(),
                        pi__username=project__pi__username.strip(),
                        status__name=project_status,
                    )
                except:
                    print(project__title, project__pi__username)

                grant_status_choice_obj = GrantStatusChoice.objects.get(
                    name=status.title()
                )
