...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--all", action="store_true", help="Run all non-LRC-only checks"
        )
        parser.add_argument(
            "--allocation-date",
            action="store_true",
    ...
    def handle(self, *args, **options):
        ...
        for option in options:
            if (
                isinstance(options[option], bool)
                and options[option]
                and option not in ("all", "lrc_all")
            ):
    ...
    def handle_allocation_date(self):
        ...
        """
        allocations = (
            Allocation.objects.select_related("project", "project__status")
        ...
        for allocation in allocations:
            ...
            if ComputingAllowance(resource).is_yearly():
                ...
                if (
                    ...
                    and end_date != YEARLY_ALLOCATION_PERIOD.end_date
                ):
                    self.stdout.write(
                        self.style.ERROR(
                            f"{resource_name} {id} "
                ...
                if project_status == "Active" and not any(
                    end_date != ica.end_date for ica in INSTRUCTIONAL_ALLOCATION_PERIODS
                ):
                    self.stdout.write(
                        self.style.ERROR(
                            f"{resource_name} {id} "
            ...
            # Condo or Recharge
                ...
                ComputingAllowance(resource).is_recharge()
                or ComputingAllowance(resource).is_condo()
            ):
    ...
    def handle_project_inactive(self):
        ...
        for project in inactive_projects:
            for allocation in project.allocation_set.select_related(
                "status"
            ).prefetch_related("allocationattribute_set", "resources"):
                try:
    ...
    def handle_project_pi(self):
        ...
        """
        projects = Project.objects.select_related("status").order_by(
            "status__name", "name"
        )
        for project in projects:
            if not project.pis().exists():
                self.stdout.write(
                    self.style.ERROR(
                        f"{project.status.name} Project {project.name} has no PIs."
                    )
                )
    ...
    def handle_user_project(self):
        ...
        for userprofile in userprofiles:
            ...
            user_project_exists = ProjectUser.objects.filter(user=user).exists()
    ...
    def handle_lrc_user_billing(self):
        ...
        """
        userprofiles = UserProfile.objects.filter(cluster_uid__isnull=False).only(
            "id",
            "billing_activity",
            "user__first_name",
            "user__last_name",
    ...
    def handle_lrc_recharge_allocation_billing(self):
        ...
        allocations = Allocation.objects.prefetch_related("resources").filter(
            resources__name__endswith="Compute"
        )
        for allocation in allocations:
            if (
                allocation.allocationattribute_set.filter(
                    type="Billing Activity"
                ).count()
                == 0
            ):
                self.stdout.write(
    ...
    def handle_lrc_recharge_allocation_user_billing(self):
        ...
        allocation_users = AllocationUser.objects.filter(
            allocation_resources__name__endswith="Compute"
        )
