...
from django.db.models import CharField, DurationField, ExpressionWrapper, F, Func, Value
...
from coldfront.core.project.forms_.renewal_forms.request_forms import (
    DeprecatedProjectRenewalSurveyForm,
)
...
class Command(BaseCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for allowance in self.computing_allowance_interface.allowances():
            self.allowance_prefixes.append(
                self.computing_allowance_interface.code_from_name(allowance.name)
            )
        self.allocation_periods = [
            period for period in AllocationPeriod.objects.values_list("name", flat=True)
        ]
    ...
    def add_subparsers(self, subparsers):
        ...
        latest_jobs_by_user_parser = subparsers.add_parser(
            "latest_jobs_by_user",
        ...
        new_cluster_accounts_parser = subparsers.add_parser(
            "new_cluster_accounts",
            help="Export list of new user accounts created since a given date.",
        )
        ...
        job_avg_queue_time_parser = subparsers.add_parser(
            "job_avg_queue_time",
            help="Export average queue time for jobs between the given dates.",
        )
        job_avg_queue_time_parser.add_argument(
            "--start_date",
            help='Starting date for jobs. Must take the form of "MM-DD-YYYY".',
            type=valid_date,
        )
        job_avg_queue_time_parser.add_argument(
            "--end_date",
            help='Ending date for jobs. Must take the form of "MM-DD-YYYY".',
            type=valid_date,
        )
        job_avg_queue_time_parser.add_argument(
            "--allowance_type",
        ...
        job_avg_queue_time_parser.add_argument(
            "--partition", help="Filter jobs by the partition they requested.", type=str
        )

        user_subparser = subparsers.add_parser("users", help="Export user data.")
        ...
        user_subparser.add_argument(
            "--pi_only", help="Only return PI users.", action="store_true"
        )

        project_subparser = subparsers.add_parser(
            "projects", help="Export projects data"
        )
        ...
        new_project_requests_subparser = subparsers.add_parser(
            "new_project_requests", help="Export new project requests"
        )
        new_project_requests_subparser.add_argument(
            "--type",
        ...
        new_project_requests_subparser.add_argument(
            "--start_date",
            help='Get Requests from this date.Must take the form of "MM-DD-YYYY".',
            type=valid_date,
        )
        new_project_requests_subparser.add_argument(
            "--format",
        ...
        new_project_survey_responses_subparser = subparsers.add_parser(
            "new_project_survey_responses",
            help="Export survey responses for new projects",
        )
        new_project_survey_responses_subparser.add_argument(
            "--format",
            help="Format to dump new project survey responses in",
            type=str,
            required=True,
            choices=["json", "csv"],
        )
        ...
        renewal_survey_responses_subparser = subparsers.add_parser(
            "renewal_survey_responses",
            help="Export survey responses for project renewals",
        )
        renewal_survey_responses_subparser.add_argument(
            "--format",
            help="Format to dump renewal survey responses in",
            type=str,
            required=True,
            choices=["json", "csv"],
        )
    ...
    def handle_latest_jobs_by_user(self, *args, **options):
        ...
        query_set = query_set.order_by("userid", "-submitdate").distinct("userid")

        if format == "csv":
            query_set = query_set.values_list(*fields)
            self.to_csv(query_set, header=[*fields], output=output, error=error)

        else:
            query_set = query_set.values(*fields)
            self.to_json(query_set, output=output, error=error)
    ...
    def handle_new_cluster_accounts(self, *args, **options):
        ...
        query_set = AllocationUserAttribute.objects.filter(
            allocation_attribute_type=cluster_account_status, value="Active"
        )
        ...
        query_set = query_set.order_by("username", "-created").distinct("username")
    ...
    def handle_users(self, *args, **kwargs):
        ...
        def _get_department_fields_for_user(_user):
            ...
            departments."""
            from coldfront.plugins.departments.utils.queries import (
            ...
            authoritative_str = ";".join([s for s in authoritative_department_strs])
    ...
    def handle_new_project_requests(self, *args, **kwargs):
        ...
        for request in requests:
            requesters.append(
                f"{request.requester.first_name} "
        ...
        for project, status, requester, pi, request in zip(
            projects, statuses, requesters, pis, requests
        ):
    ...
    def handle_renewal_survey_responses(self, *args, **kwargs):
        ...
        _surveys = list(
            allocation_requests.values_list("renewal_survey_answers", flat=True)
        )
        _surveys = [i for i in _surveys if i != {}]
        if len(_surveys) == 0:
            raise Exception(
                "There are no valid renewal requests in the specified allocation period."
            )

        if {} in _surveys:
            raise Exception(
                "This allocation period does not have an associated survey."
            )
        surveys = []
        ...
        for k, v in form.fields.items():
            ...
            if (isinstance(v, forms.MultipleChoiceField)) or (
                isinstance(v, forms.ChoiceField)
            ):
        ...
        if format == "csv":
            ...
            try:
                ...
                writer = csv.DictWriter(
                    kwargs.get("stdout", stdout), headers, extrasaction="ignore"
                )
...
def valid_date(s):
    try:
        ...
        msg = f'{s} is not a valid date. Must take the form of "MM-DD-YYYY".'
