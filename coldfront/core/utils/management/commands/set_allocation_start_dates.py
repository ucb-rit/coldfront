...
class Command(BaseCommand):
    help = "Sets Allocation start dates for the Projects listed in the given file."
        ...
        with open(kwargs["file"]) as list_file:
            for i, entry in enumerate(list_file):
                ...
                try:
                    ...
                    continue
                try:
                    parsed_date = datetime.strptime(date_time, self.date_format).date()
                except ValueError:
    ...
    def set_allocation_start_date(self, project, date):
        ...
        try:
            ...
            allocation.save()
            self.logger.info(f"Set Allocation {allocation.pk} start_date to {date}.")
