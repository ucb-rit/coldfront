...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        with open(
            os.path.join(base_dir, "local_data/R3_resource_attributes_types.tsv")
        ) as file:
            for line in file:
                attribute_type_name, resource_type_name, name, required = (
                    line.strip().split("\t")
                )

                resource_attribute_type_obj, created = (
        ...
        with open(
            os.path.join(base_dir, "local_data/R5_resource_attributes.tsv")
        ) as file:
            for line in file:
                if not line.strip():
                    continue
                (
                    ...
                    resource_attribute_type_type_name,
                    resource_name,
                    value,
                ) = line.strip().split("\t")
                ...
                if resource_attribute_type_name == "Access":
                    ...
                    resource_attribute_obj, created = (
                        ResourceAttribute.objects.get_or_create(
                            resource_attribute_type=ResourceAttributeType.objects.get(
                                name=resource_attribute_type_name,
                                attribute_type__name=resource_attribute_type_type_name,
                            ),
