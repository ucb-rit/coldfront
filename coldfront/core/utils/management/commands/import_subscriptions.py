...
from coldfront.core.subscription.models import (
    AttributeType,
    Subscription,
    ...
    def handle(self, *args, **options):
        ...
        with open(filepath, "r") as fp:
            for line in fp:
                ...
                subscription_attribute_type_obj = (
                    SubscriptionAttributeType.objects.create(
        ...
        subscription_status_choices["ACT"] = SubscriptionStatusChoice.objects.get(
            name="Active"
        )
        subscription_status_choices["PEN"] = SubscriptionStatusChoice.objects.get(
            name="Pending"
        )
        subscription_status_choices["EXP"] = SubscriptionStatusChoice.objects.get(
            name="Expired"
        )
        subscription_status_choices["DEN"] = SubscriptionStatusChoice.objects.get(
            name="Denied"
        )
        subscription_status_choices["REV"] = SubscriptionStatusChoice.objects.get(
            name="Revoked"
        )
        subscription_status_choices["UNP"] = SubscriptionStatusChoice.objects.get(
            name="Unpaid"
        )
        subscription_status_choices["NEW"] = SubscriptionStatusChoice.objects.get(
            name="New"
        )
        subscription_status_choices["INA"] = SubscriptionStatusChoice.objects.get(
            name="Inactive (Renewed)"
        )
        subscription_status_choices["APR"] = SubscriptionStatusChoice.objects.get(
            name="Approved"
        )

        subscription_user_status_choices = {}
        subscription_user_status_choices["ACT"] = (
            SubscriptionUserStatusChoice.objects.get(name="Active")
        )
        subscription_user_status_choices["PEA"] = (
            SubscriptionUserStatusChoice.objects.get(name="Denied")
        )
        subscription_user_status_choices["PER"] = (
            SubscriptionUserStatusChoice.objects.get(name="Pending - Add")
        )
        subscription_user_status_choices["DEN"] = (
            SubscriptionUserStatusChoice.objects.get(name="Pending - Remove")
        )
        subscription_user_status_choices["REM"] = (
            SubscriptionUserStatusChoice.objects.get(name="Removed")
        )
        ...
        with open(filepath, "r") as fp:
            ...
            for line in lines:
                ...
                end_date_datetime_obj = datetime.datetime.strptime(
                    active_util, "%Y-%m-%d"
                )

                created = datetime.datetime.strptime(
                    created.strip(), "%Y-%m-%d %H:%M:%S"
                ).date()
                modified = datetime.datetime.strptime(
                    modified.strip(), "%Y-%m-%d %H:%M:%S"
                ).date()
                ...
                if attributes != "N/A":
                    for subscription_attribute in attributes.split(";"):
                        name, value, is_private, usage_value = (
                            subscription_attribute.split(",")
                        )
                        if is_private == "True":
                            is_private = True
                        else:
                        ...
                        subscription_attribute_type_obj = (
                            SubscriptionAttributeType.objects.get(name=name)
                        )
                        subscription_attribute_obj = SubscriptionAttribute.objects.create(
                            subscription=subscription_obj,
                            subscription_attribute_type=subscription_attribute_type_obj,
                        ...
                        if (
                            subscription_attribute_obj.subscription_attribute_type.has_usage
                            and usage_value != "N/A"
                        ):
                            subscription_attribute_usage_obj = (
                                SubscriptionAttributeUsage(
