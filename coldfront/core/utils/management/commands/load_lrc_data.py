...
class Command(BaseCommand):
    ...
    def add_subparsers(subparsers):
        ...
        def add_file_argument(parser, file_arg_name):
            ...
            path."""
            parser.add_argument(
                file_arg_name, help=f"The path to the {file_arg_name} file.", type=str
            )
        ...
        billing_ids_parser = subparsers.add_parser(
            "billing_ids", help="Load IDs to be used for monthly billing from a file."
        )
        ...
        add_file_argument(project_pis_and_managers_parser, "project_users_file")

        projects_and_project_users_parser = subparsers.add_parser(
            "projects_and_project_users",
            help=("Load Projects and their ProjectUsers from a pre-defined file."),
        )
        add_file_argument(projects_and_project_users_parser, "project_users_file")
    ...
    def handle_billing_ids(self, *args, **options):
        ...
        def get_or_create_with_cache(model, key, cache, **get_or_create_kwargs):
        ...
        for username, billing_id in user_account_fee_ids.items():
            ...
            if created:
                self.logger.info(
                    f"Invalid, unknown BillingProject {bp_identifier} was created."
                )
            ...
            billing_activity, created = get_or_create_with_cache(
                ...
                identifier=ba_identifier,
            )
            if created:
                self.logger.info(
                    f"Invalid, unknown BillingActivity {billing_id} was created."
                )
        ...
        for project_name in job_usage_fee_id_users_by_project:
            ...
            job_usage_fee_id_users = job_usage_fee_id_users_by_project[project_name]
            max_user_count = 0
            ...
            for billing_id, usernames in job_usage_fee_id_users.items():
                ...
                for username in usernames:
                    ...
                    set_allocation_user_attribute_value(
                        allocation_user, "Billing Activity", str(billing_activity.pk)
                    )
                    self.logger.info(
                ...
                if employee_id in user_data_by_employee_id:
                    ...
                    if matching_users.exists():
                        ...
                        if matching_email_addresses.exists():
                            new_host_user = matching_email_addresses.first().user
                        else:
    ...
    def handle_project_pis_and_managers(self, *args, **options):
        ...
        project_pis_and_managers_data = self.get_project_pis_and_managers_data(
            options["project_users_file"]
        )
    ...
    def handle_projects_and_project_users(self, *args, **options):
        ...
        project_and_project_users_data = self.get_project_and_project_users_data(
            options["project_users_file"]
        )
    ...
    def create_project_pis_and_managers(self, project_pis_and_managers_data):
        ...
        manager_project_user_role = ProjectUserRoleChoice.objects.get(name="Manager")
        ...
        for project_name, user_data in project_pis_and_managers_data.items():
            ...
            for email, attributes in user_data.items():
                ...
                try:
                    ...
                    self.logger.error(
                        f"Email {email} unexpectedly belongs to multiple users."
                    )
                    continue
                # Prefer existing names (from the cluster) over those from the
                # spreadsheet.
                user.first_name = user.first_name or attributes["first_name"].title()
                user.last_name = user.last_name or attributes["last_name"].title()
                user.save()
                username = user.username
                ...
                if created:
                    self.logger.info(f"UserProfile for User {username} was created.")
                user_profile.middle_name = (
                    user_profile.middle_name or attributes["middle_name"].title()
                )
                ...
                try:
                    ...
                    self.logger.info(
                        f"ProjectUser ({project_name}, {username}) was created."
                    )
                else:
    ...
    def create_projects_and_project_users(self, project_and_project_users_data):
    ...
    def get_billing_ids_data(self, billing_file_path):
        ...
        with open(billing_file_path, "r") as billing_file:
            for line in billing_file:
                ...
                if len(fields) != 8:
                    self.logger.error(f"The entry {fields} does not have 8 fields.")
                    continue
                user_username = fields[0].strip()
                if not user_username:
                    self.logger.error(f"The entry {fields} is missing a username.")
                    continue
                ...
                all_pids = set.union(
                    {user_account_fee_pid}, set(job_usage_fee_pids.values())
                )
        ...
        return {
            ...
            "job_usage_fee_id_users_by_project": (job_usage_fee_id_users_by_project),
        }
    ...
    def get_departmental_cluster_names():
        ...
        names, in lowercase."""
        all_names = set([name.lower() for name in get_compute_resource_names()])
    ...
    def get_project_pis_and_managers_data(self, project_users_file_path):
        ...
        def parse_users_from_string(s):
            ...
            for part in parts:
                ...
                if not self.is_full_name_valid(_full_name):
                    self.logger.error(
                        f"The entry {fields} has an invalid name: {_full_name}."
                    )
                    break
                _email = part[left_chevron_pos + 1 : -1]
                if not self.is_email_address_valid(_email):
                    self.logger.error(
                        f"The entry {fields} has an invalid email: {_email}."
                    )
                    break
        ...
        with open(project_users_file_path, "r") as project_users_file:
            ...
            for row in reader:
                ...
                if len(fields) != 10:
                    self.logger.error(f"The entry {fields} does not have 10 fields.")
                    continue
                row_dict = {}
        ...
        with open(project_users_file_path, "r") as project_users_file:
            ...
            for row in reader:
                ...
                if not self.is_project_name_valid(
                    ...
                    continue
                usernames = set(
                    [u.strip().lower() for u in fields[9].split(",") if u.strip()]
                )
        ...
        with open(passwd_file_path, "r") as passwd_file:
            for line in passwd_file:
                ...
                if full_name_and_email.count(",") != 1:
                    self.logger.error(
                        f"The user {fields} has a malformed full name and email pair."
                    )
                    continue
                full_name, user_email = full_name_and_email.split(",")
                if not self.is_full_name_valid(full_name):
                    self.logger.error(f"The user {fields} has a malformed full name.")
                    continue
                names = self.get_first_middle_last_names(full_name)
    ...
    def is_project_name_valid(project_name, departmental_cluster_names):
        ...
        return (
            project_name.startswith(LAWRENCIUM_PROJECT_PREFIXES)
            or project_name in departmental_cluster_names
        )

    def set_up_departmental_project_allocations(self, usernames_by_project_name):
        ...
        for project in Project.objects.filter(name__in=departmental_cluster_names):
            resource = Resource.objects.get(name=f"{project.name.upper()} Compute")
            ...
            usernames_on_cluster = usernames_by_project_name.get(project.name, {})
    ...
    def set_up_lawrencium_project_allocations(self, usernames_by_project_name):
        ...
        if not isinstance(current_allowance_year_period, AllocationPeriod):
            raise AllocationPeriod.DoesNotExist(
                "Unexpected: No AllocationPeriod exists for the current allowance year."
            )
    ...
    def set_up_project_allocation_to_compute_resource(
        self, project, resource, start_date=None, end_date=None, num_service_units=None
    ):
        ...
        allocation_attribute, created = AllocationAttribute.objects.update_or_create(
            allocation_attribute_type=allocation_attribute_type,
            allocation=allocation,
            defaults=allocation_attribute_defaults,
        )
    ...
    @staticmethod
    def set_up_project_allocation_user(
        project_user,
        allocation,
        ...
        allocation_user = get_or_create_active_allocation_user(allocation, user)
