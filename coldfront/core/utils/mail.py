...
def send_email(subject, body, sender, receiver_list, cc=[], html_body=""):
    ...
    try:
        email = EmailMultiAlternatives(
            subject, body, sender, receiver_list, cc=cc, reply_to=reply_to
        )
        if html_body:
...
def send_email_template(
    subject, template_name, context, sender, receiver_list, cc=[], html_template=None
):
    ...
    return send_email(
        subject, plain_body, sender, receiver_list, cc=cc, html_body=html_body
    )
...
def dummy_email_address():
    ...
    if not dev_email_list:
        raise ImproperlyConfigured(
            "There should be at least one address in EMAIL_DEVELOPMENT_EMAIL_LIST."
        )
