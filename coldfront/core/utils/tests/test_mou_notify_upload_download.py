...
class MOUTestBase(TestBase):
    ...
    def download_unsigned_mou_url(pk, request_type):
        return reverse(
            f"{request_type}-request-download-unsigned-mou",
            kwargs={"pk": pk, "request_type": request_type},
        )

    @staticmethod
    def upload_mou_url(pk, request_type):
        return reverse(
            f"{request_type}-request-upload-mou",
            kwargs={"pk": pk, "request_type": request_type},
        )
    ...
    def download_mou_url(pk, request_type):
        return reverse(
            f"{request_type}-request-download-mou",
            kwargs={"pk": pk, "request_type": request_type},
        )
    ...
    def _test_download_upload_download(self, request_type, expected_recipients):
        ...
        self.assertEqual(
            response.get("Content-Disposition"), f'attachment; filename="{filename}"'
        )
...
class TestNewProjectMOUNotifyUploadDownload(MOUTestBase):
    ...
    def setUp(self):
        ...
        self.project, self.request = self.create_project_and_request(
            "ic_testproject", computing_allowance, self.user
        )

    @staticmethod
    def create_project_and_request(project_name, computing_allowance, requester_and_pi):
    ...
    def test_new_project(self):
        ...
        self.assertEqual(
            self.request.extra_fields["course_department"], "Dept. of Testing"
        )
        ...
        self.assertEqual(
            ...
            "[MyBRC-User-Portal] Savio Project Request Ready To Be Signed",
        )
...
class TestAllocationAdditionMOUNotifyUploadDownload(MOUTestBase):
    ...
    def test_allocation_addition(self):
        ...
        self.assertEqual(
            self.request.extra_fields["chartstring_contact_name"], "Test User"
        )
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(
            mail.outbox[0].subject,
            "[MyBRC-User-Portal] Service Units Purchase Request Ready To Be Signed",
        )
...
class SecureDirMOUNotifyUploadDownload(MOUTestBase):
    ...
    def setUp(self):
        ...
        self.request = self.create_secure_directory_request(
            self.project, self.user, self.user
        )
    ...
    def test_secure_dir(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject,
            "[MyBRC-User-Portal] Secure Directory Request Ready To Be Signed",
        )
