...
class TestBaseExportData(TestBase):
    ...
    def convert_output(self, out, format):
        if format == "json":
            ...
            raise CommandError(
                'convert_output out_format must be either "csv" or "json"'
            )
...
class TestLatestJobsByUser(TestBaseExportData):
    ...
    def test_json_no_date(self):
        ...
        output, error = self.call_command(
            "export_data", "latest_jobs_by_user", "--format=json"
        )
        ...
        for index in range(2):
            ...
            submit_date_str = datetime.datetime.strftime(job.submitdate, DATE_FORMAT)
    ...
    def test_csv_no_date(self):
        ...
        output, error = self.call_command(
            "export_data", "latest_jobs_by_user", "--format=csv"
        )
        ...
        for index, item in enumerate(output):
            if index == 0:
                ...
                submit_date_str = datetime.datetime.strftime(
                    job.submitdate, DATE_FORMAT
                )
...
class TestNewClusterAccounts(TestBaseExportData):
    ...
    def setUp(self):
        ...
        self.pre_time = utc_now_offset_aware().replace(tzinfo=None, microsecond=0)
    ...
    def test_json_no_date(self):
        ...
        exporting as JSON"""
        output, error = self.call_command(
            "export_data", "new_cluster_accounts", "--format=json"
        )
        output = self.convert_output(output, "json")
        ...
        for index, item in enumerate(output):
            self.assertEqual(item["username"], f"user{index}")
            date_created = datetime.datetime.strptime(item["date_created"], DATE_FORMAT)
    ...
    def test_json_with_date(self):
        ...
        date_created = datetime.datetime.strptime(
            output[0]["date_created"], DATE_FORMAT
        )
        # TODO: This comparison is flaky. Make it not so.
        # self.assertTrue(self.pre_time <= date_created <= post_time)
    ...
    def test_csv_no_date(self):
        ...
        exporting as CSV"""
        output, error = self.call_command(
            "export_data", "new_cluster_accounts", "--format=csv"
        )
        output = self.convert_output(output, "csv")
        ...
        for index, item in enumerate(output):
            if index == 0:
                ...
                self.assertEqual(item[0], f"user{index - 1}")
                date_created = datetime.datetime.strptime(item[1], DATE_FORMAT)
                # TODO: This comparison is flaky. Make it not so.
                # self.assertTrue(self.pre_time <= date_created <= post_time)
...
class TestJobAvgQueueTime(TestBaseExportData):
    ...
    def test_no_dates(self):
        ...
        output, error = self.call_command("export_data", "job_avg_queue_time")
    ...
    def test_start_date(self):
        ...
        output, error = self.call_command(
            "export_data", "job_avg_queue_time", f"--start_date={start_date}"
        )
        self.assertIn("24hrs 0mins 0secs", output)
        self.assertEqual(error, "")
    ...
    def test_end_date(self):
        ...
        output, error = self.call_command(
            "export_data", "job_avg_queue_time", f"--end_date={end_date}"
        )
...
class TestProjects(TestBase):
    ...
    def setUp(self):
        ...
        active_project_ids = list(
            map(lambda x: x[0], active_projects.values_list("id"))
        )
        ...
        self.fc_queryset = [
            project for project in base_queryset if project["id"] in fc_project_ids
        ]
    ...
    def test_default(self):
        out, err = StringIO(""), StringIO("")
        call_command("export_data", "projects", "--format=csv", stdout=out, stderr=err)
    ...
    def test_format(self):
        ...
        out, err = StringIO(""), StringIO("")
        call_command("export_data", "projects", "--format=json", stdout=out, stderr=err)
...
class TestNewProjectRequests(TestBase):
    ...
    def setUp(self):
        ...
        fca_allocation_type = ComputingAllowanceInterface().name_short_from_name(
        ...
        for index in range(15):
            test_user = User.objects.create(
                ...
                email=f"user{index}@nonexistent.com",
            )
            project = Project.objects.create(
                name=f"test_project_{index}", status=project_status
            )

            projects.append(project.name)
            statuses.append(request_status.name)
            requesters.append(
                f"{test_user.first_name} "
        ...
        vector_queryset = []
        vector_requests = VectorProjectAllocationRequest.objects.all().values_list(
            *vector_headers
        )
        for project, project_status, requester, pi, request in zip(
            projects, statuses, requesters, pis, vector_requests
        ):
        ...
        savio_queryset = []
        savio_requests = SavioProjectAllocationRequest.objects.all().values_list(
            *savio_headers
        )
        for project, project_status, requester, pi, request in zip(
            projects[5:], statuses[5:], requesters[5:], pis[5:], savio_requests
        ):
            request = list(request)
            request[1] = str(request[1])
            request[2] = str(request[2])
        ...
        savio_newrequests = (
            ...
            .values_list(*savio_headers)
        )
        for project, project_status, requester, pi, request in zip(
            projects[5:], statuses[5:], requesters[5:], pis[5:], savio_newrequests
        ):
            request = list(request)
            request[1] = str(request[1])
            request[2] = str(request[2])
        ...
        self.savio_newqueryset = list(
            map(lambda r: dict(zip(savio_headers, r)), savio_newqueryset)
        )
        self.savio_queryset = list(
            map(lambda r: dict(zip(savio_headers, r)), savio_queryset)
        )
        self.vector_queryset = list(
            map(lambda r: dict(zip(vector_headers, r)), vector_queryset)
        )
...
class TestNewProjectSurveyResponses(TestBase):
    ...
    def setUp(self):
        ...
        for index in range(5):
            ...
            project = Project.objects.create(
                name=f"{project_prefix}test_project{index}", status=project_status
            )
    ...
    def test_new_project_survey_responses_json(self):
        ...
        for index, item in enumerate(output):
            ...
            self.assertDictEqual(
                item["new_project_survey_responses"],
                self.fixtures[index].survey_answers,
            )
...
class TestRenewalSurveyResponses(TestBase):
    ...
    def test_get_renewal_survey_responses_json(self):
        ...
        for index, item in enumerate(output):
            ...
            self.assertEqual(
                item["renewal_survey_responses"],
                self.swap_form_answer_id_for_text(fixture.renewal_survey_answers),
            )
        err.seek(0)
        self.assertEqual(err.read(), "")
    ...
    def test_get_renewal_survey_responses_csv(self):
        ...
        for index, item in enumerate(reader):
            ...
            sample = (
                "6. Based upon your overall experience using BRC "
