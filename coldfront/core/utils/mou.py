...
def get_context(request_obj):
    ...
    if isinstance(request_obj, SavioProjectAllocationRequest) and \
        ...
        context['dept_and_pi'] = f'{course_dept}/{pi_name}'
        context['between'] = f'{brc_name} (BRC) and {pi_name}'
        context['re'] = f'{dept_and_pi} ICA Agreement'
        ...
        context['allowance_end'] = request_obj.allocation_period.end_date
        context['allowance_last_month'] = allowance_end.strftime('%B %Y')
        context['allowance_year'] = AllocationPeriod.objects.get(
        ...
        context['signature'] = f'{pi_name}<br>{course_dept}'
        context['body'] = \
            ...
            f'for a course will generally be the Faculty member who requested the ICA and who signs the MOU form or the ' \
            f'course GSI) who will attempt to resolve issues and questions from students.</li>' \
            ...
            f'and up to {num_students} associated accounts for trainees. BRC will provide standard support to the point-of-contact, ' \
            f'including resolution of access issues, etc.' \
            f'<ul><li>The allowance will expire at the end of {allowance_last_month}. If {dept_and_pi} requires additional ' \
            f'resources for {allowance_year_short}, a renewal application can be submitted in {allowance_last_month}.</li>' \
            ...
            f'<li>The student/trainee accounts on Savio will be disabled when the allowance expires, ' \
        ...
        body = \
            ...
            f'BRC staff will provide {pi_name} with the details for using the purchased allowance, including how to submit ' \
        ...
        between = f'RTL / Research IT and {pi_name}'
        re = f'P2/P3 Savio project Researcher Use Agreement'
        signature = f'{pi_name}<br>{project}'
        mou_title = 'Researcher Use Agreement (RUA)'
        mou_subtitle = 'for using P2/P3 data in the Savio HPC environment'
        body = \
'''
