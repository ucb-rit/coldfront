...
class SecureDirManageUsersRequestListView(
    ...
    def test_func(self):
        ...
        if self.request.user.has_perm(
            "allocation.view_securediradduserrequest"
        ) and self.request.user.has_perm("allocation.view_securedirremoveuserrequest"):
            return True
    ...
    def get_queryset(self):
        ...
        secure_dir_request_search_form = SecureDirManageUsersSearchForm(
            self.request.GET
        )

        if self.completed:
            request_list = self.request_obj.objects.filter(status__in=complete_status)
        else:
            request_list = self.request_obj.objects.filter(status__in=pending_status)
        ...
        if secure_dir_request_search_form.is_valid():
            ...
            if data.get("project_name"):
                request_list = request_list.filter(
                    allocation__project__name__icontains=data.get("project_name")
                )

            if data.get("directory_name"):
                request_list = request_list.filter(
                    directory__icontains=data.get("directory_name")
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        if secure_dir_request_search_form.is_valid():
            ...
            context["secure_dir_request_search_form"] = SecureDirManageUsersSearchForm()
...
class SecureDirManageUsersUpdateStatusView(
    LoginRequiredMixin, UserPassesTestMixin, FormView
):
    form_class = SecureDirManageUsersRequestUpdateStatusForm
    template_name = "secure_dir/secure_dir_manage_user_request_update_status.html"
    ...
    def test_func(self):
        ...
        if self.request.user.has_perm(
            "allocation.change_securediradduserrequest"
        ) or self.request.user.has_perm("allocation.change_securedirremoveuserrequest"):
            return True
    ...
    def form_valid(self, form):
        ...
        if "Pending" not in cur_status:
            message = (
                f"Secure directory user {self.language_dict['noun']} "
        ...
        secure_dir_status_choice = self.request_status_obj.objects.filter(
...
class SecureDirManageUsersCompleteStatusView(
    LoginRequiredMixin, UserPassesTestMixin, FormView
):
...
class SecureDirManageUsersDenyRequestView(
    ...
    def post(self, request, *args, **kwargs):
        ...
        self.secure_dir_request.status = self.request_status_obj.objects.get(
            name="Denied"
        )
        self.secure_dir_request.completion_time = utc_now_offset_aware()
        self.secure_dir_request.save()
