...
from coldfront.core.allocation.utils_.secure_dir_utils.new_directory import (
    ...
    SecureDirRequestRunner,
    get_default_secure_dir_paths,
...
class NewSecureDirRequestViewAccessibilityMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        is_user_authorized = user.is_superuser or is_user_manager_or_pi_of_project(
            user, project_obj
        )
        ...
        if not is_project_eligible_for_secure_dirs(project_obj):
            message = (
                f"Project {project_obj.name} is ineligible for secure directories."
            )
...
class SecureDirRequestLandingView(
    LoginRequiredMixin, NewSecureDirRequestViewAccessibilityMixin, TemplateView
):
...
class SecureDirRequestWizard(
    LoginRequiredMixin, NewSecureDirRequestViewAccessibilityMixin, SessionWizardView
):
    FORMS = [
    ...
    def _set_data_from_previous_steps(self, step, dictionary):
        ...
        rdm_consultation_step = self.step_numbers_by_form_name["rdm_consultation"]
        if step > rdm_consultation_step:
            try:
                ...
                has_consulted_rdm = "Yes" if rdm_consultation_form_data else "No"
