...
from coldfront.core.allocation.forms_.secure_dir_forms import (
    SecureDirRDMConsultationReviewForm,
...
from coldfront.core.allocation.utils_.secure_dir_utils.new_directory import (
    SecureDirRequestApprovalRunner,
    SecureDirRequestDenialRunner,
...
class SecureDirRequestListView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
...
class SecureDirRequestDetailView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, DetailView
):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["unsigned_download_url"] = reverse(
            "secure-dir-request-download-unsigned-mou",
            kwargs={"pk": self.request_obj.pk, "request_type": "secure-dir"},
        )
        context["signed_download_url"] = reverse(
            "secure-dir-request-download-mou",
            kwargs={"pk": self.request_obj.pk, "request_type": "secure-dir"},
        )
        context["signed_upload_url"] = reverse(
            "secure-dir-request-upload-mou",
            kwargs={"pk": self.request_obj.pk, "request_type": "secure-dir"},
        )
    ...
    def get_checklist(self):
        ...
        checklist.append(
            [
                ...
                notified["timestamp"],
                True,
                reverse("secure-dir-request-notify-pi", kwargs={"pk": pk}),
            ]
        )
        ...
        checklist.append(
            [
                ...
                mou["timestamp"],
                is_notified,
                reverse("secure-dir-request-review-mou", kwargs={"pk": pk}),
            ]
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if sec_dir_allocations.filter(project=self.request_obj.project).exists():
            message = (
                f"The project {self.request_obj.project.name} already "
    ...
    def get_setup_status(self):
        ...
        state = self.request_obj.state
        if (
            state["rdm_consultation"]["status"] == "Denied"
            or state["mou"]["status"] == "Denied"
        ):
            return "N/A"
    ...
    def is_checklist_complete(self):
        ...
        return (
            ...
            and self.request_obj.state["setup"]["status"] == "Completed"
        )


class SecureDirRequestReviewRDMConsultView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, FormView
):
    form_class = SecureDirRDMConsultationReviewForm
    template_name = "secure_dir/secure_dir_request/secure_dir_consult_rdm.html"
    ...
    def get_success_url(self):
        return reverse(
            "secure-dir-request-detail", kwargs={"pk": self.kwargs.get("pk")}
        )


class SecureDirRequestReviewMOUView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, FormView
):
    form_class = ReviewStatusForm
    template_name = "secure_dir/secure_dir_request/secure_dir_mou.html"
    ...
    def _conditionally_send_ready_for_processing_email(self):
        ...
        if self.request_obj.status.name != "Approved - Processing":
            return
        try:
            send_secure_directory_request_ready_for_processing_email(self.request_obj)
        except Exception as e:
            logger.exception(f"Failed to send notification email. Details:\n{e}")


class SecureDirRequestReviewSetupView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, FormView
):
    form_class = SecureDirSetupForm
    template_name = "secure_dir/secure_dir_request/secure_dir_setup.html"
...
class SecureDirRequestReviewDenyView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, FormView
):
    form_class = ReviewDenyForm
    template_name = "secure_dir/secure_dir_request/secure_dir_review_deny.html"
...
class SecureDirRequestUndenyRequestView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, View
):
    def test_func(self):
        """UserPassesTestMixin tests."""
        if self.request.user.is_superuser:
            return True
        message = "You do not have permission to undeny a secure directory request."
        messages.error(self.request, message)
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        disallowed_status_names = list(
            SecureDirRequestStatusChoice.objects.filter(~Q(name="Denied")).values_list(
                "name", flat=True
            )
        )
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            reverse("secure-dir-request-detail", kwargs={"pk": kwargs.get("pk")})
        )


class SecureDirRequestEditDepartmentView(
    LoginRequiredMixin, UserPassesTestMixin, SecureDirRequestMixin, FormView
):
    template_name = (
        "secure_dir/secure_dir_request/secure_dir_request_edit_department.html"
    )
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, message)
        return HttpResponseRedirect(
            reverse("secure-dir-request-detail", kwargs={"pk": self.request_obj.pk})
        )
...
class SecureDirRequestNotifyPIView(
    MOURequestNotifyPIViewMixIn, SecureDirRequestEditDepartmentView
):
    def email_pi(self):
        super()._email_pi(
            "Secure Directory Request Ready To Be Signed",
