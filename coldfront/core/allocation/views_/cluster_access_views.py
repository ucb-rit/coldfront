...
class AllocationRequestClusterAccountView(
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not user_obj.userprofile.access_agreement_signed_date:
            message = f"User {user_obj.username} has not signed the access agreement."
            messages.error(self.request, message)
            return redirect
...
class AllocationClusterAccountRequestListView(
    ...
    def get_queryset(self):
        ...
        if cluster_search_form.is_valid():
            ...
            if data.get("username"):
                cluster_access_request_list = cluster_access_request_list.filter(
                    allocation_user__user__username__icontains=data.get("username")
                )

            if data.get("email"):
                cluster_access_request_list = cluster_access_request_list.filter(
                    allocation_user__user__email__icontains=data.get("email")
                )
    ...
    def test_func(self):
        ...
        message = "You do not have permission to review cluster account requests."
        messages.error(self.request, message)
    ...
    def get_context_data(self, **kwargs):
        ...
        if flag_enabled("LRC_ONLY"):
            for cluster_access_request in cluster_access_requests:
                cluster_access_request.host_user_lbl_email = host_user_lbl_email(
...
class AllocationClusterAccountUpdateStatusView(
    LoginRequiredMixin, UserPassesTestMixin, FormView
):
    form_class = AllocationClusterAccountUpdateStatusForm
    template_name = "allocation/allocation_update_cluster_account_status.html"
    ...
    def test_func(self):
        ...
        message = "You do not have permission to modify a cluster access request."
        messages.error(self.request, message)
...
class AllocationClusterAccountActivateRequestView(
    LoginRequiredMixin, UserPassesTestMixin, FormView
):
    form_class = AllocationClusterAccountRequestActivationForm
    template_name = "allocation/allocation_activate_cluster_account_request.html"
    ...
    def test_func(self):
        ...
        message = "You do not have permission to activate a cluster access request."
        messages.error(self.request, message)
    ...
    def form_valid(self, form):
        ...
        try:
            with transaction.atomic():
                ...
                self.request_obj.save()
                runner = ClusterAccessRequestCompleteRunner(self.request_obj)
...
class AllocationClusterAccountDenyRequestView(
    LoginRequiredMixin, UserPassesTestMixin, View
):
    def test_func(self):
        """UserPassesTestMixin tests."""
        ...
        message = "You do not have permission to deny a cluster access request."
        messages.error(self.request, message)
    ...
    def post(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(reverse("allocation-cluster-account-request-list"))
