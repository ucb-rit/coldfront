...
def send_expiry_emails():
    ...
    for days_remaining in sorted(set(EMAIL_ALLOCATION_EXPIRING_NOTIFICATION_DAYS)):
        ...
        for allocation_obj in Allocation.objects.filter(
            status__name="Active", end_date=expring_in_days
        ):
            ...
            for allocation_user in allocation_obj.project.projectuser_set.all():
                if (
                    allocation_user.enable_notifications
                    and allocation_obj.allocationuser_set.filter(
            ...
            logger.info(
                "Allocation to {} expiring in {} days email sent to {}.".format(
    ...
    for allocation_attribute in AllocationAttribute.objects.filter(
        value=today, allocation_attribute_type__name="send_expiry_email_on_date"
    ):
    ...
    for allocation_obj in Allocation.objects.filter(end_date=expring_in_days):
        ...
        project_url = "{}/{}/{}/".format(
            CENTER_BASE_URL.strip("/"), "project", allocation_obj.project.pk
        )
        ...
        logger.info(
            "Allocation to {} expired email sent to {}.".format(
