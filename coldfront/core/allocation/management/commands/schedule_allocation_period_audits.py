...
class Command(BaseCommand):
    ...
    def _handle_synchronous(self, emails):
        ...
        allocation_period_names = self._get_allocation_period_names_to_audit(year)
    ...
    def _next_instance_of_date(month, day):
        ...
        date_this_year = display_time_zone_date_to_utc_datetime(date(year, month, day))
        if now >= date_this_year:
            return display_time_zone_date_to_utc_datetime(date(year + 1, month, day))
    ...
    def _send_audit_exception_email(allocation_period_name, task, emails):
        ...
        subject = f'AllocationPeriod "{allocation_period_name}" Audit: Exception'
...
def audit_command_task_hook(task):
    ...
    task_next_run = utc_now_offset_aware() + timedelta(
        days=RERUN_FAILED_AUDIT_DELAY_DAYS
    )
    ...
    if not task.success:
        ...
        if len(task.args) > 1 and task.args[1] == "--email":
            ...
            #  optional arguments.
            emails = task.args[2:]
            Command._send_audit_exception_email(allocation_period_name, task, emails)
    else:
