...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            raise CommandError(f"Invalid AllocationPeriod {allocation_period_name}.")
    ...
    def parse_input_file(self, file_path, allocation_period):
        ...
        for input_dict in input_dicts:
            ...
            if isinstance(pi, User) and has_non_denied_renewal_request(
            ...
            try:
                post_project = Project.objects.prefetch_related("projectuser_set").get(
    ...
    def process_already_renewed_pis(self, already_renewed, allocation_period):
        ...
        for pi_user in already_renewed:
            pi_str = f"{pi_user.first_name} {pi_user.last_name} ({pi_user.email})"
            message = (
    ...
    def process_valid_objects(self, valid, allocation_period, process, dry_run):
        ...
        approved_renewal_status = AllocationRenewalRequestStatusChoice.objects.get(
            name="Approved"
        )
        complete_renewal_status = AllocationRenewalRequestStatusChoice.objects.get(
            name="Complete"
        )
        for valid_dict in valid:
            ...
            pi_str = f"{pi_name_parts['first']} {pi_name_parts['last']} ({pi_email})"
            ...
            try:
                with transaction.atomic():
                    ...
                    requester_user = self._update_or_create_user_and_email_address(
                        requester_email
                    )
                    requester_str = (
                    ...
                    pi_user = self._update_or_create_user_and_email_address(
                        pi_email,
                        first_name=pi_name_parts["first"],
                        middle_name=pi_name_parts["middle"],
                        last_name=pi_name_parts["last"],
                        set_is_pi=True,
                    )
                    pi_str = (
                    ...
                    request = AllocationRenewalRequest.objects.create(**request_kwargs)
    ...
    def _update_or_create_user_and_email_address(
        self, email, first_name="", middle_name="", last_name="", set_is_pi=False
    ):
