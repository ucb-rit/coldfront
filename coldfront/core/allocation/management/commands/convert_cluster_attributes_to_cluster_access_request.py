...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        attribute_value_to_request_status = {
            ...
            "Active": ClusterAccessRequestStatusChoice.objects.get(name="Complete"),
            "Denied": ClusterAccessRequestStatusChoice.objects.get(name="Denied"),
        }

        allocation_user_attributes = (
            AllocationUserAttribute.objects.prefetch_related("history")
            .filter(
        ...
        for attribute in allocation_user_attributes.iterator():
            ...
            for historical_attribute in sorted_historical_attributes:
                ...
                request_status = attribute_value_to_request_status[attribute_value]
                created = False
                if attribute_value == "Pending - Add":
                    ...
                    if not latest_pending_add_dt:
                        continue
                    if latest_active_dt and latest_active_dt > latest_pending_add_dt:
                        continue
                    _, created = self.update_or_create_request(
                        historical_attribute,
                        request_status,
                ...
                num_created_by_request_status_name[request_status.name] += int(created)
            ...
            if latest_historical_attribute.value in ("Pending - Add", "Processing"):
                request_status = attribute_value_to_request_status[
                    latest_historical_attribute.value
                ]
                _, created = self.update_or_create_request(
                    latest_historical_attribute,
                    request_status,
        ...
        for (
            request_status_name,
            num_created,
        ) in num_created_by_request_status_name.items():
            message = (
    ...
    @staticmethod
    def update_or_create_request(
        historical_allocation_user_attribute, status, request_time, completion_time=None
    ):
