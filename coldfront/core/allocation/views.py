...
class AllocationDetailView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_superuser:
            attributes_with_usage = [
                attribute
                for attribute in allocation_obj.allocationattribute_set.all().order_by(
            ...
            attributes_with_usage = [
                attribute
                for attribute in allocation_obj.allocationattribute_set.filter(
        ...
        elif allocation_obj.project.projectuser_set.filter(
            user=self.request.user
        ).exists():
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if not end_date:
                end_date = datetime.datetime.now() + relativedelta(
            ...
            if old_status != "Active" and new_status == "Active":
                ...
                if EMAIL_ENABLED:
                    ...
                    for allocation_user in allocation_obj.allocationuser_set.exclude(
    ...
    def _update_context(self, allocation_obj, attributes, context):
        ...
        context["allocation_users"] = allocation_users.exclude(status__name="Removed")
    ...
    def _add_compute_specific_context(self, allocation_obj, allocation_users, context):
        ...
        for allocation_user in allocation_users:
            attributes = allocation_user.allocationuserattribute_set.select_related(
            ...
            except (AllocationUserAttribute.DoesNotExist, AttributeError, ValueError):
                usage = "0.00"
        ...
        if is_lrc:
            self._add_lrc_billing_id_context(allocation_obj, allocation_users, context)

    def _add_lrc_billing_id_context(self, allocation_obj, allocation_users, context):
        ...
        if context["user_account_fee_billing_ids_visible"]:
            ...
            context["user_account_fee_billing_ids"] = user_account_fee_billing_ids
        ...
        allowance_resource = computing_allowance_interface.allowance_from_project(
            ...
            for allocation_user in allocation_users:
                ...
                try:
                    ...
                    billing_id = BillingActivity.objects.get(
                        pk=int(billing_attribute.value)
                    ).full_id()
                except (
                    AllocationUserAttribute.DoesNotExist,
                    AttributeError,
                    BillingActivity.DoesNotExist,
                    ValueError,
                ):
                    billing_id = "N/A"
                recharge_fee_billing_ids[allocation_user.user.username] = billing_id
    ...
    def _add_secure_dir_specific_context(self, allocation_obj, context):
        ...
        context["add_remove_users_buttons_visible"] = add_remove_users_buttons_visible

    @staticmethod
    def _is_compute_allocation(allocation_obj):
        """Return whether the Allocation represents access to compute
        resources."""
        return allocation_obj.resources.filter(name__endswith=" Compute").exists()
    ...
    def _is_secure_dir_allocation(allocation_obj):
        ...
        return allocation_obj.resources.filter(
            name__endswith=" P2/P3 Directory"
        ).exists()
    ...
    def get_context_data(self, **kwargs):
        ...
        for resource in user_resources:
            if resource.resourceattribute_set.filter(
                resource_attribute_type__name="quantity_default_value"
            ).exists():
...
class AllocationAddUsersView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if allocation_obj.is_locked and not self.request.user.is_superuser:
            messages.error(
                request,
                "You cannot modify this allocation because it is locked! Contact support for details.",
            )
            return HttpResponseRedirect(
                reverse("allocation-detail", kwargs={"pk": allocation_obj.pk})
            )
...
class AllocationRemoveUsersView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_users_to_remove(self, allocation_obj):
        ...
        exclude_pks = list(allocation_obj.project.pis().values_list("pk", flat=True))
        ...
        if formset.is_valid():
            ...
            for form in formset:
                ...
                if user_form_data["selected"]:
                    ...
                    user_obj = User.objects.get(username=user_form_data.get("username"))
                    if allocation_obj.project.pis().filter(pk=user_obj.pk).exists():
                        continue
...
class AllocationActivateRequestView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, pk):
        ...
        domain_url = get_domain_url(self.request)
        allocation_url = "{}{}".format(
            domain_url, reverse("allocation-detail", kwargs={"pk": allocation_obj.pk})
        )
...
class AllocationDenyRequestView(LoginRequiredMixin, UserPassesTestMixin, View):
    ...
    def get(self, request, pk):
        ...
        allocation_status_denied_obj = AllocationStatusChoice.objects.get(name="Denied")
