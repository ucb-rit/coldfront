...
class TestClusterAccessRunnersBase(TestBase):
    ...
    def setUp(self):
        ...
        self.project0 = Project.objects.create(name="project0", status=project_status)
    ...
    def _get_cluster_account_status_attr(self):
        ...
        cluster_access_attribute = AllocationUserAttribute.objects.filter(
...
class TestClusterAccessRequestCompleteRunner(TestClusterAccessRunnersBase):
    ...
    def test_service_units_attribute_created(self):
        ...
        self.assertTrue(pre_time <= service_units_attribute.created <= post_time)
    ...
    def test_service_units_attribute_not_created(self):
        ...
        AllocationAttribute.objects.filter(
            allocation_attribute_type__name="Service Units",
            allocation=self.alloc_user_obj.allocation,
        ).delete()
    ...
    def test_exception_inside_transaction_rollback(self):
        ...
        with patch.object(
            ClusterAccessRequestCompleteRunner, "_set_cluster_uid", raise_exception
        ):
    ...
    def test_exception_outside_transaction_no_rollback(self):
        ...
        with patch.object(
            ClusterAccessRequestCompleteRunner, "_send_emails_safe", raise_exception
        ):
            with self.assertLogs(self._module, "INFO") as log_cm:
...
class TestClusterAccessRequestDenialRunner(TestClusterAccessRunnersBase):
    ...
    def _assert_emails_sent(self):
        email_body = [
            f"access request under project {self.project0.name}",
            f"and allocation {self.alloc_obj.allocation.pk} has been denied.",
        ]
    ...
    def _assert_post_state(self):
        ...
        self.assertTrue(self._get_cluster_account_status_attr().exists())
        self.assertEqual(
            self._get_cluster_account_status_attr().first().value, "Denied"
        )
    ...
    def test_email_failure_no_rollback(self):
        ...
        with patch.object(
            ClusterAccessRequestDenialRunner, "_send_denial_emails", raise_exception
        ):
            with self.assertLogs(self._module, "INFO") as log_cm:
                self.runner.run()
    ...
    def test_exception_outside_transaction_no_rollback(self):
        ...
        with patch.object(
            ClusterAccessRequestDenialRunner, "_send_emails_safe", raise_exception
        ):
            with self.assertLogs(self._module, "INFO") as log_cm:
                with self.assertRaises(Exception) as exc_cm:
                    self.runner.run()
...
class TestClusterAccessRequestRunner(TestBase):
    ...
    def setUp(self):
        ...
        # ProjectUser.
        self.project = self.create_active_project_with_pi("fc_project", self.user)
    ...
    def _assert_post_state(self):
        """Assert that the relevant objects have the expected state,
        assuming that the runner has run successfully."""
        pending_add_allocation_user_attributes = AllocationUserAttribute.objects.filter(
        ...
        cluster_access_requests = ClusterAccessRequest.objects.filter(
            allocation_user__allocation=self.allocation, allocation_user__user=self.user
        )
    ...
    def test_asserts_allocation_user_active(self):
        ...
        other_statuses = AllocationUserStatusChoice.objects.exclude(pk=active_status.pk)
    ...
    def test_failure(self):
        ...
        with patch.object(
            ClusterAccessRequestRunner,
            "_create_cluster_access_request",
            raise_exception,
        ):
            runner = ClusterAccessRequestRunner(
                self.allocation_user, email_strategy=email_strategy
    ...
    def test_validates_no_existing_cluster_access(self):
        ...
        for invalid_value in ("Pending - Add", "Processing", "Active"):
            ...
            with self.assertRaises(expected_exception) as cm:
                runner.run()
            self.assertIn("already has pending or active access", str(cm.exception))
