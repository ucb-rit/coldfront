...
class TestCreateSecureDir(TestBase):
    ...
    def test_allocation_objects_created(self):
        ...
        scratch_p2p3_path = scratch_p2p3_directory.resourceattribute_set.get(
            resource_attribute_type__name="path"
        )
        ...
        groups_p2p3_subdirectory = AllocationAttribute.objects.filter(
            ...
            value=os.path.join(groups_p2p3_path.value, self.subdirectory_name),
        )

        scratch_p2p3_subdirectory = AllocationAttribute.objects.filter(
            allocation_attribute_type=allocation_attribute_type,
            allocation=scratch_allocation,
            value=os.path.join(scratch_p2p3_path.value, self.subdirectory_name),
        )
...
class TestGetSecureDirManageUserRequestObjects(TestBase):
    ...
    def test_action_add(self):
        ...
        self.assertEqual(
            dummy_object.request_status_obj, SecureDirAddUserRequestStatusChoice
        )
    ...
    def test_action_remove(self):
        ...
        self.assertEqual(
            dummy_object.request_status_obj, SecureDirRemoveUserRequestStatusChoice
        )
