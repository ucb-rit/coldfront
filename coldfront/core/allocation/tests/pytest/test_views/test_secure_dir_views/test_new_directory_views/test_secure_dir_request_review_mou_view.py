...
def superuser(db):
    ...
    user = User.objects.create_superuser(username="admin", email="admin@test.com")
...
def pi(db):
    ...
    pi_user = User.objects.create_user(username="pi", email="pi@test.com")
...
def project(db):
    ...
    return Project.objects.create(name="test_project", status=project_status)
...
def _create_secure_dir_request(
    db, project, requester, pi, rdm_consultation_status="Pending"
):
...
class TestSecureDirRequestReviewMOUViewEmailSending:
    ...
    def secure_dir_request_for_mou(self):
        ...
        return _create_secure_dir_request(
            None, self.project, self.pi, self.pi, rdm_consultation_status="Approved"
        )

    def test_email_sent_when_mou_approved(self, client, secure_dir_request_for_mou):
        ...
        with patch(
            ...
            data = {"status": "Approved", "justification": "Test justification"}
            ...
            assert secure_dir_request_for_mou.status.name == "Approved - Processing"

            # Verify email sent
            mock_send.assert_called_once_with(secure_dir_request_for_mou)
    ...
    def test_email_not_sent_if_rdm_pending(
        ...
        request_obj = _create_secure_dir_request(
            None, project_with_pi, pi, pi, rdm_consultation_status="Pending"
        )
        ...
        with patch(
            ...
            url = reverse(
                "secure-dir-request-review-mou", kwargs={"pk": request_obj.pk}
            )
            data = {"status": "Approved", "justification": "Test justification"}
            response = client.post(url, data)
    ...
    def test_email_not_sent_if_denied(self, client, superuser, pi, project_with_pi):
        """Email not sent if MOU is denied."""
        # Create request with RDM consultation approved
        request_obj = _create_secure_dir_request(
            None, project_with_pi, pi, pi, rdm_consultation_status="Approved"
        )
        ...
        with patch(
            ...
            data = {"status": "Denied", "justification": "Test denial"}
            response = client.post(url, data)
    ...
    def test_permission_required(self, client, pi, secure_dir_request_for_mou):
...
class TestSecureDirRequestMixinEmailMethod:
    ...
    def test_sends_email_if_status_is_approved_processing(self, mock_send):
        """Email sent when status is Approved - Processing."""
        from coldfront.core.allocation.views_.secure_dir_views.new_directory.approval_views import (
            SecureDirRequestReviewMOUView,
        )
    ...
    def test_handles_email_send_exception_gracefully(self, mock_send, mock_logger):
    ...
    @pytest.mark.parametrize(
        "status_name", ["Under Review", "Approved - Complete", "Denied"]
    )
    @patch(
        "coldfront.core.allocation.views_."
        "secure_dir_views.new_directory.approval_views."
