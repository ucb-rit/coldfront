...
class TestSecureDirBase(TestBase):
    ...
    def setUp(self):
        ...
        for i in range(2):
            ...
            project = Project.objects.create(name=f"project{i}", status=project_status)
        ...
        pi.role = ProjectUserRoleChoice.objects.get(name="Principal Investigator")
        ...
        self.groups_allocation = create_secure_directory(
            self.project1, self.subdirectory_name, "groups"
        )
        self.scratch_allocation = create_secure_directory(
            self.project1, self.subdirectory_name, "scratch"
        )
        ...
        self.groups_path = self.groups_allocation.allocationattribute_set.get(
            allocation_attribute_type__name__icontains="Directory"
        ).value

        self.scratch_path = self.scratch_allocation.allocationattribute_set.get(
            allocation_attribute_type__name__icontains="Directory"
        ).value
...
class TestSecureDirManageUsersView(TestSecureDirBase):
    ...
    def test_correct_users_to_add(self):
        ...
        temp_project = Project.objects.create(
            name="temp_project", status=ProjectStatusChoice.objects.get(name="Active")
        )
        ProjectUser.objects.create(
        ...
        for i in range(2, 6):
            temp_user = User.objects.create(
                username=f"user{i}", email=f"email{i}@email.com"
            )
        ...
        SecureDirAddUserRequest.objects.create(
            ...
            status=SecureDirAddUserRequestStatusChoice.objects.get(name="Pending"),
        )
        ...
        SecureDirRemoveUserRequest.objects.create(
            user=self.user2,
            allocation=self.groups_allocation,
            status=SecureDirRemoveUserRequestStatusChoice.objects.get(name="Pending"),
        )
        ...
        SecureDirRemoveUserRequest.objects.create(
            user=self.user3,
            allocation=self.groups_allocation,
            status=SecureDirRemoveUserRequestStatusChoice.objects.get(name="Complete"),
        )
    ...
    def test_correct_users_to_remove(self):
        ...
        for i in range(2, 5):
            ...
            AllocationUser.objects.create(
                allocation=self.groups_allocation, user=temp_user, status=active_status
            )
            setattr(self, f"user{i}", temp_user)
    ...
    def test_add_users(self):
        ...
        kwargs = {"pk": self.scratch_allocation.pk, "action": "add"}
        response = self.post_response(self.pi, self.url, kwargs=kwargs, data=form_data)
        ...
        self.assertRedirects(
            response,
            reverse("allocation-detail", kwargs={"pk": self.scratch_allocation.pk}),
        )
    ...
    def test_remove_users(self):
        ...
        self.assertRedirects(
            response,
            reverse("allocation-detail", kwargs={"pk": self.groups_allocation.pk}),
        )
    ...
    def test_content(self):
        ...
        response = self.get_response(self.pi, self.url, kwargs=kwargs)
        html = response.content.decode("utf-8")
        self.assertIn(f"Add users to: {self.groups_path}", html)
        ...
        self.assertIn(f"Remove users from: {self.scratch_path}", html)
...
class TestSecureDirManageUsersRequestListView(TestSecureDirBase):
    ...
    def test_no_requests(self):
        ...
        for status in ["pending", "completed"]:
            for action in ["add", "remove"]:
                ...
                if status == "completed":
                    message = f"No completed secure directory {action} user requests!"
                else:
                    message = (
                        f"No new or pending secure directory {action} user requests!"
                    )
    ...
    def test_correct_add_user_requests_shown(self):
        ...
        SecureDirAddUserRequest.objects.create(
            user=self.user1,
            allocation=self.groups_allocation,
            status=SecureDirAddUserRequestStatusChoice.objects.get(name="Complete"),
            directory=self.groups_path,
        )
        ...
        SecureDirAddUserRequest.objects.create(
            user=self.user2,
            allocation=self.scratch_allocation,
            status=SecureDirAddUserRequestStatusChoice.objects.get(name="Denied"),
    ...
    def test_correct_remove_user_requests_shown(self):
        ...
        SecureDirRemoveUserRequest.objects.create(
            user=self.user2,
            allocation=self.scratch_allocation,
            status=SecureDirRemoveUserRequestStatusChoice.objects.get(name="Denied"),
            directory=self.scratch_path,
        )
...
class TestSecureDirManageUsersDenyRequestView(TestSecureDirBase):
    ...
    def test_access(self):
        ...
        for action, request in [
            ...
            status_code_test(self.post_response(self.pi, self.url, kwargs, data), False)
    ...
    def test_deny_add_request(self):
        ...
        manager_role = ProjectUserRoleChoice.objects.get(name="Manager")
        project_user1 = ProjectUser.objects.get(project=self.project1, user=self.user1)
        ...
        self.assertTrue(
            pre_time <= self.add_request.completion_time <= utc_now_offset_aware()
        )

        # Test that the correct emails are sent
        email_body = [
            f"The request to add first0 last0 (user0) to "
    ...
    def test_deny_removal_request(self):
        ...
        # Add two managers to the project. Add one of them to the directory.
        manager_role = ProjectUserRoleChoice.objects.get(name="Manager")
        project_user0 = ProjectUser.objects.get(project=self.project1, user=self.user0)
        ...
        data = {"reason": "This is a test for denying SecureDirRemoveUserRequest."}
        pre_time = utc_now_offset_aware()
        ...
        self.assertTrue(
            pre_time <= self.remove_request.completion_time <= utc_now_offset_aware()
        )

        # Test that the correct emails are sent
        email_body = [
            f"The request to remove first1 last1 (user1) from "
    ...
    def test_deny_removal_request_bad_status(self):
        ...
        self.remove_request.status = SecureDirRemoveUserRequestStatusChoice.objects.get(
            name="Complete"
        )
...
class TestSecureDirManageUsersUpdateStatusView(TestSecureDirBase):
    ...
    def test_status_updated(self):
        ...
        for request, action in [
            ...
            response = self.post_response(
                self.admin, self.url, kwargs=kwargs, data=data
            )
            ...
            self.assertRedirects(
                ...
                reverse(
                    "secure-dir-manage-users-request-list",
                    kwargs={"action": action, "status": "pending"},
                ),
            )
    ...
    def test_bad_request_status(self):
        ...
        self.add_request.status = SecureDirAddUserRequestStatusChoice.objects.get(
            name="Denied"
        )
...
class TestSecureDirManageUsersCompleteStatusView(TestSecureDirBase):
    ...
    def setUp(self):
        ...
        self.add_request = SecureDirAddUserRequest.objects.create(
            user=self.user0,
            allocation=self.groups_allocation,
            status=SecureDirAddUserRequestStatusChoice.objects.get(name="Processing"),
            directory=self.groups_path,
        )
    ...
    def test_add_request_status_updated(self):
        ...
        # Test that the correct emails are sent.
        email_body = [
            f"The request to add first0 last0 (user0) to the secure "
    ...
    def test_remove_request_status_updated(self):
        ...
        # Test that the correct emails are sent.
        email_body = [
            f"The request to remove first1 last1 (user1) from the "
