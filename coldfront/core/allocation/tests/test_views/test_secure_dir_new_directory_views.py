...
class TestSecureDirRequestBase(TestBase):
    ...
    def setUp(self):
        ...
        # Create two PIs.
        for i in range(2):
            pi = User.objects.create(username=f"pi{i}", email=f"pi{i}@nonexistent.com")
...
class TestSecureDirRequestWizard(TestSecureDirRequestBase):
    ...
    def test_post_creates_request(self):
        ...
        pi_project_user_pk = ProjectUser.objects.get(
            project=self.project1, user=self.pi0
        ).pk
        ...
        self.assertEqual(request.data_description, form_data[1]["1-data_description"])
        self.assertEqual(request.rdm_consultation, form_data[2]["2-rdm_consultants"])
        self.assertEqual(
            request.directory_name, f"pl1_{form_data[3]['3-directory_name']}"
        )
...
class TestSecureDirRequestListView(TestSecureDirRequestBase):
    ...
    def test_pending_requests(self):
        ...
        self.request1.status = SecureDirRequestStatusChoice.objects.get(
        ...
        self.assertContains(response, format_badge.format(self.request0.status.name))
        self.assertContains(response, self.project1.name)
        self.assertContains(response, format_badge.format(self.request1.status.name))
    ...
    def test_no_pending_requests(self):
        ...
        for request in SecureDirRequest.objects.all():
            request.status = SecureDirRequestStatusChoice.objects.get(name="Denied")
            request.save()
    ...
    def test_completed_requests(self):
        ...
        self.request0.status = SecureDirRequestStatusChoice.objects.get(
            name="Approved - Complete"
        )
        self.request0.save()
        self.request1.status = SecureDirRequestStatusChoice.objects.get(name="Denied")
        self.request1.save()
        ...
        self.assertContains(response, self.project0.name)
        self.assertContains(
            response, '<span class="badge badge-success">Approved - Complete</span>'
        )
        self.assertContains(response, self.project1.name)
        self.assertContains(response, '<span class="badge badge-danger">Denied</span>')
    ...
    def test_no_completed_requests(self):
        ...
        completed requests."""
        for request in SecureDirRequest.objects.all():
            request.status = SecureDirRequestStatusChoice.objects.get(
                name="Under Review"
            )
            request.save()
...
class TestSecureDirRequestDetailView(TestSecureDirRequestBase):
    ...
    def setUp(self):
        ...
        self.url0 = reverse(
            "secure-dir-request-detail", kwargs={"pk": self.request0.pk}
        )
    ...
    def test_post_request_approves_request(self):
        ...
        self.assertTrue(
            pre_time < self.request0.completion_time < utc_now_offset_aware()
        )
    ...
    def test_post_request_creates_allocations(self):
        ...
        groups_alloc = Allocation.objects.filter(
            project=self.project0, resources__in=[groups_p2p3_directory]
        )
        ...
        scratch_alloc = Allocation.objects.filter(
            project=self.project0, resources__in=[scratch_p2p3_directory]
        )
    ...
    def test_post_request_emails_sent(self):
        ...
        email_body = [
            f"Your request for a secure directory for project "
...
class TestSecureDirRequestUndenyRequestView(TestSecureDirRequestBase):
    ...
    def setUp(self):
        ...
        self.url = reverse("secure-dir-request-undeny", kwargs={"pk": self.request0.pk})
...
class TestSecureDirRequestReviewDenyView(TestSecureDirRequestBase):
    ...
    def setUp(self):
        ...
        self.request0.state["rdm_consultation"]["status"] = "Approved"
        self.request0.state["mou"]["status"] = "Approved"
        self.request0.state["other"]["justification"] = "This is a test justification."
        self.request0.state["other"]["timestamp"] = utc_now_offset_aware().isoformat()
        self.request0.save()

        self.url = reverse(
            "secure-dir-request-review-deny", kwargs={"pk": self.request0.pk}
        )
        self.success_url = reverse(
            "secure-dir-request-detail", kwargs={"pk": self.request0.pk}
        )
    ...
    def test_post_request_denies_request(self):
        ...
        self.assertEqual(
            self.request0.state["other"]["justification"], data["justification"]
        )
...
class TestSecureDirRequestReviewRDMConsultView(TestSecureDirRequestBase):
    ...
    def setUp(self):
        ...
        self.url = reverse(
            "secure-dir-request-review-rdm-consultation",
    ...
    def test_post_updates_request(self):
        ...
        self.assertTrue(pre_time < timestamp < utc_now_offset_aware())
        self.assertEqual(
            self.request0.state["rdm_consultation"]["justification"],
            data["justification"],
        )
...
class TestSecureDirRequestReviewMOUView(TestSecureDirRequestBase):
    ...
    def setUp(self):
        ...
        self.url = reverse(
            "secure-dir-request-review-mou", kwargs={"pk": self.request0.pk}
        )
        self.success_url = reverse(
            "secure-dir-request-detail", kwargs={"pk": self.request0.pk}
        )
    ...
    def test_post_updates_request(self):
        ...
        self.assertTrue(pre_time < timestamp < utc_now_offset_aware())
        self.assertEqual(
            self.request0.state["mou"]["justification"], data["justification"]
        )
...
class TestSecureDirRequestReviewSetupView(TestSecureDirRequestBase):
    ...
    def setUp(self):
        ...
        self.request0.state["mou"]["status"] = "Approved"
        self.request0.state["mou"]["timestamp"] = utc_now_offset_aware().isoformat()
        self.request0.save()

        self.url = reverse(
            "secure-dir-request-review-setup", kwargs={"pk": self.request0.pk}
        )
        self.success_url = reverse(
            "secure-dir-request-detail", kwargs={"pk": self.request0.pk}
        )
    ...
    def test_post_updates_request(self):
        ...
        self.request0.refresh_from_db()
        self.assertRedirects(response, self.success_url)
        self.assertEqual(self.request0.directory_name, f"pl1_{data['directory_name']}")
        self.assertEqual(self.request0.status.name, "Approved - Processing")
        ...
        self.assertTrue(pre_time < timestamp < utc_now_offset_aware())
        self.assertEqual(
            self.request0.state["setup"]["justification"], data["justification"]
        )
