...
class TestApproveRenewalRequestsForAllocationPeriod(TestBase):
    ...
    def setUp(self):
        ...
        computing_allowance = Resource.objects.get(name=self.computing_allowance_name)
    ...
    def call_command(allocation_period_id, dry_run=False):
        ...
        args = ["approve_renewal_requests_for_allocation_period", allocation_period_id]
        if dry_run:
            args.append("--dry_run")
    ...
    def create_request_and_supporting_objects(self, allocation_period):
        ...
        pi = User.objects.create(
            email="pi@email.com", first_name="PI", last_name="User", username="pi"
        )
    ...
    def test_allocation_period_not_allowance_year(self):
        ...
        if flag_enabled("LRC_ONLY"):
            ...
            return
        _id = (
            AllocationPeriod.objects.exclude(name__startswith="Allowance Year")
            .first()
            .id
        )
        with self.assertRaises(CommandError) as cm:
            self.call_command(_id)
        self.assertIn("does not represent an allowance year", str(cm.exception))
    ...
    def test_allocation_period_already_started(self):
        ...
        _id = AllocationPeriod.objects.filter(start_date__lt=current_date).first().id
        with self.assertRaises(CommandError) as cm:
            self.call_command(_id)
    ...
    def test_requests_limited_by_conditions(self):
        ...
        request, _, _, _ = self.create_request_and_supporting_objects(allocation_period)
        ...
        for allowance_name in allowance_names:
            prefix = computing_allowance_interface.code_from_name(allowance_name)
            project.name = prefix + project.name[3:]
            project.save()
            request.computing_allowance = Resource.objects.get(name=allowance_name)
            request.save()
