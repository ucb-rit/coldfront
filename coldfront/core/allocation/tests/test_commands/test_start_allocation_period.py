...
from coldfront.core.allocation.management.commands.start_allocation_period import (
    Command as StartAllocationPeriodCommand,
)
...
from coldfront.core.project.utils_.renewal_utils import (
    ...
    get_previous_allowance_year_period,
)
...
class TestStartAllocationPeriod(TestBase):
    ...
    def setUp(self):
        ...
        TimedResourceAttribute.objects.create(
            resource=Resource.objects.get(name="Instructional Computing Allowance"),
        ...
        for allowance_name in (BRCAllowances.FCA, BRCAllowances.ICA, BRCAllowances.PCA):
        ...
        for allowance_name, allowance in computing_allowances.items():
            project_name_prefix = self.computing_allowance_interface.code_from_name(
                allowance_name
            )
        ...
        AllocationRenewalRequest.objects.create(
            requester=User.objects.get(username=f"{fc_existing.name}_requester"),
    ...
    def assert_existing_project_pre_state(self, allocation_period, project):
        ...
        self.assertEqual(Decimal(allocation_attribute.value), pre_allocation_allowance)
    ...
    def assert_multiple_last_k_historical_values(self, data, pre_t, post_t):
        ...
        for obj, attr, pairs in data:
            self.assert_last_k_historical_values(obj, attr, pairs, pre_t, post_t)
    ...
    def assert_new_project_request_to_process(
        self, expected_num_fcas=None, expected_num_icas=None, expected_num_pcas=None
    ):
    ...
    def assert_output(
        ...
        deactivation_prefix = "DRY RUN: Would deactivate" if dry_run else "Deactivated"
        processing_prefix_template = (
            "DRY RUN: Would process {0}" if dry_run else "Processed {0}"
        )
        new_project_request_processing_prefix = processing_prefix_template.format(
        ...
        for line in output_lines:
            if line.startswith(deactivation_prefix):
                project_id, actual_project_name = (
                    self.extract_deactivation_message_entries(line, dry_run=dry_run)
                )
                project_id = int(project_id)
                expected_project_name = project_names_by_id.pop(project_id, None)
                if expected_project_name is None:
                    self.fail(
                        f"Project {project_id} is unexpectedly marked for deactivation."
                    )
                self.assertEqual(expected_project_name, actual_project_name)
            elif line.startswith(new_project_request_processing_prefix):
                request_id, num_service_units = self.extract_processing_message_entries(
                    line, SavioProjectAllocationRequest, dry_run=dry_run
                )
                request_id = int(request_id)
                num_service_units = Decimal(num_service_units)
                expected_num_service_units = num_sus_by_new_project_request_id.pop(
                    request_id, None
                )
    ...
    def assert_post_state_activated(self, project, pre_time, post_time):
        ...
        allowance_name = self.allowance_name_from_project_name(project)
        allocation_period = self.current_allocation_periods_by_allowance[allowance_name]
        ...
        self.assert_last_k_historical_values(
            project, "status", latest_project_statuses, pre_time, post_time
        )
        ...
        self.assert_last_k_historical_values(
            allocation, "status", latest_allocation_statuses, pre_time, post_time
        )

        latest_allocation_start_dates = ((self.current_date, True), (None, False))
        self.assert_last_k_historical_values(
            allocation, "start_date", latest_allocation_start_dates, pre_time, post_time
        )
        ...
        self.assert_last_k_historical_values(
            allocation, "end_date", latest_allocation_end_dates, pre_time, post_time
        )
        ...
        latest_allocation_attribute_usage_values = ((zero, True),)
        self.assert_last_k_historical_values(
    ...
    def assert_post_state_deactivated(self, project, pre_time, post_time):
        ...
        active_allocation_status = AllocationStatusChoice.objects.get(name="Active")
        expired_allocation_status = AllocationStatusChoice.objects.get(name="Expired")
        ...
        allocation_users = allocation.allocationuser_set.exclude(
            user__username__in=[f"{project.name}_requester", f"{project}_pi"]
        )
    ...
    def assert_post_state_deactivated_renewed(self, project, pre_time, post_time):
    ...
    def assert_projects_to_deactivate(
        self, expected_num_fcas=None, expected_num_icas=None, expected_num_pcas=None
    ):
    ...
    def assert_renewal_requests_to_process(
        self, expected_num_fcas=None, expected_num_icas=None, expected_num_pcas=None
    ):
    ...
    @staticmethod
    def call_command(allocation_period_id, skip_deactivations=False, dry_run=False):
    ...
    def create_project(
        ...
        pi = User.objects.create(username=f"{name}_pi", email=f"{name}_pi@email.com")
        ...
        with patch.object(AllocationPeriod, "assert_not_ended", no_op):
            with patch.object(AllocationPeriod, "assert_started", no_op):
                ...
                if process:
                    processing_runner = SavioProjectProcessingRunner(*runner_args)
    ...
    def create_project_users(project, k):
        ...
        active_allocation_status = AllocationUserStatusChoice.objects.get(name="Active")
        service_units_type = AllocationAttributeType.objects.get(name="Service Units")

        service_units_by_username = {}
        for i in range(k):
            username = f"{project.name}_user_{i}"
            user = User.objects.create(username=username, email=f"{username}@email.com")
            ProjectUser.objects.create(
                project=project, user=user, **project_user_kwargs
            )
            allocation_user = AllocationUser.objects.create(
                allocation=allocation, user=user, status=active_allocation_status
            )
            ...
            service_units_by_username[username] = (
                allocation_user_allowance,
                allocation_user_usage,
            )
    ...
    def test_failed_deactivations_preempt_processing(self):
        ...
        def assert_outcome(_id, error_message):
            ...
            path = (
                "coldfront.core.allocation.management.commands.start_allocation_period"
            )
        ...
        num_complete_renewal_requests = AllocationRenewalRequest.objects.filter(
        ...
        assert_outcome(
            ...
            ('Failed to deactivate 1/1 "Instructional Computing Allowance" Projects.'),
        )
        ...
        self.assertEqual(
            ...
            AllocationRenewalRequest.objects.filter(status__name="Complete").count(),
        )
    ...
    def test_multiple_runs_avoid_redundant_work(self):
        ...
        fc_existing = Project.objects.get(name="fc_existing")
        fc_existing_str = f"Deactivated Project {fc_existing.id} ({fc_existing.name})"
        self.assertIn(fc_existing_str, output)

        pc_existing = Project.objects.get(name="pc_existing")
        pc_existing_str = f"Deactivated Project {pc_existing.id} ({pc_existing.name})"
    ...
    def test_new_project_request_processing_eligibility(self):
        ...
        approved_scheduled_status = ProjectAllocationRequestStatusChoice.objects.get(
        ...
        fc_future = self.create_project(
            ...
            self.next_allowance_year,
            num_service_units,
            approve=True,
        )
        fc_future_request = SavioProjectAllocationRequest.objects.get(project=fc_future)
    ...
    def test_output_for_allowance_year_period(self):
        ...
        # processed.
        num_sus_by_new_project_request_id = self.assert_new_project_request_to_process(
            **kwargs
        )
        # One FCA renewal request should be processed.
        num_sus_by_renewal_request_id = self.assert_renewal_requests_to_process(
            expected_num_fcas=1
        )
        # There should be 5 entities.
        expected_num_entities = (
            len(project_names_by_id)
        ...
        for dry_run in (True, False):
            ...
            self.assert_output(
                output,
                expected_num_entities + 2 * int(not dry_run),
                deepcopy(project_names_by_id),
                deepcopy(num_sus_by_new_project_request_id),
                deepcopy(num_sus_by_renewal_request_id),
                dry_run=dry_run,
            )
    ...
    def test_project_deactivation_eligibility(self):
        ...
        def assert_message_in_command_output(included, project_name="fc_existing"):
            message_template = (
                "DRY RUN: Would deactivate Project {0} ({1}) and reset Service Units."
            )
        ...
        fc_existing = Project.objects.get(name=f"{project_name_prefix}existing")
        accounting_allocation_objects = get_accounting_allocation_objects(fc_existing)
        ...
        self.assertTrue(
            allocation.end_date is None
            or allocation.end_date < allocation_period.start_date
        )
        ...
        for offset_days in (0, 1):
            allocation.end_date = allocation_period.start_date + timedelta(
                days=offset_days
            )
            allocation.save()
        ...
        other_statuses = ProjectStatusChoice.objects.exclude(pk=active_status.pk)
    ...
    def test_skip_deactivations_flag(self):
        ...
        dry_run = False
        with patch.object(
            StartAllocationPeriodCommand, "deactivate_projects", raise_exception
        ):
    ...
    def test_starts_allowance_year_period(self):
        ...
        self.assert_post_state_deactivated_renewed(fc_existing, pre_time, post_time)
