...
class ClusterRequestSearchForm(forms.Form):
    ...
    project_name = forms.CharField(
        label="Project Title", max_length=100, required=False
    )
    username = forms.CharField(label="Username", max_length=100, required=False)
...
class AllocationPeriodChoiceField(forms.ModelChoiceField):
    def __init__(self, *args, **kwargs):
        ...
        if self.computing_allowance is not None and not isinstance(
    ...
    def label_from_instance(self, obj):
        ...
        return (
            f"{obj.name} ({obj.start_date} - {obj.end_date}) ({num_service_units} SUs)"
        )
