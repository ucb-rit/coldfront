...
def get_secure_dir_manage_user_request_objects(self, action):
    ...
    request_obj = SecureDirAddUserRequest if add_bool else SecureDirRemoveUserRequest
    request_status_obj = (
        SecureDirAddUserRequestStatusChoice
        if add_bool
        else SecureDirRemoveUserRequestStatusChoice
    )
...
class SecureDirectoryAddUserRequestRunner(SecureDirectoryManageUserRequestRunner):
    ...
    def _send_email_to_admins(self):
        user_str = (
            f"{self._user.first_name} {self._user.last_name} ({self._user.email})"
        )
        ...
        template_name = "email/secure_dir_request/new_secure_dir_add_user_request.txt"
