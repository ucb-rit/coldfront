...
class TestProjectViewSetList:
    ...
    def test_filter_with_multiple_allowance_types(self, api_client, api_test_data):
        ...
        if len(codes_list) >= 2:
            ...
            for result in json["results"]:
                assert result["name"].startswith(code1) or result["name"].startswith(
                    code2
                )
        ...
        if len(codes_list) >= 3:
            ...
            for result in json["results"]:
                assert (
                    result["name"].startswith(code1)
                    or result["name"].startswith(code2)
                    or result["name"].startswith(code3)
                )
    ...
    def test_filter_combined(self, api_client, api_test_data):
        ...
        combined_count = Project.objects.filter(
            status__name="Active", name__istartswith=code
        ).count()
        assert json["count"] == combined_count
