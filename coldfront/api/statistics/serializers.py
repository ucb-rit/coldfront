...
from coldfront.core.statistics.models import Job, Node
...
class JobSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        instance.jobslurmid = validated_data.get("jobslurmid", instance.jobslurmid)
        instance.submitdate = validated_data.get("submitdate", instance.submitdate)
        ...
        instance.jobstatus = validated_data.get("jobstatus", instance.jobstatus)
        instance.partition = validated_data.get("partition", instance.partition)
    ...
    def validate(self, data):
        ...
        if startdate and enddate and enddate < startdate:
            message = (
                f"Job end date {enddate} occurs before Job start date {startdate}."
            )
            self.logger.error(message)
            raise serializers.ValidationError(message)
        if submitdate and enddate and enddate < submitdate:
            message = (
                f"Job end date {enddate} occurs before Job submit date {submitdate}."
            )
            self.logger.error(message)
            raise serializers.ValidationError(message)
    ...
    def validate_accountid(self, account):
        ...
        # existing one.
        if self.instance:
            if self.instance.accountid and account.pk != self.instance.accountid.pk:
                message = (
