...
class TestJobBase(TestAPIBase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create(username="user0", email="user0@nonexistent.com")
        ...
        self.project = Project.objects.create(name="fc_project", status=project_status)
        status = ProjectUserStatusChoice.objects.get(name="Active")
        user_role = ProjectUserRoleChoice.objects.get(name="User")
        self.project_user = ProjectUser.objects.create(
            user=self.user, project=self.project, role=user_role, status=status
        )
        manager_role = ProjectUserRoleChoice.objects.get(name="Manager")
        ProjectUser.objects.create(
            user=self.pi, project=self.project, role=manager_role, status=status
        )
        ...
        self.default_start = display_time_zone_date_to_utc_datetime(period_start_date)
        self.default_end = (
            display_time_zone_date_to_utc_datetime(period_end_date)
            + timedelta(hours=24)
            - timedelta(microseconds=1)
        )

        # Create a "CLUSTER_NAME Compute" Allocation for the Project.
        allocation_objects = create_project_allocation(self.project, Decimal("1000.00"))
        ...
        self.allocation_user_attribute = allocation_objects.allocation_user_attribute
