...
class TestCanSubmitJobView(TestJobBase):
    ...
    def assert_result(
        self, job_cost, user_id, account_id, status_code, success, message
    ):
        """Check that the given parameters produce the given status
        code, success, and message."""
        response = self.client.get(
            TestCanSubmitJobView.get_url(job_cost, user_id, account_id)
        )
    ...
    def test_compute_allocation_not_expected_always_allowed(self):
        ...
        for prefix in prefixes_with_allowances:
            ...
            allowance = ComputingAllowance(
                computing_allowance_interface.allowance_from_project(self.project)
            )
            ...
            self.assert_result(job_cost, "0", self.project.name, 200, result, message)
        ...
        for project_name in departmental_project_names:
            ...
            with enable_deployment("LRC"):
                ...
                self.allocation.resources.clear()
                message = (
                    f"Account {self.project.name} has no active compute allocation."
                )
                self.assert_result(
    ...
    def test_cost_exceeds_allocation(self):
        ...
        self.assertEqual(self.account_usage.value, Decimal(settings.ALLOCATION_MIN))
        ...
        self.user_account_usage.value = Decimal(self.allocation_user_attribute.value)
