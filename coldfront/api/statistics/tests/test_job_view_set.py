...
from coldfront.api.statistics.utils import (
    convert_utc_datetime_to_unix_timestamp,
...
from coldfront.core.user.models import ExpiringToken, UserProfile
...
class TestJobList(TestJobBase):
    ...
    def setUp(self):
        ...
        for i in range(self.num_projects):
            ...
            allocation_objects = create_project_allocation(project, allocation_amount)
        ...
        for i in range(self.num_users):
            ...
            project = Project.objects.get(name=f"fc_project_{i // self.num_projects}")
        ...
        index = 12
        dt = self.default_start.replace(hour=index, minute=0, second=0, microsecond=0)
    ...
    def test_jobstatus_filter(self):
        ...
        results_dict = self.assert_results(url, status_code, count)
        self.assertEqual(results_dict[str(job.jobslurmid)]["jobstatus"], jobstatus)
    ...
    def test_amount_filters(self):
        ...
        count = 8
        self.assert_results(TestJobList.get_url(min_amount="250"), status_code, count)
        self.assert_results(TestJobList.get_url(max_amount="250"), status_code, count)
        self.assert_results(
            TestJobList.get_url(min_amount="250", max_amount="250"), status_code, count
        )
        ...
        results_dict = self.assert_results(
            TestJobList.get_url(min_amount=minimum, max_amount=maximum), status_code, 8
        )
        for jobslurmid in results_dict:
        ...
        for jobslurmid in results_dict:
            self.assertLessEqual(Decimal(results_dict[jobslurmid]["amount"]), maximum)
    ...
    def test_partition_filter(self):
        ...
        status_code, count = 200, 1
        results_dict = self.assert_results(url, status_code, count)
        self.assertEqual(results_dict[str(job.jobslurmid)]["partition"], partition)
    ...
    def test_end_time_filter(self):
        ...
        end_dt = self.default_start.replace(hour=15, minute=0, second=0, microsecond=0)
    ...
    def test_multiple_filters(self):
        ...
        start_time = convert_utc_datetime_to_unix_timestamp(start_dt)
        end_dt = self.default_start.replace(hour=18, minute=0, second=0, microsecond=0)
        end_time = convert_utc_datetime_to_unix_timestamp(end_dt)
    ...
    def test_result_ordering(self):
        ...
        for job in jobs:
            self.assertIn("submitdate", job)
            submitdate = datetime.strptime(job["submitdate"], "%Y-%m-%dT%H:%M:%SZ")
    ...
    def test_total_amount_field(self):
        ...
        expected_total = Job.objects.aggregate(total_amount=Sum("amount"))[
            "total_amount"
        ]
    ...
    def test_total_cpu_time_field(self):
        ...
        expected_total = Job.objects.aggregate(total_cpu_time=Sum("cpu_time"))[
            "total_cpu_time"
        ]
...
class TestJobSerializer(TestJobBase):
    ...
    def test_required_fields(self):
        ...
        for field in required_fields:
            ...
            response = self.client.post(self.post_url, data, format="json")
            self.assertEqual(response.status_code, 400)
            self.assertEqual("This field is required.", response.json()[field][0])
    ...
    def test_non_null_amount(self):
        ...
        response = self.client.post(self.post_url, data, format="json")
        self.assertEqual(response.status_code, 400)
        self.assertEqual("This field may not be null.", response.json()["amount"][0])
        self.assertFalse(Job.objects.all())
    ...
    def test_invalid_datetime_format(self):
        ...
        for field, value in fields.items():
            ...
            response = self.client.post(self.post_url, data, format="json")
            self.assertEqual(response.status_code, 400)
            self.assertIn("Datetime has wrong format.", response.json()[field][0])
        self.assertFalse(Job.objects.all())
    ...
    def test_no_user_account_association(self):
        ...
        self.assertIn("non_field_errors", json)
        message = (
            f"User {self.user.username} is not a member of account {self.project.name}."
        )
        self.assertEqual(json["non_field_errors"][0], message)
    ...
    def test_no_active_compute_allocation(self):
        ...
        self.allocation.status = AllocationStatusChoice.objects.get(name="Expired")
    ...
    def test_no_active_compute_allocation_lrc_departmental(self):
        ...
        departmental_project_names = ("alice", "alsacc", "etna", "nano", "vulcan")
        other_resource = Resource.objects.get(name="BALDUR Compute")
        for project_name in departmental_project_names:
            message = f"Account {project_name} has no active compute allocation."
            self.project.name = project_name
            self.project.save()
            self.data["accountid"] = project_name
            resource = Resource.objects.get(name=f"{project_name.upper()} Compute")
            self.allocation.resources.add(resource)
            # The allocation is expired.
            self.allocation.status = AllocationStatusChoice.objects.get(name="Expired")
            self.allocation.save()
    ...
    def test_userid_changed_on_put(self):
        ...
        self.assertEqual(response.status_code, 201)
        data = self.data.copy()
        user = User.objects.create(username="user1", email="user1@nonexistent.com")
        user_profile = UserProfile.objects.get(user=user)
        ...
        response = self.client.put(
            TestJobSerializer.put_url(self.data["jobslurmid"]), data, format="json"
        )
        self.assertEqual(response.status_code, 400)
        json = response.json()
        self.assertIn("userid", json)
        message = (
            f"Specified user {user} does not match already associated user {self.user}."
        )
    ...
    def test_accountid_changed_on_put(self):
        ...
        data = self.data.copy()
        project_status = ProjectStatusChoice.objects.get(name="Active")
        project = Project.objects.create(name="OTHER_PROJECT", status=project_status)
...
class TestJobViewSet(TestJobBase):
    ...
    def get_usage_values(allocation_objects):
        ...
        """
        account_usage = AllocationAttributeUsage.objects.get(
            pk=allocation_objects.allocation_attribute_usage.pk
        )
        user_account_usage = AllocationUserAttributeUsage.objects.get(
    ...
    def test_get(self):
        ...
        for field in ("submitdate", "startdate", "enddate"):
            self.assertEqual(
                job[field].replace("T", " ").replace("Z", ""), self.data[field]
            )
    ...
    def test_post(self):
        ...
        for field in ("submitdate", "startdate", "enddate"):
            self.assertEqual(
                getattr(job, field).strftime(self.date_format), self.data[field]
            )
    ...
    def test_patch_job_not_supported(self):
        ...
        supported."""
        response = self.client.patch(TestJobViewSet.put_url("1"), {}, format="json")
        self.assertEqual(response.status_code, 200)
        json = response.json()
    ...
    def test_put(self):
        ...
        response = self.client.put(
            TestJobViewSet.put_url(self.data["jobslurmid"]), data, format="json"
        )
        self.assertEqual(response.status_code, 200)
        job.refresh_from_db()
        self.assertEqual(job.jobslurmid, self.data["jobslurmid"])
        ...
        self.assertEqual(job.enddate.strftime(self.date_format), data["enddate"])
        self.assertEqual(job.userid, self.user)
        self.assertEqual(job.accountid, self.project)
    ...
    def test_put_creates_if_nonexistent(self):
        ...
        response = self.client.put(
            TestJobViewSet.put_url(self.data["jobslurmid"]), self.data, format="json"
        )
        self.assertEqual(response.status_code, 200)
        job = Job.objects.get(jobslurmid=self.data["jobslurmid"])
        self.assertEqual(job.jobslurmid, self.data["jobslurmid"])
    ...
    def test_optional_amount_skips_usage(self):
        ...
        data = self.data.copy()
        data.pop("amount")
        response = self.client.post(TestJobViewSet.post_url, data, format="json")
        self.assertEqual(response.status_code, 201)
    ...
    def test_other_requests_not_allowed(self):
        ...
        URL) are not allowed."""
        self.assertEqual(self.client.post(TestJobViewSet.put_url("1")).status_code, 405)
    ...
    def test_post_invalid_job_start_date(self):
        ...
        pre_allocation_usage, pre_user_usage = allocation_usage.value, user_usage.value
        ...
        data["submitdate"] = (
            allocation_objects.allocation.start_date - timedelta(days=6)
        ).strftime(self.date_format)
        data["startdate"] = (
            allocation_objects.allocation.start_date - timedelta(days=5)
        ).strftime(self.date_format)
        ...
        self.assertTrue(Job.objects.filter(jobslurmid=data["jobslurmid"]).exists())
    ...
    def test_post_invalid_job_end_date(self):
        ...
        # Allocation's end_date.
        data["submitdate"] = (
            allocation_objects.allocation.start_date + timedelta(days=4)
        ).strftime(self.date_format)
        data["startdate"] = (
            allocation_objects.allocation.start_date + timedelta(days=5)
        ).strftime(self.date_format)
        data["enddate"] = (
            allocation_objects.allocation.end_date + timedelta(days=5)
        ).strftime(self.date_format)
    ...
    def test_post_job_missing_dates(self):
        ...
        for date in ["submitdate", "startdate"]:
            ...
            self.assertFalse(Job.objects.filter(jobslurmid=data["jobslurmid"]).exists())
        ...
        for allocation_type in allocation_types:
            ...
            for date in ["start_date", "end_date"]:
                ...
                if date == "start_date" or (
                    date == "end_date"
                    and allocation_type in allocation_types_with_end_dates
                ):
                    self.assertEqual(pre_allocation_usage, allocation_usage.value)
                    self.assertEqual(pre_user_usage, user_usage.value)
                else:
