...
from coldfront.api.allocation.filters import (
    AllocationAttributeFilter,
    AllocationFilter,
    AllocationUserAttributeFilter,
    AllocationUserFilter,
    ClusterAccessRequestFilter,
)
from coldfront.api.allocation.serializers import (
    AllocationAttributeSerializer,
...
class AllocationAttributeViewSet(
...
class AllocationUserAttributeViewSet(
    mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet
):
...
class HistoricalAllocationAttributeViewSet(
    mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet
):
...
class HistoricalAllocationUserAttributeViewSet(
    mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet
):
    ...
    def get_queryset(self):
        allocation_user_pk = self.kwargs.get("allocation_user_pk", None)
        allocation_user_attributes = HistoricalAllocationUserAttribute.objects.all()
        if allocation_user_pk:
            allocation_user_attributes = allocation_user_attributes.filter(
                allocation_user=allocation_user_pk
            )
...
class ClusterAccessRequestViewSet(
