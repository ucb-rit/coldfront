    ...
    def test_no_filters(self):
        ...
        self.assertEqual(
            ...
            HistoricalAllocationAttribute.objects.filter(
                id=allocation_attribute.pk
            ).count(),
        )
        self.assertIsNone(json["next"])
        self.assertIsNone(json["previous"])
        for result in json["results"]:
            historical_allocation_attribute = HistoricalAllocationAttribute.objects.get(
                history_id=result["history_id"]
            )
            self.assertEqual(
                historical_allocation_attribute.history_id, result["history_id"]
            )
            ...
            self.assertEqual(
                historical_allocation_attribute.history_type, result["history_type"]
            )
            self.assertEqual(
            ...
            self.assertEqual(
                historical_allocation_attribute.allocation.pk, result["allocation"]
            )
            self.assertEqual(
                historical_allocation_attribute.history_user, result["history_user"]
            )
...
class TestRetrieveAllocationAttributeHistory(TestAllocationBase):
    ...
    def test_valid_pk(self):
        ...
        historical_allocation_attribute = HistoricalAllocationAttribute.objects.filter(
            id=allocation_attribute.id
        ).first()
        url = self.endpoint_url(
            allocation.pk, allocation_attribute.pk, historical_allocation_attribute.pk
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, HTTPStatus.OK)
        json = response.json()
    ...
    def test_invalid_pk(self):
        ...
        pk = sum(HistoricalAllocationAttribute.objects.values_list("pk", flat=True)) + 1
