...
class TestListAllocationAttributes(TestAllocationBase):
    ...
    def test_no_filters(self):
        ...
        for result in json["results"]:
            allocation_attribute = AllocationAttribute.objects.get(id=result["id"])
            ...
            self.assertEqual(allocation_attribute.allocation.id, result["allocation"])
            ...
            self.assertEqual(
                usage.allocation_attribute.id, result["usage"]["allocation_attribute"]
            )
...
class TestRetrieveAllocationAttributes(TestAllocationBase):
    ...
    def endpoint_url(self, allocation_pk, attribute_pk):
        """Return the URL for the endpoint."""
        return f"{self.allocations_base_url}{allocation_pk}/attributes/{attribute_pk}/"
