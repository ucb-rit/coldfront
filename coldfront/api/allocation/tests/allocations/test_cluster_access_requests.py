...
class TestClusterAccessRequestsBase(TestAllocationBase):
    ...
    def setUp(self):
        ...
        for i in range(4):
            kwargs = {
                "allocation_user": AllocationUser.objects.get(
                    user__username=f"user{i}", allocation__project__name="fc_project0"
                ),
        ...
        self.allocation_user0 = AllocationUser.objects.get(
            user=self.user0, allocation__project=self.fc_project0
        )
...
class TestListClusterAccessRequests(TestClusterAccessRequestsBase):
    ...
    def test_no_filters(self):
        ...
        for result in json["results"]:
            cluster_access_request = ClusterAccessRequest.objects.get(pk=result["id"])
            assert_cluster_access_request_serialization(
...
class TestUpdatePatchClusterAccessRequests(TestClusterAccessRequestsBase):
    ...
    def _assert_complete_emails_sent(self):
        email_body = [
            f"now has access to the project {self.fc_project0.name}.",
    ...
    def _assert_denial_emails_sent(self):
        email_body = [
            f"access request under project {self.fc_project0.name}",
            f"and allocation {self.allocation_user0.allocation.pk} has been denied.",
        ]
    ...
    def _assert_pre_state(self):
        ...
        self.assertNotEqual(
            self.allocation_su_attr.value, self.allocation_user_su_attr.value
        )
        self.assertFalse(
            self._get_cluster_account_status_attr(self.allocation_user0).exists()
        )
        self.assertEqual(self.request0.allocation_user.pk, self.allocation_user0.pk)

    def _assert_post_state(
        self, pre_time, post_time, status, check_username_clusteruid=True, check_su=True
    ):
        """Assert that the relevant objects have the expected state,
        ...
        self.assertEqual(
            self._get_cluster_account_status_attr(self.allocation_user0).first().value,
            attr_status,
        )
        self.assertEqual(self.request0.allocation_user.pk, self.allocation_user0.pk)
    ...
    def test_read_only_fields_ignored(self):
        ...
        self.assertNotEqual(
            data.get("allocation_user").get("id"), self.request0.allocation_user.pk
        )
    ...
    def test_exception_causes_rollback_denial(self):
        ...
        url = self.pk_url(BASE_URL, self.request0.pk)
        data = {"completion_time": utc_now_offset_aware(), "status": "Denied"}
        with patch.object(ClusterAccessRequestDenialRunner, "run", raise_exception):
            response = self.client.patch(url, data)
