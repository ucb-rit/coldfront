...
class TestListAllocationUserAttributeHistory(TestAllocationBase):
    ...
    def test_response_format(self):
        ...
        url = self.endpoint_url(allocation_user.pk, allocation_user_attribute.pk)
    ...
    def test_no_filters(self):
        ...
        self.assertEqual(
            ...
            HistoricalAllocationUserAttribute.objects.filter(
                id=allocation_user_attribute.pk
            ).count(),
        )
        ...
        for result in json["results"]:
            ...
            self.assertEqual(
                historical_allocation_user_attribute.history_id, result["history_id"]
            )
            self.assertEqual(historical_allocation_user_attribute.id, result["id"])
            ...
            self.assertEqual(
                historical_allocation_user_attribute.allocation.pk, result["allocation"]
            )
            self.assertEqual(
...
class TestRetrieveAllocationUserAttributeHistory(TestAllocationBase):
    ...
    def test_valid_pk(self):
        ...
        historical_allocation_user_attribute = (
            HistoricalAllocationUserAttribute.objects.filter(
                id=allocation_user_attribute.id
            ).first()
        )
        url = self.endpoint_url(
            allocation_user.pk,
            allocation_user_attribute.pk,
    ...
    def test_invalid_pk(self):
        ...
        pk = (
            sum(HistoricalAllocationUserAttribute.objects.values_list("pk", flat=True))
            + 1
        )
        url = self.endpoint_url(allocation_user.pk, allocation_user_attribute.pk, pk)
        response = self.client.get(url)
