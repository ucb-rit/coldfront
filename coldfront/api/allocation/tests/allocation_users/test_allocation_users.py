...
class TestListAllocationUsers(TestAllocationBase):
    ...
    def test_no_filters(self):
        ...
        for result in json["results"]:
            ...
            self.assertEqual(allocation_user.allocation.id, result["allocation"])
            self.assertEqual(allocation_user.user.username, result["user"])
            self.assertEqual(allocation_user.allocation.project.name, result["project"])
    ...
    def test_resources_filter(self):
        ...
        for result in json["results"]:
            self.assertEqual(next(allocation_ids_iterator), result["allocation"])
            self.assertTrue(
                Resource.objects.filter(allocation=allocation, name=resource_name)
            )
        ...
        for result in json["results"]:
            ...
            self.assertTrue(
                Resource.objects.filter(allocation=allocation, name="Other Compute")
            )
