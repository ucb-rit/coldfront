...
class AllocationAttributeSerializer(serializers.ModelSerializer):
    ...
    usage = AllocationAttributeUsageSerializer(source="allocationattributeusage")
...
class ClusterAccessRequestSerializer(serializers.ModelSerializer):
    ...
    status = serializers.SlugRelatedField(
        slug_field="name", queryset=ClusterAccessRequestStatusChoice.objects.all()
    )
    ...
    def validate(self, data):
        ...
        if "cluster_uid" in data:
            ...
            if queryset.exists():
                if (
                    queryset.first().pk
                    != self.instance.allocation_user.user.userprofile.pk
                ):
                    message = f"A user with cluster_uid {cluster_uid} already exists."
                    raise serializers.ValidationError(message)
