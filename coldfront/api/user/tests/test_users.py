...
class TestCreateUsers(TestUsersBase):
    ...
    def test_permissions_by_role(self):
        ...
        url = BASE_URL
        method = "POST"
        users = [(self.user0, True), (self.staff_user, True), (self.superuser, True)]
...
class TestRetrieveUsers(TestUsersBase):
    ...
    def test_valid_pk_lrc(self):
        ...
        for inputs, outputs in inputs_and_outputs.items():
            ...
            self.assertEqual(response.status_code, HTTPStatus.OK)
            json = response.json()
            profile_serializer_fields = PROFILE_SERIALIZER_FIELDS + (
                "billing_activity",
                "host_user",
                "host_user_lbl_email",
            )
            ...
            self.assertEqual(
                json_profile["host_user_lbl_email"], expected_host_user_lbl_email
            )
